...
### 3.6 Add explicit MOE Learning Outcome mapping — auditable sequence ✅ DONE

> **`SCOPE_AND_SEQUENCE.md` exists**, generated by
> `scripts/gen-scope-sequence.mjs` from `curriculum.js`, `decodability.js`
> and the progression gate. Every phase, stage, target sound, sample word,
> mastery bar, code-release tier and gate criterion in it is read out of the
> running code.
>
> **Generated, because a hand-written one is not auditable.** It drifts the
> moment a stage moves, and a specialist reading it has no way to tell
> whether it still describes the app. `scopeSequence.test.js` regenerates and
> compares byte-for-byte, and CI runs `npm run check:scope`, so a stale copy
> fails the build. The guard was verified to fail both ways: editing the
> document, and changing a gate constant without regenerating.
>
> Two traps worth recording, because both would have produced a green test
> that guaranteed nothing:
>
> - The script's CLI branch ran on **import**, so the test would have
>   regenerated the file it was about to check and passed whatever had
>   drifted. It is now guarded to run only when invoked directly.
> - A `?? 0.2` fallback on a mistyped constant name would have printed a
>   plausible wrong number in the one document whose purpose is being
>   checkable. `pct()` now throws on a non-number.
>
> A per-stage word-count column was dropped rather than shipped wrong:
> structural stages such as `cvc-a` are filtered out of the bank by word
> shape, not by a matching group name, so the naive count read 0 for most
> stages and 132 for one. The document says so.
>
> `PROGRESSION_GATE` moved to `progressionGate.js` — pure policy with no
> runtime dependencies, so tooling outside the browser can read the numbers
> without pulling in the store. `progression.js` re-exports it.
>
> **Still open below:** the per-item `component` / `learningOutcome` /
> `marks` / `reviewedBy` / `syllabusVersion` metadata, and any mapping to
> MOE or another published syllabus. The document states plainly that it
> claims neither that nor an evidence base, because a scope and sequence
