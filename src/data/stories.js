/**
 * Giri Stories — 4-Band Decodable Reading System
 *
 * Band A: Core Decodable Minis     (25–45 words, 4–6 sentences)
 * Band B: Decodable Story Readers  (45–80 words, 6–10 sentences)
 * Band C: Fluency Readers          (80–140 words, paragraph format)
 * Band D: Bridge / Chapter Readers (140–250 words, multi-paragraph)
 *
 * Each band progresses on 5 tracks:
 *   1. Code complexity   (short vowels → long → r-controlled → diphthongs)
 *   2. Text length       (25 → 250 words)
 *   3. Sentence complexity (one clause → full paragraph syntax)
 *   4. Narrative complexity (one event → delayed resolution)
 *   5. Support load      (strong pattern → mostly independent)
 *
 * HFW Tiers:
 *   Tier 1: the, a, and, is, it, to, in, on, my
 *   Tier 2: was, said, we, he, she, they, of, for, with, went
 *   Tier 3: could, would, should, there, where, one, some, were
 *   Band A → Tier 1 only | Band B → 1–2 | Band C → 1–3 | Band D → flexible
 *
 * Sight-word weave: the ten Band A minis (core-a-01…10) are aligned 1:1 to
 * the ten easy sight-word quests. Each story's `sightWords` list is the
 * cumulative set of words its quest (and all earlier quests) have
 * pre-taught, so Story N may use exactly what Quests 1…N introduced. The
 * decodability validator treats those as legal — see getIntroducedSightWords
 * in sightwords.js and the `sight` route in decodability.js.
 */

import { getIntroducedSightWords } from './sightwords.js';

export const BAND_META = [
  {
    band: 'A', level: 1,
    label: 'Core Decodable Minis',
    targetSounds: 'Short a \u00b7 e \u00b7 i \u00b7 o \u00b7 u',
    prop: '\ud83c\udfa9 Red Hat', color: '#ef4444', bg: '#fee2e2',
    mascotState: 'holdCard',
    wordRange: '25\u201345', sentenceRange: '4\u20136',
  },
  {
    band: 'B', level: 2,
    label: 'Decodable Story Readers',
    targetSounds: 'a-e \u00b7 i-e \u00b7 o-e \u00b7 ai \u00b7 ay \u00b7 ee \u00b7 oa',
    prop: '\ud83c\udf82 Big Cake', color: '#f97316', bg: '#ffedd5',
    mascotState: 'celebrate',
    wordRange: '45\u201380', sentenceRange: '6\u201310',
  },
  {
    band: 'C', level: 3,
    label: 'Fluency Readers',
    targetSounds: 'ar \u00b7 or \u00b7 er \u00b7 ir \u00b7 ur + digraphs',
    prop: '\u2b50 Shiny Star', color: '#8b5cf6', bg: '#f3e8ff',
    mascotState: 'thinking',
    wordRange: '80\u2013140', sentenceRange: '10\u201318',
  },
  {
    band: 'D', level: 4,
    label: 'Bridge Readers',
    targetSounds: 'oi \u00b7 oy \u00b7 ou \u00b7 ow \u00b7 au \u00b7 aw \u00b7 air \u00b7 are \u00b7 ear',
    prop: '\ud83c\udf50 Rare Pear', color: '#0d9488', bg: '#ccfbf1',
    mascotState: 'trophy',
    wordRange: '140\u2013250', sentenceRange: '15\u201330',
  },
];

// ── Stories ───────────────────────────────────────────────────────────────────

export const STORIES = [

  // ╔══════════════════════════════════════════════════════════════════════════╗
  // ║  BAND A — Core Decodable Minis  (16 books)                            ║
  // ║  25–45 words · 4–6 sentences · Tier 1 HFW only · refrain max 2×      ║
  // ╚══════════════════════════════════════════════════════════════════════════╝

  // ── Short-a set (4) ───────────────────────────────────────────────────────

  {
    id: 'core-a-01',
    band: 'A', level: 1, phase: 'short-a',
    title: "Giri's Red Hat",
    emoji: '\ud83c\udfa9',
    mascotState: 'holdCard',
    illustration: 'giri_level01_story01_red-hat.png.jpg',
    targetGraphemes: ['a'],
    allowedHFWTier: 1,
    targetWordCount: 42,
    actualWordCount: 43,
    decodableRatio: 0.93,
    textType: 'mini-decodable',
    refrainCount: 0,
    sightWords: getIntroducedSightWords('core-a-01'),
    lines: [
      { type: 'text', text: 'Giri had a red hat. It sat on top.' },
      { type: 'text', text: 'A gust of wind! The hat did not stop.' },
      { type: 'text', text: 'Giri ran and ran. The hat fell flat.' },
      { type: 'text', text: 'Plop! It fell on a big cat.' },
      { type: 'end', text: 'The cat sat on the hat. Giri got it back.' },
    ],
    talkAboutIt: ['Where did the hat land at the end?'],
  },

  {
    id: 'core-a-02',
    band: 'A', level: 1, phase: 'short-a',
    title: 'Giri and the Cat',
    emoji: '\ud83d\udc31',
    mascotState: 'holdCard',
    illustration: 'giri_level01_story02_cat.jpg',
    targetGraphemes: ['a'],
    allowedHFWTier: 1,
    targetWordCount: 40,
    actualWordCount: 44,
    decodableRatio: 0.93,
    textType: 'mini-decodable',
    refrainCount: 0,
    sightWords: getIntroducedSightWords('core-a-02'),
    lines: [
      { type: 'text', text: 'A cat sat next to Giri.' },
      { type: 'text', text: 'Giri did a jig. Then the cat did a jig!' },
      { type: 'text', text: 'Giri had a bit of ham in his bag.' },
      { type: 'text', text: 'He held it up. The cat ran to it. Snap!' },
      { type: 'end', text: 'Then the cat sat on his lap. A nap!' },
    ],
    talkAboutIt: ['Where did the cat nap at the end?'],
  },

  {
    id: 'core-a-03',
    band: 'A', level: 1, phase: 'short-a',
    title: 'Giri and the Jam',
    emoji: '\ud83c\udf53',
    mascotState: 'encourage',
    illustration: 'giri_level01_story03_jam.jpg',
    targetGraphemes: ['a'],
    allowedHFWTier: 1,
    targetWordCount: 42,
    actualWordCount: 42,
    decodableRatio: 0.88,
    textType: 'mini-decodable',
    refrainCount: 0,
    sightWords: getIntroducedSightWords('core-a-03'),
    lines: [
      { type: 'text', text: 'Giri had a pan of jam.' },
      { type: 'text', text: '"This jam is for my ham," he said.' },
      { type: 'text', text: 'An ant ran up. It sat in the jam!' },
      { type: 'text', text: 'Giri got a twig. He got the ant off.' },
      { type: 'end', text: 'So the ant had a bit. It had the rest.' },
    ],
    talkAboutIt: ['How did Giri get the ant off?'],
  },

  {
    id: 'core-a-04',
    band: 'A', level: 1, phase: 'short-a',
    title: "Giri's Nap",
    emoji: '\ud83d\ude34',
    mascotState: 'neutral',
    illustration: 'giri_level01_story04_nap.jpg',
    targetGraphemes: ['a'],
    allowedHFWTier: 1,
    targetWordCount: 40,
    actualWordCount: 43,
    decodableRatio: 0.91,
    textType: 'mini-decodable',
    refrainCount: 0,
    sightWords: getIntroducedSightWords('core-a-04'),
    lines: [
      { type: 'text', text: 'It was a hot day. Giri had a nap.' },
      { type: 'text', text: '"I will nap in the sun too," said the cat.' },
      { type: 'text', text: 'Tap, tap! He sat up fast.' },
      { type: 'text', text: 'A bug ran on his leg. "I must go!"' },
      { type: 'end', text: 'The bug ran off. He can nap at last.' },
    ],
    talkAboutIt: ['Why did Giri want a nap?'],
  },

  // ── Short-e / i set (4) ───────────────────────────────────────────────────

  {
    id: 'core-a-05',
    band: 'A', level: 1, phase: 'short-ei',
    title: 'Giri and the Hen',
    emoji: '\ud83d\udc14',
    mascotState: 'holdCard',
    illustration: 'giri_level01_story05_hen.jpg',
    targetGraphemes: ['e', 'i'],
    allowedHFWTier: 1,
    targetWordCount: 42,
    actualWordCount: 44,
    decodableRatio: 0.86,
    textType: 'mini-decodable',
    refrainCount: 0,
    sightWords: getIntroducedSightWords('core-a-05'),
    lines: [
      { type: 'text', text: 'A white hen sat on ten eggs.' },
      { type: 'text', text: 'One egg did wiggle. Then it did a big wiggle!' },
      { type: 'text', text: 'Crack! Now a wet chick! It is pink.' },
      { type: 'text', text: 'The chick did not sit. It ran into a log.' },
      { type: 'end', text: 'Giri got it. All is well. One white chick!' },
    ],
    talkAboutIt: ['What came out of the white egg?'],
  },

  {
    id: 'core-a-06',
    band: 'A', level: 1, phase: 'short-ei',
    title: "Giri's Big Dig",
    emoji: '\u26cf\ufe0f',
    mascotState: 'celebrate',
    illustration: 'giri_level01_story06_dig.jpg',
    targetGraphemes: ['i'],
    allowedHFWTier: 1,
    targetWordCount: 42,
    actualWordCount: 44,
    decodableRatio: 0.86,
    textType: 'mini-decodable',
    refrainCount: 0,
    sightWords: getIntroducedSightWords('core-a-06'),
    lines: [
      { type: 'text', text: 'Giri dug a big pit. Dig, dig, dig!' },
      { type: 'text', text: 'Clink! What is in the pit? A tin lid!' },
      { type: 'text', text: 'The lid had a map. What can this be?' },
      { type: 'text', text: '"I must dig again!" So Giri dug and dug.' },
      { type: 'end', text: 'A red gem! "Do it again!" Giri came back.' },
    ],
    talkAboutIt: ['What did Giri dig up in the end?'],
  },

  {
    id: 'core-a-07',
    band: 'A', level: 1, phase: 'short-ei',
    title: 'Giri and the Bed',
    emoji: '\ud83d\udecf\ufe0f',
    mascotState: 'neutral',
    illustration: 'giri_level01_story07_bed.jpg',
    targetGraphemes: ['e', 'i'],
    allowedHFWTier: 1,
    targetWordCount: 40,
    actualWordCount: 45,
    decodableRatio: 0.89,
    textType: 'mini-decodable',
    refrainCount: 0,
    sightWords: getIntroducedSightWords('core-a-07'),
    lines: [
      { type: 'text', text: 'Giri had a big rip in his red bed.' },
      { type: 'text', text: '"I know I can fix it!" He got a pin.' },
      { type: 'text', text: 'When did he fix it? Fast, fast!' },
      { type: 'text', text: 'A cat sat above the bed. A moth ran over.' },
      { type: 'end', text: 'There! The rip is hid. The bed is best!' },
    ],
    talkAboutIt: ['How did Giri fix the rip in his bed?'],
  },

  {
    id: 'core-a-08',
    band: 'A', level: 1, phase: 'short-ei',
    title: "Giri's Tin Lid",
    emoji: '\ud83e\ude99',
    mascotState: 'encourage',
    illustration: 'giri_level01_story08_tin-lid.jpg',
    targetGraphemes: ['i'],
    allowedHFWTier: 1,
    targetWordCount: 38,
    actualWordCount: 45,
    decodableRatio: 0.87,
    textType: 'mini-decodable',
    refrainCount: 0,
    sightWords: getIntroducedSightWords('core-a-08'),
    lines: [
      { type: 'text', text: 'The lid fell off the tin. Clink! It hit the rim.' },
      { type: 'text', text: 'Giri hit the lid. "This is a bell!" Ding, ding!' },
      { type: 'text', text: 'A red hen ran up. She can tap!' },
      { type: 'text', text: 'Her taps and his taps were a song.' },
      { type: 'end', text: 'Ding! Tap! "We are a band!" said Giri.' },
    ],
    talkAboutIt: ['What did Giri and the hen make with taps?'],
  },

  // ── Short-o / u set (4) ───────────────────────────────────────────────────

  {
    id: 'core-a-09',
    band: 'A', level: 1, phase: 'short-ou',
    title: 'Giri and the Pot',
    emoji: '\ud83c\udf72',
    mascotState: 'holdCard',
    illustration: 'giri_level01_story02_pot.png.jpg',
    targetGraphemes: ['o'],
    allowedHFWTier: 1,
    targetWordCount: 42,
    actualWordCount: 45,
    decodableRatio: 0.87,
    textType: 'mini-decodable',
    refrainCount: 0,
    sightWords: getIntroducedSightWords('core-a-09'),
    lines: [
      { type: 'text', text: 'Giri had a hot pot on the log.' },
      { type: 'text', text: 'The pot did not stop. "What is this about?"' },
      { type: 'text', text: 'A frog sat on top. It has a name: Fizz.' },
      { type: 'text', text: 'Many frogs hop up! Ten frogs sit on the hot pot.' },
      { type: 'end', text: '"This is their pot!" What a story!' },
    ],
    talkAboutIt: ['Whose pot was it in the end?'],
  },

  {
    id: 'core-a-10',
    band: 'A', level: 1, phase: 'short-ou',
    title: 'Giri and the Pup',
    emoji: '\ud83d\udc36',
    mascotState: 'celebrate',
    illustration: 'giri_level01_story10_pup.jpg',
    targetGraphemes: ['u'],
    allowedHFWTier: 1,
    targetWordCount: 42,
    actualWordCount: 44,
    decodableRatio: 0.91,
    textType: 'mini-decodable',
    refrainCount: 0,
    sightWords: getIntroducedSightWords('core-a-10'),
    lines: [
      { type: 'text', text: 'A pup dug in the mud. "How much mud!"' },
      { type: 'text', text: '"Does the pup want a bath?"' },
      { type: 'text', text: 'Giri got a tub of suds. Some got on the pup!' },
      { type: 'text', text: 'Rub, rub! The mud runs off. Fun!' },
      { type: 'end', text: 'The pup is not in the mud. Giri is also wet!' },
    ],
    talkAboutIt: ['What did Giri wash off the pup?'],
  },

  {
    id: 'core-a-11',
    band: 'A', level: 1, phase: 'short-ou',
    title: 'Giri and the Dog',
    emoji: '\ud83d\udc15',
    mascotState: 'encourage',
    illustration: 'giri_level01_story11_dog.jpg',
    targetGraphemes: ['o'],
    allowedHFWTier: 1,
    targetWordCount: 40,
    actualWordCount: 45,
    decodableRatio: 0.96,
    textType: 'mini-decodable',
    refrainCount: 0,
    lines: [
      { type: 'text', text: 'Drip, drip. Mud dots on the path.' },
      { type: 'text', text: 'Giri went dot to dot. Dot, dot, dot.' },
      { type: 'text', text: 'At the end sat a lost dog on a log. Pant, pant!' },
      { type: 'text', text: 'Giri got broth and a snug rug. The dog slept.' },
      { type: 'end', text: 'Not lost. The dog is with a pal.' },
    ],
    talkAboutIt: ['How did Giri find the lost dog?'],
  },

  {
    id: 'core-a-12',
    band: 'A', level: 1, phase: 'short-ou',
    title: "Giri's Hot Cup",
    emoji: '\u2615',
    mascotState: 'neutral',
    illustration: 'giri_level01_story12_hot-cup.jpg',
    targetGraphemes: ['o', 'u'],
    allowedHFWTier: 1,
    targetWordCount: 40,
    actualWordCount: 42,
    decodableRatio: 0.98,
    textType: 'mini-decodable',
    refrainCount: 0,
    lines: [
      { type: 'text', text: 'Giri set a hot cup on the rug. Bad plan!' },
      { type: 'text', text: 'The cup fell. The rug got wet, wet, wet.' },
      { type: 'text', text: 'Mop, mop, mop! The spot is not wet.' },
      { type: 'text', text: 'Then the cat sat smack on that spot. Cats!' },
      { type: 'end', text: 'Next cup: not on the rug.' },
    ],
    talkAboutIt: ['Who took the clean spot at the end?'],
  },

  // ── Mixed review set (4) ──────────────────────────────────────────────────

  {
    id: 'core-a-13',
    band: 'A', level: 1, phase: 'mixed-short',
    title: 'Giri and the Bun',
    emoji: '\ud83c\udf5e',
    mascotState: 'holdCard',
    illustration: 'giri_level01_story03_bun.png.jpg',
    targetGraphemes: ['a', 'e', 'i', 'o', 'u'],
    allowedHFWTier: 1,
    targetWordCount: 42,
    actualWordCount: 44,
    decodableRatio: 0.93,
    textType: 'mini-decodable',
    refrainCount: 0,
    lines: [
      { type: 'text', text: 'Giri met a bun on a rug. The bun had legs!' },
      { type: 'text', text: 'Run, bun, run! It ran past the cat and the hen.' },
      { type: 'text', text: 'Snap! Giri got the bun at last.' },
      { type: 'text', text: 'The legs? Ants! Six ants had the bun.' },
      { type: 'end', text: 'Giri splits the bun with the ants.' },
    ],
    talkAboutIt: ['What were the bun’s "legs" really?'],
  },

  {
    id: 'core-a-14',
    band: 'A', level: 1, phase: 'mixed-short',
    title: 'Giri Gets Wet',
    emoji: '\ud83d\udca6',
    mascotState: 'celebrate',
    illustration: 'giri_level01_story04_wet.png.jpg',
    targetGraphemes: ['a', 'e', 'i', 'o', 'u'],
    allowedHFWTier: 1,
    targetWordCount: 40,
    actualWordCount: 45,
    decodableRatio: 0.89,
    textType: 'mini-decodable',
    refrainCount: 0,
    lines: [
      { type: 'text', text: 'Splish, splash! Giri got in a big tub of suds in the sun.' },
      { type: 'text', text: 'Suds on his legs. Suds on his chin. Fun!' },
      { type: 'text', text: 'Then Giri sat on a hot mat. Drip, drip.' },
      { type: 'text', text: 'The sun did its job. Giri got dry.' },
      { type: 'end', text: 'Wet, then dry. The sun wins.' },
    ],
    pretaught: ['dry'],
    talkAboutIt: ['What was the sun’s job in this story?'],
  },

  {
    id: 'core-a-15',
    band: 'A', level: 1, phase: 'mixed-short',
    title: 'Giri and the Box',
    emoji: '\ud83d\udce6',
    mascotState: 'neutral',
    illustration: 'giri_level01_story15_box.jpg',
    targetGraphemes: ['a', 'e', 'i', 'o', 'u'],
    allowedHFWTier: 1,
    targetWordCount: 42,
    actualWordCount: 44,
    decodableRatio: 0.95,
    textType: 'mini-decodable',
    refrainCount: 0,
    lines: [
      { type: 'text', text: 'The big tan box did buzz. Buzz, buzz!' },
      { type: 'text', text: 'Is it a bell? Is it a jet? Giri did a check.' },
      { type: 'text', text: 'A red bug! It sat on his hand. It did not bite.' },
      { type: 'text', text: 'Zip! The bug flit off.' },
      { type: 'end', text: 'A box can hold a big shock!' },
    ],
    pretaught: ['bite'],
    talkAboutIt: ['What did Giri guess before opening the box?'],
  },

  {
    id: 'core-a-16',
    band: 'A', level: 1, phase: 'mixed-short',
    title: "Giri's Big Run",
    emoji: '\ud83c\udfc3',
    mascotState: 'encourage',
    illustration: 'giri_level01_story16_big-run.jpg',
    targetGraphemes: ['a', 'e', 'i', 'o', 'u'],
    allowedHFWTier: 1,
    targetWordCount: 42,
    actualWordCount: 43,
    decodableRatio: 0.95,
    textType: 'mini-decodable',
    refrainCount: 0,
    lines: [
      { type: 'text', text: 'Giri ran. The pup ran. A big run in the hot sun!' },
      { type: 'text', text: 'Pant, pant! Run, run, run!' },
      { type: 'text', text: 'The pond! Giri got in. Splash! The pup got in. Splash!' },
      { type: 'text', text: 'In the end, the pond won.' },
      { type: 'end', text: 'The best end to a run is a splash.' },
    ],
    talkAboutIt: ['Who won the big run?'],
  },

  // ╔══════════════════════════════════════════════════════════════════════════╗
  // ║  BAND B — Decodable Story Readers  (12 books)                         ║
  // ║  45–80 words · 6–10 sentences · Tier 1–2 HFW · refrain 1–2×          ║
  // ║  Story shape: intro → problem → attempt → solution → ending           ║
  // ╚══════════════════════════════════════════════════════════════════════════╝

  {
    id: 'core-b-01',
    band: 'B', level: 2, phase: 'long-a',
    title: 'Giri Bakes a Cake',
    emoji: '\ud83c\udf82',
    mascotState: 'celebrate',
    illustration: 'giri_level02_story01_bake-cake.png.jpg',
    targetGraphemes: ['a_e', 'ai', 'ay'],
    allowedHFWTier: 2,
    targetWordCount: 70,
    actualWordCount: 79,
    decodableRatio: 0.94,
    textType: 'story-reader',
    refrainCount: 0,
    sightWords: getIntroducedSightWords('core-b-01'),
    lines: [
      { type: 'intro',   text: 'Giri had a great plan. He wanted to bake two cakes for his pal Jay.' },
      { type: 'label',   text: 'Problem:' },
      { type: 'beat',    text: 'But the cake stayed flat in the pan. It did not rise a bit!' },
      { type: 'label',   text: 'Attempt:' },
      { type: 'beat',    text: '"Wake up, cake!" Giri gave the pan a brave shake. He saw it stay flat. "Oh no!"' },
      { type: 'label',   text: 'Solution:' },
      { type: 'beat',    text: 'He did not wait. He took the flat cake out and made a stack: cake, jam, cake, jam!' },
      { type: 'end',     text: 'Jay ate it by the lake. "Give me two more!" That flat cake was great.' },
    ],
    talkAboutIt: ['How did Giri turn a flat cake into a great cake?'],
  },

  {
    id: 'core-b-02',
    band: 'B', level: 2, phase: 'long-o',
    title: 'Giri and the Speedy Boat',
    emoji: '\u26f5',
    mascotState: 'celebrate',
    illustration: 'giri_level02_story02_speedy-boat.png.jpg',
    targetGraphemes: ['oa', 'o_e'],
    allowedHFWTier: 2,
    targetWordCount: 65,
    actualWordCount: 76,
    decodableRatio: 0.96,
    textType: 'story-reader',
    refrainCount: 0,
    sightWords: getIntroducedSightWords('core-b-02'),
    lines: [
      { type: 'intro',   text: 'Giri had a small boat. He hoped to row home while the stream was slow.' },
      { type: 'label',   text: 'Problem:' },
      { type: 'beat',    text: 'But the boat did not float! It sat low by a big stone. "What is in here?"' },
      { type: 'label',   text: 'Attempt:' },
      { type: 'beat',    text: 'Giri gave it a shove. No go! Then he did find a toad. They had made it a home!' },
      { type: 'label',   text: 'Solution:' },
      { type: 'beat',    text: '"Take this home, toad!" Giri made it a moss home on the stone.' },
      { type: 'end',     text: 'The toad hopped off. The boat rose and rode the foam home!' },
    ],
    talkAboutIt: ['Why was the boat sitting so low in the water?'],
  },

  {
    id: 'core-b-03',
    band: 'B', level: 2, phase: 'long-e',
    title: 'Giri Helps the Bee',
    emoji: '\ud83d\udc1d',
    mascotState: 'pointRight',
    illustration: 'giri_level02_story03_helps-bee.png.jpg',
    targetGraphemes: ['ee', 'ea'],
    allowedHFWTier: 2,
    targetWordCount: 70,
    actualWordCount: 80,
    decodableRatio: 0.91,
    textType: 'story-reader',
    refrainCount: 0,
    sightWords: getIntroducedSightWords('core-b-03'),
    lines: [
      { type: 'intro',   text: 'Giri met a weak bee by a green tree. It was too tired to see its way home.' },
      { type: 'label',   text: 'Problem:' },
      { type: 'beat',    text: 'The bee could not reach its hive. "I need a good rest," it said with a weep.' },
      { type: 'label',   text: 'Attempt:' },
      { type: 'beat',    text: 'Giri had a plan. "Would a leaf help? A leaf could lift you up high!"' },
      { type: 'label',   text: 'Solution:' },
      { type: 'beat',    text: 'He set the bee on a green leaf. "This should reach the tree," said Giri.' },
      { type: 'end',     text: 'The bee zoomed up to see its home. A good deed, and a good day!' },
    ],
    talkAboutIt: ['How did the bee get up to its tree?'],
  },

  {
    id: 'core-b-04',
    band: 'B', level: 2, phase: 'long-i',
    title: "Giri's Big Day",
    emoji: '\ud83c\udfa1',
    mascotState: 'clap',
    illustration: 'giri_level02_story04_big-day.png.jpg',
    targetGraphemes: ['i_e', 'igh'],
    allowedHFWTier: 2,
    targetWordCount: 70,
    actualWordCount: 80,
    decodableRatio: 0.96,
    textType: 'story-reader',
    refrainCount: 0,
    sightWords: getIntroducedSightWords('core-b-04'),
    lines: [
      { type: 'intro',   text: 'Giri climbed a high hill with his bright kite. He would fly it across the wide sky.' },
      { type: 'label',   text: 'Problem:' },
      { type: 'beat',    text: 'But the sky was still. No wind! The kite fell to the side. "Come on, wind!" he sighed.' },
      { type: 'label',   text: 'Attempt:' },
      { type: 'beat',    text: 'Giri held the line tight. "I will run and make my own wind!" And he did.' },
      { type: 'label',   text: 'Solution:' },
      { type: 'beat',    text: 'His run made the wind rise! The kite went up high, across the bright sky.' },
      { type: 'end',     text: 'By the water, Giri sat with a smile. "Come and see my kite fly!"' },
    ],
    talkAboutIt: ['How did Giri make his own wind?'],
  },

  {
    id: 'core-b-05',
    band: 'B', level: 2, phase: 'long-a',
    title: 'Giri and the Snail',
    emoji: '\ud83d\udc0c',
    mascotState: 'pointLeft',
    illustration: 'giri_level02_story05_snail.jpg',
    targetGraphemes: ['ai', 'ay'],
    allowedHFWTier: 2,
    targetWordCount: 60,
    actualWordCount: 78,
    decodableRatio: 0.92,
    textType: 'story-reader',
    refrainCount: 0,
    sightWords: getIntroducedSightWords('core-b-05'),
    lines: [
      { type: 'intro',   text: 'Giri met a small snail on the trail one rainy day in May.' },
      { type: 'label',   text: 'Problem:' },
      { type: 'beat',    text: 'The snail sat right in the way of a drain. Cold rain ran down from the sky.' },
      { type: 'label',   text: 'Attempt:' },
      { type: 'beat',    text: '"Wait, snail!" Giri put out his hand and gave it a soft push, under a big leaf.' },
      { type: 'label',   text: 'Solution:' },
      { type: 'beat',    text: 'The snail slid to safety, out of the cold rain. Giri waited all the way.' },
      { type: 'end',     text: 'The snail waved its tail. "I love this snail," said Giri. "It may take all day!"' },
    ],
    talkAboutIt: ['Why did Giri have to wait so long?'],
  },

  {
    id: 'core-b-06',
    band: 'B', level: 2, phase: 'long-i',
    title: 'Giri Makes a Kite',
    emoji: '\ud83e\ude81',
    mascotState: 'whiteboard',
    illustration: 'giri_level02_story06_kite.jpg',
    targetGraphemes: ['i_e', 'igh'],
    allowedHFWTier: 2,
    targetWordCount: 70,
    actualWordCount: 80,
    decodableRatio: 0.96,
    textType: 'story-reader',
    refrainCount: 0,
    sightWords: getIntroducedSightWords('core-b-06'),
    lines: [
      { type: 'intro',   text: 'Giri had a fine plan to make a kite. He had made kites once before.' },
      { type: 'label',   text: 'Problem:' },
      { type: 'beat',    text: 'He tied the strings tight, but the kite fell on its side. It had no tail!' },
      { type: 'label',   text: 'Attempt:' },
      { type: 'beat',    text: 'Giri sighed. "This kite will not fly because it has no tail," he said.' },
      { type: 'label',   text: 'Solution:' },
      { type: 'beat',    text: 'He cut one white stripe, then the other, and tied both tight to the kite.' },
      { type: 'end',     text: 'Now the kite had a fine tail! Once upon a time it rose high, white as salt in the sky.' },
    ],
    talkAboutIt: ['What did the kite need to fly?'],
  },

  {
    id: 'core-b-07',
    band: 'B', level: 2, phase: 'long-i',
    title: "Giri's Bike Ride",
    emoji: '\ud83d\udeb2',
    mascotState: 'celebrate',
    illustration: 'giri_level02_story07_bike-ride.jpg',
    targetGraphemes: ['i_e'],
    allowedHFWTier: 2,
    targetWordCount: 65,
    actualWordCount: 74,
    decodableRatio: 0.92,
    textType: 'story-reader',
    refrainCount: 0,
    sightWords: getIntroducedSightWords('core-b-07'),
    lines: [
      { type: 'intro',   text: 'Giri went on a bike ride along a wide white lane in the sun.' },
      { type: 'label',   text: 'Problem:' },
      { type: 'beat',    text: 'The bike hit a stone! Giri slid off and bumped his side. "Who left this here?"' },
      { type: 'label',   text: 'Attempt:' },
      { type: 'beat',    text: 'A young pup came by. "Are you sad?" Giri wiped his face.' },
      { type: 'label',   text: 'Solution:' },
      { type: 'beat',    text: 'He rode on until he felt fine. Then he went back to give the lane a fix.' },
      { type: 'end',     text: 'Giri rolled the wide stone off the side. "Now the lane is safe to ride!"' },
    ],
    talkAboutIt: ['Why did Giri roll the stone off the lane?'],
  },

  {
    id: 'core-b-08',
    band: 'B', level: 2, phase: 'long-o',
    title: 'Giri and the Goat',
    emoji: '\ud83d\udc10',
    mascotState: 'encourage',
    illustration: 'giri_level02_story08_goat.jpg',
    targetGraphemes: ['oa', 'o_e'],
    allowedHFWTier: 2,
    targetWordCount: 65,
    actualWordCount: 75,
    decodableRatio: 0.93,
    textType: 'story-reader',
    refrainCount: 0,
    sightWords: getIntroducedSightWords('core-b-08'),
    lines: [
      { type: 'intro',   text: 'One day, Giri met a goat on a road. It had left its mother and brother.' },
      { type: 'label',   text: 'Problem:' },
      { type: 'beat',    text: "The goat took a bite of Giri's soft coat! \"That is not something to eat!\"" },
      { type: 'label',   text: 'Attempt:' },
      { type: 'beat',    text: '"Hold on, friend! Do not become a wolf!" Giri held out a big pile of oats.' },
      { type: 'label',   text: 'Solution:' },
      { type: 'beat',    text: 'The goat gave up the coat and chose to chomp on the tasty oats.' },
      { type: 'end',     text: 'Giri went home with his coat. The goat ran to its mother and brother.' },
    ],
    talkAboutIt: ['Why did the goat stop eating the coat?'],
  },

  {
    id: 'core-b-09',
    band: 'B', level: 2, phase: 'long-e',
    title: 'Giri at the Beach',
    emoji: '\ud83c\udfd6\ufe0f',
    mascotState: 'clap',
    illustration: 'giri_level02_story09_beach.jpg',
    targetGraphemes: ['ea', 'ee'],
    allowedHFWTier: 2,
    targetWordCount: 65,
    actualWordCount: 79,
    decodableRatio: 0.92,
    textType: 'story-reader',
    refrainCount: 0,
    sightWords: getIntroducedSightWords('core-b-09'),
    lines: [
      { type: 'intro',   text: 'Giri went to the beach to see the green sea. A girl and a boy came too.' },
      { type: 'label',   text: 'Problem:' },
      { type: 'beat',    text: 'A wave reached his feet! He leaped back with a squeak. "That was just too quick!"' },
      { type: 'label',   text: 'Attempt:' },
      { type: 'beat',    text: '"We must run from the wave!" The girl and the boy ran with Giri up the beach.' },
      { type: 'label',   text: 'Solution:' },
      { type: 'beat',    text: 'Then the wave went back. "It is tag with the sea!" They all leaped in and back.' },
      { type: 'end',     text: 'They had a sweet peach treat. What a day at the sea!' },
    ],
    talkAboutIt: ['What game did Giri play with the sea?'],
  },

  {
    id: 'core-b-10',
    band: 'B', level: 2, phase: 'long-a',
    title: 'Giri and the Rain',
    emoji: '\ud83c\udf27\ufe0f',
    mascotState: 'thinking',
    illustration: 'giri_level02_story10_rain.jpg',
    targetGraphemes: ['ai', 'ay'],
    allowedHFWTier: 2,
    targetWordCount: 65,
    actualWordCount: 80,
    decodableRatio: 0.93,
    textType: 'story-reader',
    refrainCount: 0,
    sightWords: getIntroducedSightWords('core-b-10'),
    lines: [
      { type: 'intro',   text: 'Giri was on a long gray trail one cool day in May. He thought it would stay dry.' },
      { type: 'label',   text: 'Problem:' },
      { type: 'beat',    text: 'A heavy rain came down fast! "Where can I hide?" Giri got soaked on the muddy trail.' },
      { type: 'label',   text: 'Attempt:' },
      { type: 'beat',    text: '"Wait it out!" He hid in a dry cave to watch the rain. Drip, drip, drip.' },
      { type: 'label',   text: 'Solution:' },
      { type: 'beat',    text: 'The rain stopped. Giri made big mud prints all the way home. Stamp, stamp!' },
      { type: 'end',     text: 'He looked down at his trail of prints. "Where the rain fell, I had fun!"' },
    ],
    talkAboutIt: ['What did Giri make with his feet on the muddy trail?'],
  },

  {
    id: 'core-b-11',
    band: 'B', level: 2, phase: 'long-e',
    title: "Giri's Green Seed",
    emoji: '\ud83c\udf31',
    mascotState: 'pointRight',
    illustration: 'giri_level02_story11_green-seed.jpg',
    targetGraphemes: ['ee', 'ea'],
    allowedHFWTier: 2,
    targetWordCount: 65,
    actualWordCount: 80,
    decodableRatio: 0.95,
    textType: 'story-reader',
    refrainCount: 0,
    sightWords: getIntroducedSightWords('core-b-11'),
    lines: [
      { type: 'intro',   text: 'Giri had a green seed. "Maybe it will grow into a tree," he said.' },
      { type: 'label',   text: 'Problem:' },
      { type: 'beat',    text: 'But the seed did not seem to grow. Giri gave it a nod of his head and waited.' },
      { type: 'label',   text: 'Attempt:' },
      { type: 'beat',    text: 'His friend came by. "A seed may need a deep drink each day." So Giri gave it a drink.' },
      { type: 'label',   text: 'Solution:' },
      { type: 'beat',    text: 'He let it feel the heat of the sun. At last, a green leaf reached up!' },
      { type: 'end',     text: 'The seed has a friend in Giri. Maybe it will be a tree!' },
    ],
    talkAboutIt: ['What did the seed need to grow?'],
  },

  {
    id: 'core-b-12',
    band: 'B', level: 2, phase: 'long-o',
    title: "Giri's Cosy Home",
    emoji: '\ud83c\udfe0',
    mascotState: 'neutral',
    illustration: 'giri_level02_story12_cosy-home.jpg',
    targetGraphemes: ['o_e', 'oa'],
    allowedHFWTier: 2,
    targetWordCount: 70,
    actualWordCount: 80,
    decodableRatio: 0.96,
    textType: 'story-reader',
    refrainCount: 0,
    sightWords: getIntroducedSightWords('core-b-12'),
    lines: [
      { type: 'intro',   text: 'Giri went to make a cosy home in a stone cave by an old oak tree.' },
      { type: 'label',   text: 'Problem:' },
      { type: 'beat',    text: 'A nosy mole dug a deep hole in the stone wall. It ate its way right in!' },
      { type: 'label',   text: 'Attempt:' },
      { type: 'beat',    text: 'Giri went to fill it. "A hole can be of use! Do not throw it away."' },
      { type: 'label',   text: 'Solution:' },
      { type: 'beat',    text: 'He made the hole a note slot and let a note go down to the mole.' },
      { type: 'end',     text: 'Now Giri and the mole have a way to chat. Notes go to and fro!' },
    ],
    talkAboutIt: ['What did Giri turn the hole into?'],
  },

  // ── Long-u set (3) ────────────────────────────────────────────────────────

  {
    id: 'core-b-13',
    band: 'B', level: 2, phase: 'long-u',
    title: 'Giri and the Flute',
    emoji: '🎶',
    mascotState: 'celebrate',
    illustration: 'giri_level02_story13_flute.jpg',
    targetGraphemes: ['u_e', 'ue', 'oo'],
    allowedHFWTier: 2,
    targetWordCount: 70,
    actualWordCount: 80,
    decodableRatio: 0.96,
    textType: 'story-reader',
    refrainCount: 0,
    sightWords: getIntroducedSightWords('core-b-13'),
    lines: [
      { type: 'intro',   text: 'Giri had a flute. He liked to play a tune in the cool shade.' },
      { type: 'label',   text: 'Problem:' },
      { type: 'beat',    text: 'One day the flute made no tune. It had been full of grit and dust.' },
      { type: 'label',   text: 'Attempt:' },
      { type: 'beat',    text: 'Giri used a cube of soap and a tub of suds. "This will make it new," he did say.' },
      { type: 'label',   text: 'Solution:' },
      { type: 'beat',    text: 'Soon the flute played the same sweet tune as before. The notes came out true.' },
      { type: 'end',     text: 'By the green tree, people sat to see him play. "Play the same tune!" they did say.' },
    ],
    talkAboutIt: ['Why did the flute stop making a tune?'],
  },

  {
    id: 'core-b-14',
    band: 'B', level: 2, phase: 'long-u',
    title: "Giri's New Kite",
    emoji: '🪁',
    mascotState: 'pointRight',
    illustration: 'giri_level02_story14_blue-kite.jpg',
    targetGraphemes: ['ew', 'ue'],
    allowedHFWTier: 2,
    targetWordCount: 70,
    actualWordCount: 77,
    decodableRatio: 0.92,
    textType: 'story-reader',
    refrainCount: 0,
    sightWords: getIntroducedSightWords('core-b-14'),
    lines: [
      { type: 'intro',   text: 'Giri drew a plan for a new kite with a deep blue pen.' },
      { type: 'label',   text: 'Problem:' },
      { type: 'beat',    text: 'The wind blew and blew. His old kite flew into a tree and got stuck. "Please help!"' },
      { type: 'label',   text: 'Attempt:' },
      { type: 'beat',    text: 'Giri made the new kite. He glued the frame and tied each stick tight.' },
      { type: 'label',   text: 'Solution:' },
      { type: 'beat',    text: '"Is your kite pretty?" asked his pal. "It is! And each bit is blue," said Giri, happy.' },
      { type: 'end',     text: 'The new kite flew up, up! "Please fly!" Giri felt so happy. "The best kite yet!"' },
    ],
    talkAboutIt: ['What did Giri do when his old kite got stuck?'],
  },

  {
    id: 'core-b-15',
    band: 'B', level: 2, phase: 'long-u',
    title: 'Giri and the Moon Pool',
    emoji: '🌙',
    mascotState: 'thinking',
    illustration: 'giri_level02_story15_moon-pool.jpg',
    targetGraphemes: ['oo', 'i_e'],
    allowedHFWTier: 2,
    targetWordCount: 75,
    actualWordCount: 78,
    decodableRatio: 0.95,
    textType: 'story-reader',
    refrainCount: 0,
    sightWords: getIntroducedSightWords('core-b-15'),
    lines: [
      { type: 'intro',   text: 'One night Giri went to the pool to see the moon shine like a plate.' },
      { type: 'label',   text: 'Problem:' },
      { type: 'beat',    text: 'A grey mist hid the moon. "Do not leave!" Giri said to the moon. The pool went dim.' },
      { type: 'label',   text: 'Attempt:' },
      { type: 'beat',    text: 'Giri sat to keep still. Two flies did zoom by. He ate a snack.' },
      { type: 'label',   text: 'Solution:' },
      { type: 'beat',    text: 'Soon the mist swept away. The moon came back, bright as the moon on the sea.' },
      { type: 'end',     text: 'This night, this moon, this pool — Giri will keep these in mind. Time to leave!' },
    ],
    talkAboutIt: ['What hid the moon for a little while?'],
  },

  // ── Short-vowel digraph review set (2) ────────────────────────────────────

  {
    id: 'core-b-16',
    band: 'B', level: 2, phase: 'short-digraphs',
    title: 'Giri and the Shell Shop',
    emoji: '🐚',
    mascotState: 'encourage',
    illustration: 'giri_level02_story16_shell-shop.jpg',
    targetGraphemes: ['sh', 'th', 'ck'],
    allowedHFWTier: 2,
    targetWordCount: 75,
    actualWordCount: 79,
    decodableRatio: 0.91,
    textType: 'story-reader',
    refrainCount: 0,
    sightWords: getIntroducedSightWords('core-b-16'),
    lines: [
      { type: 'intro',   text: 'Giri went to the shell shop. Shells sat in a big box on a shelf.' },
      { type: 'label',   text: 'Problem:' },
      { type: 'beat',    text: 'Bash! The box fell off the shelf. Shells slid across the shop.' },
      { type: 'label',   text: 'Attempt:' },
      { type: 'beat',    text: '"I can fix this myself!" It did not take much time. He got a brush and a bin.' },
      { type: 'label',   text: 'Solution:' },
      { type: 'beat',    text: 'He put the shells back in the box. A moth did fly off with a flick!' },
      { type: 'end',     text: 'The man was so glad. This shop was his life! He held up a shell to the light.' },
    ],
    talkAboutIt: ['How did Giri help at the shell shop?'],
  },

  {
    id: 'core-b-17',
    band: 'B', level: 2, phase: 'short-digraphs',
    title: 'Giri Sings a Long Song',
    emoji: '🎤',
    mascotState: 'clap',
    illustration: 'giri_level02_story17_song.jpg',
    targetGraphemes: ['ng', 'qu', 'ck'],
    allowedHFWTier: 2,
    targetWordCount: 75,
    actualWordCount: 78,
    decodableRatio: 0.92,
    textType: 'story-reader',
    refrainCount: 0,
    sightWords: getIntroducedSightWords('core-b-17'),
    lines: [
      { type: 'intro',   text: 'Giri had a song to sing in class. He sang it with a quick clap and a click.' },
      { type: 'label',   text: 'Problem:' },
      { type: 'beat',    text: 'At the test run, Giri got stuck. The song went wrong. Nothing at all!' },
      { type: 'label',   text: 'Attempt:' },
      { type: 'beat',    text: 'He did not quit. "Why did I stop?" He shut an eye and sang from the top, big and strong.' },
      { type: 'label',   text: 'Solution:' },
      { type: 'beat',    text: 'He sang high and long. The song was fine! Clap, click, sing along!' },
      { type: 'end',     text: 'Then the song was a hit! His pals sang along, strong and long.' },
    ],
    talkAboutIt: ['What did Giri do when the song went wrong?'],
  },

  // ── Sight-word feature readers (dialogue minis with quest-word focus) ──

  {
    id: 'core-b-sw-01',
    band: 'B', level: 2, phase: 'long-o',
    title: 'No. No. Also No.',
    emoji: '\ud83e\udd8e',
    mascotState: 'encourage',
    illustration: 'giri_level02_story23_mole-no.jpg',
    targetGraphemes: ['o_e', 'oa'],
    allowedHFWTier: 2,
    targetWordCount: 70,
    actualWordCount: 79,
    decodableRatio: 0.92,
    textType: 'story-reader',
    refrainCount: 0,
    lines: [
      { type: 'intro',   text: 'Mole did not want to go out. "No. It is cold. I will stay home."' },
      { type: 'label',   text: 'Problem:' },
      { type: 'beat',    text: '"Come and play," said Giri. "We can jog to the lake." "No. Also no. Still no."' },
      { type: 'label',   text: 'Attempt:' },
      { type: 'beat',    text: '"Does Mole want a hot bun? I have two buns from home." A pink nose came out.' },
      { type: 'label',   text: 'Solution:' },
      { type: 'beat',    text: '"So... one bun is for me?" "Yes!" Out came Mole. No coat. No hat.' },
      { type: 'end',     text: '"See? It is fun out here." "The bun is why I came. But yes — also the fun."' },
    ],
    talkAboutIt: ['What made Mole come out of his hole in the end?'],
  },

  {
    id: 'core-b-sw-02',
    band: 'B', level: 2, phase: 'long-e',
    title: 'The Bee and Me',
    emoji: '\ud83d\udc1d',
    mascotState: 'celebrate',
    illustration: 'giri_level02_story24_bee-and-me.jpg',
    targetGraphemes: ['ee', 'ea'],
    allowedHFWTier: 2,
    targetWordCount: 72,
    actualWordCount: 80,
    decodableRatio: 0.94,
    textType: 'story-reader',
    refrainCount: 0,
    lines: [
      { type: 'intro',   text: '"I see a bee!" said Giri. "And the bee sees me!"' },
      { type: 'label',   text: 'Problem:' },
      { type: 'beat',    text: 'The bee went from tree to tree. "Wait for me! I want to see what you can see."' },
      { type: 'label',   text: 'Attempt:' },
      { type: 'beat',    text: '"Then keep up! We need to find the sweet spot." Giri did not see it. "Here? Is it here?"' },
      { type: 'label',   text: 'Solution:' },
      { type: 'beat',    text: 'The bee sat still on a green stem. "Take a deep sniff." Sweet! A bed of mint!' },
      { type: 'end',     text: '"Good find! You and me — we make a fine team." "Yes," said the bee. "Buzz!"' },
    ],
    talkAboutIt: ['How did the bee help Giri find the sweet spot?'],
  },

  // ╔══════════════════════════════════════════════════════════════════════════╗
  // ║  BAND C — Fluency Readers  (8 books)                                  ║
  // ║  80–140 words · paragraph format · Tier 1–3 HFW · refrain 0–1×       ║
  // ║  Includes comprehension, vocab, grammarSpotlight                       ║
  // ╚══════════════════════════════════════════════════════════════════════════╝

  {
    id: 'core-c-01',
    band: 'C', level: 3, phase: 'r-controlled',
    title: 'Giri and the Big Storm',
    emoji: '⛈️',
    mascotState: 'thinking',
    illustration: 'giri_level03_story01_big-storm.png.jpg',
    targetGraphemes: ['ar', 'or'],
    allowedHFWTier: 3,
    targetWordCount: 100,
    actualWordCount: 114,
    decodableRatio: 0.99,
    textType: 'fluency-reader',
    refrainCount: 0,
    sightWords: getIntroducedSightWords('core-c-01'),
    lines: [
      { type: 'paragraph', text: 'Giri was at the farm when dark skies came over the barn. The storm burst with a crash. The barn door banged hard in the wind. His own star chart, which he wrote out by hand, blew off the wall and landed in a puddle by the yard.' },
      { type: 'paragraph', text: 'Giri ran to the barn. The door was stuck. He pushed and pulled with all his might. At last, the door swung open. Giri grabbed the star chart and ran back in. The storm passed and the sun lit the yard once more.' },
      { type: 'end',       text: 'Giri hung the chart to dry. He was not going to buy a new one. He felt glad he had not given up.' },
    ],
    comprehension: [
      { q: 'Where was Giri when the storm came?', options: ['At the farm', 'At the park', 'At school', 'At the market'], answer: 0, type: 'literal' },
      { q: 'What blew off the wall?', options: ['His star chart', 'His hat', 'A picture', 'A clock'], answer: 0, type: 'literal' },
      { q: 'Why did Giri feel proud at the end?', options: ['He did not give up', 'He won a prize', 'He ran the fastest', 'The storm stopped'], answer: 0, type: 'inferential' },
    ],
    vocab: [
      { word: 'storm', meaning: 'Very bad weather with strong wind and rain', icon: '⛈️' },
      { word: 'barn', meaning: 'A big building on a farm for animals or crops', icon: '🏚️' },
      { word: 'chart', meaning: 'A sheet with pictures or information on it', icon: '📊' },
      { word: 'might', meaning: 'Great strength or power', icon: '💪' },
      { word: 'proud', meaning: 'Feeling good about something you have done', icon: '😊' },
    ],
    grammarSpotlight: [
      { pattern: 'r-controlled vowels (ar, or)', example: '"farm", "dark", "barn", "hard", "yard", "storm", "door", "more"', tip: 'When r comes after a vowel, the vowel sound changes: a+r = /ar/, o+r = /or/.' },
      { pattern: 'Past-tense verbs', example: 'The door banged. He pushed and pulled. The storm passed.', tip: 'Add -ed to most verbs to show past tense: bang → banged, push → pushed.' },
    ],
  },

  {
    id: 'core-c-02',
    band: 'C', level: 3, phase: 'r-controlled',
    title: 'Giri Finds a Star',
    emoji: '⭐',
    mascotState: 'celebrate',
    illustration: 'giri_level03_story02_finds-star.png.jpg',
    targetGraphemes: ['ar', 'ir', 'er'],
    allowedHFWTier: 3,
    targetWordCount: 105,
    actualWordCount: 98,
    decodableRatio: 0.96,
    textType: 'fluency-reader',
    refrainCount: 0,
    sightWords: getIntroducedSightWords('core-c-02'),
    lines: [
      { type: 'paragraph', text: 'Not long ago, Giri walked far from the farm one night. The sky was very dark and clear. A large bright star fell from above and landed near the river, though he was the only one to see it. Giri hurried over to find it.' },
      { type: 'paragraph', text: 'The star was stuck under a fern by the water. Giri bent down and picked it up. It was warm and gave off a soft glow. A bird chirped from a branch as if to say well done.' },
      { type: 'end',       text: 'Giri held the star close and walked home with a warm glow in his heart.' },
    ],
    pretaught: ['clear', 'near', 'heart'],
    comprehension: [
      { q: 'Where did the star land?', options: ['Near the river', 'On the farm', 'In the barn', 'On a tree'], answer: 0, type: 'literal' },
      { q: 'What was the star stuck under?', options: ['A fern', 'A rock', 'A log', 'Some mud'], answer: 0, type: 'literal' },
      { q: 'How do you think Giri felt when he found the star?', options: ['Happy and excited', 'Sad and cold', 'Angry and tired', 'Bored and sleepy'], answer: 0, type: 'inferential' },
    ],
    vocab: [
      { word: 'fern', meaning: 'A green plant with feathery leaves', icon: '🌿' },
      { word: 'chirped', meaning: 'Made a short high sound like a small bird', icon: '🐦' },
      { word: 'glow', meaning: 'A soft, steady light', icon: '✨' },
      { word: 'hurried', meaning: 'Moved quickly because something was important', icon: '🏃' },
      { word: 'branch', meaning: 'An arm of a tree that grows out from the trunk', icon: '🌳' },
    ],
    grammarSpotlight: [
      { pattern: 'r-controlled vowels (ir, er)', example: '"bird", "chirped", "fern", "river", "under", "over"', tip: 'The letters ir and er often make the same /er/ sound: bird, fern, river.' },
      { pattern: 'Simile with "as if"', example: 'The bird chirped as if to say well done.', tip: '"As if" compares something to something else to help us picture it.' },
    ],
  },

  {
    id: 'core-c-03',
    band: 'C', level: 3, phase: 'digraphs',
    title: 'Giri and the Clever Bird',
    emoji: '🐦',
    mascotState: 'pointLeft',
    illustration: 'giri_level03_story03_clever-bird.png.jpg',
    targetGraphemes: ['sh', 'ch', 'th'],
    allowedHFWTier: 3,
    targetWordCount: 110,
    actualWordCount: 112,
    decodableRatio: 0.98,
    textType: 'fluency-reader',
    refrainCount: 0,
    sightWords: getIntroducedSightWords('core-c-03'),
    lines: [
      { type: 'paragraph', text: 'A thrush sat on a branch and watched Giri from above. Giri had lost his shell necklace on the path. He searched in the thick bushes and checked behind each rock, but did not find any of it.' },
      { type: 'paragraph', text: 'The thrush gave a sharp chirp. It hopped to a patch of grass and pecked at something shiny. Giri rushed over. There was his shell necklace, half hidden under a thin leaf. Giri picked it up. "I don\'t know how you knew!" he thanked the clever bird.' },
      { type: 'end',       text: 'Giri clipped the shell necklace on. Both he and the thrush felt glad. The thrush goes where it likes, and Giri set off home into the sunshine.' },
    ],
    comprehension: [
      { q: 'What had Giri lost?', options: ['His shell necklace', 'His star chart', 'His hat', 'His bag'], answer: 0, type: 'literal' },
      { q: 'How did the thrush help?', options: ['It pecked at the necklace to show Giri', 'It sang a song', 'It flew away', 'It brought food'], answer: 0, type: 'literal' },
      { q: 'What does this story teach us?', options: ['Help can come from unexpected places', 'Birds are always helpful', 'Necklaces are easy to find', 'Bushes are the best hiding spots'], answer: 0, type: 'inferential' },
    ],
    vocab: [
      { word: 'thrush', meaning: 'A type of songbird with a speckled chest', icon: '🐦' },
      { word: 'searched', meaning: 'Looked carefully to find something', icon: '🔍' },
      { word: 'patch', meaning: 'A small area of ground', icon: '🟩' },
      { word: 'sharp', meaning: 'Clear and sudden', icon: '❗' },
      { word: 'hidden', meaning: 'Put out of sight; not easy to see', icon: '👀' },
    ],
    grammarSpotlight: [
      { pattern: 'Digraphs (sh, ch, th)', example: '"shell", "sharp", "bushes", "checked", "chirp", "thrush", "thick", "thin", "thanked"', tip: 'Two letters that make one sound: sh = /sh/, ch = /ch/, th = /th/.' },
      { pattern: 'Varied sentence openings', example: 'The thrush gave a chirp. It hopped to a patch. Giri rushed over.', tip: 'Start sentences with different words to make your writing more interesting.' },
    ],
  },

  {
    id: 'core-c-04',
    band: 'C', level: 3, phase: 'r-controlled',
    title: 'Giri at the Market',
    emoji: '🏪',
    mascotState: 'whiteboard',
    illustration: 'giri_level03_story04_market.png.jpg',
    targetGraphemes: ['ar', 'er', 'or'],
    allowedHFWTier: 3,
    targetWordCount: 110,
    actualWordCount: 118,
    decodableRatio: 0.96,
    textType: 'fluency-reader',
    refrainCount: 0,
    sightWords: getIntroducedSightWords('core-c-04'),
    lines: [
      { type: 'paragraph', text: 'Giri walked to the market to get a jar of honey for his mother. The stalls were bright with herbs, berries, and fresh corn. A trader called out, "Best herbs here!" The market began to get busy, but Giri could not see the honey stall.' },
      { type: 'paragraph', text: 'He turned a corner and there it was, tucked between a herb cart and a butter stall. Giri picked out a large jar and paid the farmer with his change. On the way home he spotted a perfect red pepper, good to eat, and got that as well.' },
      { type: 'end',       text: 'Giri gave the jar and the pepper to his mother. Her frown became a big smile. "You begin to shop like a pro!" she said.' },
    ],
    comprehension: [
      { q: 'What did Giri go to the market to get?', options: ['A jar of honey', 'A bag of corn', 'Some flowers', 'A red hat'], answer: 0, type: 'literal' },
      { q: 'Where was the honey stall?', options: ['Between a flower cart and a butter stall', 'By the door', 'At the far end', 'Next to the herbs'], answer: 0, type: 'literal' },
      { q: 'Why did Giri also buy a pepper?', options: ['He thought his mother would like it', 'It was free', 'The trader asked him to', 'He was very hungry'], answer: 0, type: 'inferential' },
    ],
    vocab: [
      { word: 'trader', meaning: 'A person who buys and sells things', icon: '🧑‍💼' },
      { word: 'herbs', meaning: 'Plants used to add taste to food', icon: '🌿' },
      { word: 'jar', meaning: 'A round glass or clay container with a lid', icon: '🫙' },
      { word: 'spotted', meaning: 'Noticed or saw something', icon: '👁️' },
      { word: 'perfect', meaning: 'Without any faults; exactly right', icon: '✨' },
    ],
    grammarSpotlight: [
      { pattern: 'r-controlled vowels (er, ar, or)', example: '"her", "corner", "farmer", "market", "jar", "corn", "flowers"', tip: 'The er, ar, and or sounds appear in many everyday words.' },
      { pattern: 'Speech marks for direct speech', example: '"Best herbs here!" the trader called.', tip: 'Use speech marks to show the exact words someone says out loud.' },
    ],
  },

  {
    id: 'core-c-05',
    band: 'C', level: 3, phase: 'digraphs',
    title: 'Giri and the Blue Bin',
    emoji: '♻️',
    mascotState: 'clap',
    illustration: 'giri_level03_story05_blue-bin.jpg',
    targetGraphemes: ['sh', 'ch', 'th', 'wh'],
    allowedHFWTier: 3,
    targetWordCount: 115,
    actualWordCount: 125,
    decodableRatio: 0.98,
    textType: 'fluency-reader',
    refrainCount: 0,
    sightWords: getIntroducedSightWords('core-c-05'),
    lines: [
      { type: 'paragraph', text: 'Giri walked home from school and stopped by the blue bin. The bin was full of tins and sheets of paper. Giri had a juice box in his bag. He knew it should be saved, but was not really sure whether it should go in the blue bin.' },
      { type: 'paragraph', text: 'He checked the chart on the bin. "Juice boxes — yes!" it said. Giri tried to crush the box flat to make it fit. The box sprang back and bumped his chin. After a few more tries, with both hands, the box went flat with a crunch.' },
      { type: 'end',       text: 'Giri pushed the flat box in through the slot. The bin made a clang, like a note of music! Giri walked home and felt glad he had done the right thing.' },
    ],
    comprehension: [
      { q: 'What was Giri trying to put in the bin?', options: ['A juice box', 'A tin can', 'A bag', 'A book'], answer: 0, type: 'literal' },
      { q: 'How did Giri find out the box could go in?', options: ['He read the chart on the bin', 'His mum told him', 'A friend said so', 'He guessed'], answer: 0, type: 'literal' },
      { q: 'What is the main message of this story?', options: ['We should recycle the right things', 'Bins are too small', 'Juice boxes are rubbish', 'Walking home is fun'], answer: 0, type: 'inferential' },
    ],
    vocab: [
      { word: 'recycle', meaning: 'To use something again instead of throwing it away', icon: '♻️' },
      { word: 'crush', meaning: 'To press something hard until it is flat', icon: '👊' },
      { word: 'chart', meaning: 'A sign or poster that gives information', icon: '📋' },
      { word: 'whether', meaning: 'If (used when there are two choices)', icon: '🤔' },
      { word: 'crunch', meaning: 'A crushing sound', icon: '🔊' },
    ],
    grammarSpotlight: [
      { pattern: 'Digraphs (ch, sh, th, wh)', example: '"checked", "chart", "chin", "sheets", "pushed", "this", "whether"', tip: 'Two letters, one sound: ch, sh, th, wh are all digraphs.' },
      { pattern: 'Compound sentences with "and"', example: 'He tried once more, and the box went flat with a crunch.', tip: 'Use "and" to join two ideas that go together.' },
    ],
  },

  {
    id: 'core-c-06',
    band: 'C', level: 3, phase: 'r-controlled',
    title: 'Giri Helps a Neighbour',
    emoji: '💡',
    mascotState: 'encourage',
    illustration: 'giri_level03_story06_neighbour.jpg',
    targetGraphemes: ['er', 'ir', 'ur'],
    allowedHFWTier: 3,
    targetWordCount: 115,
    actualWordCount: 123,
    decodableRatio: 0.94,
    textType: 'fluency-reader',
    refrainCount: 0,
    sightWords: getIntroducedSightWords('core-c-06'),
    lines: [
      { type: 'paragraph', text: 'Giri heard a knock at the door. It was Mrs Tan from the flat next door. Her light bulb had burnt out and her step stool was broken. She could not reach the top of the shelf, which was very high.' },
      { type: 'paragraph', text: 'Giri offered to help. First he stood on his tiptoes, but he was not quite tall. Then he hurried to his flat and brought back a firm step stool. He set it in place inside her door, climbed up, turned the old bulb out, and put a new one in. He did not move until the light burst on!' },
      { type: 'end',       text: 'Mrs Tan clapped and gave Giri a warm kueh to say thank you. Giri felt glad he had helped a neighbour in need.' },
    ],
    pretaught: ['heard'],
    comprehension: [
      { q: 'Who knocked at the door?', options: ['Mrs Tan', 'A park ranger', 'His mum', 'A postman'], answer: 0, type: 'literal' },
      { q: 'What did Mrs Tan need help with?', options: ['Changing a light bulb', 'Finding her keys', 'Cooking food', 'Carrying bags'], answer: 0, type: 'literal' },
      { q: 'What can we learn from Giri in this story?', options: ['Helping neighbours is kind and clever', 'Step stools always break', 'Shelves are too high', 'Do not answer the door'], answer: 0, type: 'inferential' },
    ],
    vocab: [
      { word: 'neighbour', meaning: 'Someone who lives close to you', icon: '🏠' },
      { word: 'offered', meaning: 'Said you would do something to help', icon: '🤝' },
      { word: 'returned', meaning: 'Came back to a place', icon: '↩️' },
      { word: 'firm', meaning: 'Strong and solid; not wobbly', icon: '💪' },
      { word: 'burst', meaning: 'Happened suddenly and with energy', icon: '💥' },
    ],
    grammarSpotlight: [
      { pattern: 'r-controlled vowels (er, ir, ur)', example: '"her", "turned", "returned", "first", "firm", "burnt", "burst"', tip: 'The sounds er, ir, and ur often sound the same: her, bird, turn.' },
      { pattern: 'Sequence words', example: 'First he stood. Then he hurried.', tip: 'Words like first, then, next, and at last show the order of events.' },
    ],
  },

  {
    id: 'core-c-07',
    band: 'C', level: 3, phase: 'digraphs',
    title: 'Giri at the Farm',
    emoji: '🐔',
    mascotState: 'pointRight',
    illustration: 'giri_level03_story07_farm.jpg',
    targetGraphemes: ['sh', 'ch', 'ar'],
    allowedHFWTier: 3,
    targetWordCount: 110,
    actualWordCount: 115,
    decodableRatio: 0.97,
    textType: 'fluency-reader',
    refrainCount: 0,
    sightWords: getIntroducedSightWords('core-c-07'),
    lines: [
      { type: 'paragraph', text: 'Giri spent the morning at the farm with his grandfather. The chickens scratched in the yard and the sheep sheltered under a large shed. Giri always helped to fetch fresh water from the well and fill the bucket for the horse. It was hard work, but he liked it.' },
      { type: 'paragraph', text: 'After lunch, a chicken got out at a gap in the fence. Giri set off after it, past the barn and behind the shed. At last, he got it by the cherry tree, just as his aunty waved from the shed. He carefully shut the gap with a thick branch.' },
      { type: 'end',       text: 'Grandfather smiled. "Sharp thinking, Giri," he said. Giri felt glad to be a real farm helper.' },
    ],
    comprehension: [
      { q: 'What did Giri help with first?', options: ['Fetching water from the well', 'Catching a chicken', 'Feeding the sheep', 'Picking cherries'], answer: 0, type: 'literal' },
      { q: 'How did the chicken get out?', options: ['Through a gap in the wire', 'Over the fence', 'Under the barn', 'Through the door'], answer: 0, type: 'literal' },
      { q: 'Why did Grandfather say "sharp thinking"?', options: ['Giri solved the problem by himself', 'Giri ran the fastest', 'The chicken was clever', 'The branch was sharp'], answer: 0, type: 'inferential' },
    ],
    vocab: [
      { word: 'scratched', meaning: 'Used claws to dig at the ground', icon: '🐔' },
      { word: 'sheltered', meaning: 'Stayed in a safe place out of bad weather', icon: '🏠' },
      { word: 'trough', meaning: 'A long open container for animal food or water', icon: '🪣' },
      { word: 'chased', meaning: 'Ran after something to catch it', icon: '🏃' },
      { word: 'carefully', meaning: 'In a way that avoids mistakes or danger', icon: '🧐' },
    ],
    grammarSpotlight: [
      { pattern: 'Digraphs (sh, ch)', example: '"sheep", "shed", "sheltered", "shut", "chickens", "chased", "cherry", "branch"', tip: 'sh says /sh/ as in sheep. ch says /ch/ as in chicken.' },
      { pattern: 'Time connectives', example: 'After lunch, a chicken got out. At last, he caught it.', tip: 'Words like after, at last, and then show when things happened.' },
    ],
  },

  {
    id: 'core-c-08',
    band: 'C', level: 3, phase: 'r-controlled',
    title: "Giri's First Errand",
    emoji: '🛒',
    mascotState: 'holdCard',
    illustration: 'giri_level03_story08_first-errand.jpg',
    targetGraphemes: ['ir', 'er', 'ur'],
    allowedHFWTier: 3,
    targetWordCount: 115,
    actualWordCount: 130,
    decodableRatio: 0.97,
    textType: 'fluency-reader',
    refrainCount: 0,
    sightWords: getIntroducedSightWords('core-c-08'),
    lines: [
      { type: 'paragraph', text: "Giri's mother asked him to go to the corner shop to get butter and yoghurt. It was his first errand on his own, and he wanted to help his family today. Nerves ran through his body as he walked, turning left at the big curb and then right at the fir tree." },
      { type: 'paragraph', text: 'At the shop, Giri picked up the butter, but he could not find the yoghurt. A helper in a purple shirt led him to the third shelf, behind a stack of tins. Giri held his bag open and put everything in. On the way home, a bird chirped above him as if to cheer him on.' },
      { type: 'end',       text: 'Giri handed the bag to his mother. "Perfect!" she said. Giri grinned from ear to ear. His first errand was a success.' },
    ],
    pretaught: ['ear'],
    comprehension: [
      { q: 'What did Giri need to buy?', options: ['Butter and yoghurt', 'Bread and milk', 'Eggs and jam', 'Honey and tea'], answer: 0, type: 'literal' },
      { q: 'Who helped Giri find the yoghurt?', options: ['A helper in a purple shirt', 'His mother', 'A friend', 'The shop owner'], answer: 0, type: 'literal' },
      { q: 'Why was this errand special for Giri?', options: ['It was his first time going on his own', 'He got free sweets', 'He met a new friend', 'The shop was far away'], answer: 0, type: 'inferential' },
    ],
    vocab: [
      { word: 'errand', meaning: 'A short trip to do a task, like buying something', icon: '🏃' },
      { word: 'nerves', meaning: 'A worried or anxious feeling', icon: '😬' },
      { word: 'curb', meaning: 'The edge of a pavement where it meets the road', icon: '🛤️' },
      { word: 'stirred', meaning: 'Moved or felt a feeling inside', icon: '🌊' },
      { word: 'success', meaning: 'Doing something well; getting a good result', icon: '🌟' },
    ],
    grammarSpotlight: [
      { pattern: 'r-controlled vowels (ir, er, ur)', example: '"first", "shirt", "bird", "chirped", "helper", "corner", "butter", "curb", "purple", "turn"', tip: 'ir, er, and ur can all make the same /er/ sound.' },
      { pattern: 'Adjectives for feelings', example: 'Giri stirred with nerves. Giri grinned from ear to ear.', tip: 'We can describe how someone feels using action words and phrases.' },
    ],
  },

  {
    id: 'core-c-09',
    band: 'C', level: 3, phase: 'r-controlled',
    title: 'Giri and the Baby Turtle',
    emoji: '🐢',
    mascotState: 'encourage',
    illustration: 'giri_level03_story09_baby-turtle.jpg',
    targetGraphemes: ['ur', 'ir', 'er'],
    allowedHFWTier: 3,
    targetWordCount: 115,
    actualWordCount: 121,
    decodableRatio: 1.00,
    textType: 'fluency-reader',
    refrainCount: 0,
    sightWords: getIntroducedSightWords('core-c-09'),
    lines: [
      { type: 'paragraph', text: 'Giri was at the beach one morning when he spotted a baby turtle stuck in a dip in the sand. The turtle had turned the wrong way. It was far from the water, and the hot sun had made the sand burn below its flippers.' },
      { type: 'paragraph', text: 'Giri worked fast. First, he dug the sand away from the shell. He did not try to carry it; instead he turned the little turtle to face the water. The turtle began to stir. It pushed with its flippers, slid along the wet sand, and swam off into the surf.' },
      { type: 'end',       text: 'Giri stood and watched until the turtle was finally just a dot in the sea. His first rescue! He walked home with a warm glow inside.' },
    ],
    comprehension: [
      { q: 'Where was the baby turtle stuck?', options: ['In a dip in the sand', 'In the surf', 'Under a rock', 'In a net'], answer: 0, type: 'literal' },
      { q: 'What did Giri do first?', options: ['He dug the sand away from the shell', 'He picked the turtle up', 'He called for help', 'He splashed water on it'], answer: 0, type: 'literal' },
      { q: 'Why did the turtle need help quickly?', options: ['The hot sun was burning the sand', 'It was getting dark', 'The tide was coming in', 'A bird was near'], answer: 0, type: 'inferential' },
    ],
    vocab: [
      { word: 'flippers', meaning: 'The flat arms a turtle uses to swim', icon: '🐢' },
      { word: 'surf', meaning: 'The white, foamy waves near the shore', icon: '🌊' },
      { word: 'stir', meaning: 'To start to move a little', icon: '🌀' },
      { word: 'rescue', meaning: 'Saving someone from danger', icon: '🦺' },
      { word: 'glow', meaning: 'A warm, happy feeling or soft light', icon: '✨' },
    ],
    grammarSpotlight: [
      { pattern: 'r-controlled vowels (ur, ir, er)', example: '"turtle", "turned", "burn", "surf", "first", "stir", "under", "flippers"', tip: 'When "r" follows a vowel, they team up to make a new sound: ur, ir, and er all say /er/.' },
      { pattern: 'Time-order words', example: 'First, he dug the sand away. Then he turned the turtle.', tip: 'Words like "first" and "then" show the order things happen in.' },
    ],
  },

  {
    id: 'core-c-10',
    band: 'C', level: 3, phase: 'r-controlled',
    title: "Giri's Garden Surprise",
    emoji: '🥬',
    mascotState: 'celebrate',
    illustration: 'giri_level03_story10_turnips.jpg',
    targetGraphemes: ['ir', 'ur', 'er'],
    allowedHFWTier: 3,
    targetWordCount: 110,
    actualWordCount: 121,
    decodableRatio: 0.99,
    textType: 'fluency-reader',
    refrainCount: 0,
    sightWords: getIntroducedSightWords('core-c-10'),
    lines: [
      { type: 'paragraph', text: 'Giri had a little garden plot by the void deck. He planted a row of seeds between two stones and stirred the dirt with a small fork. It was not easy, but he often gave the seeds a drink of water and waited.' },
      { type: 'paragraph', text: 'Weeks went past, but nothing came up. Giri felt glum. Then, after a big rain, he spotted something green. A funny little turnip top! Soon ten green tops stood in a perfect line. Giri turned the dirt and let them grow bigger and bigger.' },
      { type: 'end',       text: 'At last, Giri pulled up ten firm turnips. He gave three to Mrs Tan, and when his work was done, his mum cooked the rest for dinner. His first harvest was the best surprise.' },
    ],
    comprehension: [
      { q: 'Where was Giri\'s garden plot?', options: ['By the void deck', 'On a farm', 'At school', 'By the beach'], answer: 0, type: 'literal' },
      { q: 'What came up after the big rain?', options: ['Tiny turnip tops', 'A row of corn', 'Red flowers', 'Long beans'], answer: 0, type: 'literal' },
      { q: 'How do we know Giri did not give up?', options: ['He watered the seeds every morning and kept waiting', 'He bought new seeds', 'He asked Mrs Tan to help', 'He dug up the plot'], answer: 0, type: 'inferential' },
    ],
    vocab: [
      { word: 'void deck', meaning: 'The open space under an HDB block', icon: '🏢' },
      { word: 'glum', meaning: 'Quietly sad and low', icon: '😞' },
      { word: 'turnip', meaning: 'A round white root you can eat', icon: '🥬' },
      { word: 'firm', meaning: 'Solid and not soft', icon: '💪' },
      { word: 'harvest', meaning: 'The crops you pick when they are ready', icon: '🧺' },
    ],
    grammarSpotlight: [
      { pattern: 'r-controlled vowels (ir, ur, er)', example: '"stirred", "dirt", "firm", "first", "turnip", "turned", "perfect", "dinner"', tip: 'ir, ur, and er all make the same /er/ sound in these garden words.' },
      { pattern: 'Comparing with -er', example: 'The tops grew bigger and bigger.', tip: 'Adding -er to a describing word compares things: big, bigger.' },
    ],
  },

  {
    id: 'core-c-11',
    band: 'C', level: 3, phase: 'suffixes',
    title: "Giri's Helping Day",
    emoji: '🧺',
    mascotState: 'clap',
    illustration: 'giri_level03_story11_helping.jpg',
    targetGraphemes: ['ed', 'ing'],
    allowedHFWTier: 3,
    targetWordCount: 105,
    actualWordCount: 105,
    decodableRatio: 0.97,
    textType: 'fluency-reader',
    refrainCount: 0,
    sightWords: getIntroducedSightWords('core-c-11'),
    lines: [
      { type: 'paragraph', text: 'On Sunday, Giri helped all day long. In the morning he started with the dishes, washing and stacking them in a shining row. Then he dusted the shelf and folded the clean clothes.' },
      { type: 'paragraph', text: 'After lunch, Giri kept helping. He watered all eight plants and picked up the blocks. Mrs Tan needed a hand with her shopping bags, so Giri carried the lightest one up the steps. He could feel his feet getting tired, but he kept smiling.' },
      { type: 'end',       text: 'At dinner, Mum smiled and said, "You helped all day. You must be the kindest helper in the city!" Helping, Giri decided, was a great kind of fun.' },
    ],
    comprehension: [
      { q: 'What did Giri start with in the morning?', options: ['The dishes', 'The shopping', 'The plants', 'The blocks'], answer: 0, type: 'literal' },
      { q: 'How did Giri help Mrs Tan?', options: ['He carried a shopping bag up the steps', 'He watered her plants', 'He folded her clothes', 'He washed her dishes'], answer: 0, type: 'literal' },
      { q: 'How did Giri feel about helping, even when he was tired?', options: ['He still liked it and kept smiling', 'He wanted to stop', 'He felt cross', 'He felt bored'], answer: 0, type: 'inferential' },
    ],
    vocab: [
      { word: 'stacking', meaning: 'Putting things neatly on top of each other', icon: '🥞' },
      { word: 'folded', meaning: 'Bent neatly so it takes less space', icon: '👕' },
      { word: 'carried', meaning: 'Held something and took it somewhere', icon: '🛍️' },
      { word: 'lightest', meaning: 'The one that is least heavy', icon: '🪶' },
      { word: 'decided', meaning: 'Made up your mind about something', icon: '💡' },
    ],
    grammarSpotlight: [
      { pattern: 'Past tense -ed', example: '"helped", "started", "dusted", "folded", "watered", "carried", "decided"', tip: 'Adding -ed to an action word shows it already happened.' },
      { pattern: 'Present action -ing', example: '"washing", "stacking", "helping", "getting", "smiling"', tip: 'Adding -ing shows an action that is happening right now.' },
    ],
  },


  {
    id: 'core-c-sw-01',
    band: 'C', level: 3, phase: 'r-controlled',
    title: 'Could You? Would You? Should You?',
    emoji: '\ud83d\udc26',
    mascotState: 'thinking',
    illustration: 'giri_level03_story13_could-would-should.jpg',
    targetGraphemes: ['ar', 'or', 'ir'],
    allowedHFWTier: 3,
    targetWordCount: 115,
    actualWordCount: 133,
    decodableRatio: 0.93,
    textType: 'fluency-reader',
    refrainCount: 0,
    lines: [
      { type: 'paragraph', text: 'A small bird sat on the fence and did not stir. "Could we fly to the barn?" said Giri. "I could," said the bird, "but should I? A storm is coming. I can smell the rain."' },
      { type: 'paragraph', text: '"Would she come if I marched along?" said Giri. "She could ride on my arm." The bird looked at the dark sky. "We should not go far," she said. "We should get under cover first."' },
      { type: 'paragraph', text: 'Big drops of water started to fall. Giri and the bird darted under the cart by the farm. The storm passed over them with a roar. Then the sun came back and lit up the yard.' },
      { type: 'end',       text: '"The bird had it right," said Giri. "Could, would, should — you think hard first!" "Yes," said the bird. "That is why I am a smart bird."' },
    ],
    comprehension: [
      { q: 'Why did the bird not want to fly to the barn?', options: ['A storm was near', 'She was tired', 'She was scared of Giri', 'It was too dark'], answer: 0, type: 'literal' },
      { q: 'Where did Giri and the bird hide?', options: ['Under the cart', 'In the barn', 'Up a tree', 'In the house'], answer: 0, type: 'literal' },
      { q: 'What lesson did Giri learn?', options: ['Think hard before you act', 'Run fast in the rain', 'Birds cannot fly', 'Storms are fun'], answer: 0, type: 'inferential' },
    ],
  },

  // ╔══════════════════════════════════════════════════════════════════════════╗
  // ║  BAND D — Bridge Readers  (6 books)                                   ║
  // ║  140–250 words · multi-paragraph · no fixed refrain · Tier flexible   ║
  // ║  Includes comprehension, vocab, grammarSpotlight                       ║
  // ╚══════════════════════════════════════════════════════════════════════════╝

  {
    id: 'core-d-01',
    band: 'D', level: 4, phase: 'advanced-vowel',
    title: 'Giri at the Fair',
    emoji: '🎡',
    mascotState: 'celebrate',
    illustration: 'giri_level04_story01_at-the-fair.png.jpg',
    targetGraphemes: ['oi', 'oy', 'air', 'are'],
    allowedHFWTier: 3,
    targetWordCount: 170,
    actualWordCount: 145,
    decodableRatio: 1.00,
    textType: 'bridge-reader',
    refrainCount: 0,
    lines: [
      { type: 'paragraph', text: 'The annual fair had arrived and the whole town was buzzing with joy. Giri joined the crowd and walked past stalls selling toys, popcorn, and bright balloons. A pair of clowns juggled on the main stage while children pointed and cheered.' },
      { type: 'paragraph', text: 'Giri spotted a chair-mending stall at the far end. An old carpenter was repairing a wooden chair with care. One leg had a nasty crack. Giri watched as the carpenter oiled the joint and tapped a fresh dowel into place.' },
      { type: 'paragraph', text: '"Would you like to try?" the carpenter asked. Giri took the mallet and gave the dowel one careful tap. It slotted in perfectly. The carpenter nodded and smiled. "You have a fair hand for this work," he said.' },
      { type: 'end',       text: 'Giri walked home through the evening air, feeling proud. He had discovered a new skill and made a new friend — all in one joyful afternoon.' },
    ],
    comprehension: [
      { q: 'What were the clowns doing at the fair?', options: ['Juggling on the stage', 'Selling popcorn', 'Riding horses', 'Painting faces'], answer: 0, type: 'literal' },
      { q: 'What was the carpenter repairing?', options: ['A wooden chair', 'A stall', 'A toy', 'A table'], answer: 0, type: 'literal' },
      { q: 'What does "a fair hand" mean in this story?', options: ['A natural talent or skill', 'A clean hand', 'A large hand', 'A fair deal'], answer: 0, type: 'inferential' },
    ],
    vocab: [
      { word: 'annual', meaning: 'Happening once every year', icon: '📅' },
      { word: 'carpenter', meaning: 'A person who makes and repairs things from wood', icon: '🪚' },
      { word: 'dowel', meaning: 'A small round wooden peg used to hold pieces together', icon: '🪵' },
      { word: 'mallet', meaning: 'A type of hammer, often with a wooden head', icon: '🔨' },
      { word: 'discovered', meaning: 'Found out something new for the first time', icon: '💡' },
    ],
    grammarSpotlight: [
      { pattern: 'Diphthongs (oi, oy)', example: '"joy", "joyful", "toys", "joint", "oiled", "pointed"', tip: 'oi and oy make the same sound. oi usually comes in the middle (oil, joint), oy at the end (joy, toy).' },
      { pattern: 'Speech and dialogue', example: '"Would you like to try?" the carpenter asked.', tip: 'Use speech marks and a speech verb (asked, said) to show who is speaking.' },
    ],
  },

  {
    id: 'core-d-02',
    band: 'D', level: 4, phase: 'diphthongs',
    title: 'Giri and the Joyful Noise',
    emoji: '🎶',
    mascotState: 'clap',
    illustration: 'giri_level04_story02_joyful-noise.png.jpg',
    targetGraphemes: ['oi', 'oy', 'ou', 'ow'],
    allowedHFWTier: 3,
    targetWordCount: 175,
    actualWordCount: 149,
    decodableRatio: 0.99,
    textType: 'bridge-reader',
    refrainCount: 0,
    lines: [
      { type: 'paragraph', text: 'Giri was walking through town when he heard a loud, joyful sound coming from the community hall. He followed the noise and found a crowd of people singing and clapping. A woman with a brown shawl was leading the group.' },
      { type: 'paragraph', text: '"Come and join!" she called out. Giri felt a bit shy. He sat on a stool by the window and listened. The voices bounced around the room — high, low, loud, and soft. The sound made him feel warm inside.' },
      { type: 'paragraph', text: 'At last, Giri stood up and joined in. His voice was small at first, but it grew louder with each note. Soon it rang out loud and proud, bouncing round the hall. The woman gave him a proud nod. The whole group swayed and sang until the sun went down.' },
      { type: 'end',       text: 'Giri walked home humming the tune. He had found something that brought him joy — the power of voices joined together.' },
    ],
    pretaught: ['heard'],
    comprehension: [
      { q: 'Where was the singing happening?', options: ['In the community hall', 'At the park', 'At school', "In Giri's flat"], answer: 0, type: 'literal' },
      { q: 'Why did Giri not join in at first?', options: ['He felt shy', 'He was tired', 'He did not like music', 'He could not find the hall'], answer: 0, type: 'inferential' },
      { q: 'What is the main message of this story?', options: ['Joining in can bring unexpected joy', 'Singing is only for adults', 'You should always sit by the window', 'Loud noises are bad'], answer: 0, type: 'inferential' },
    ],
    vocab: [
      { word: 'community', meaning: 'A group of people who live or meet in the same area', icon: '🏘️' },
      { word: 'bounced', meaning: 'Hit a surface and came back; echoed around', icon: '🔊' },
      { word: 'swayed', meaning: 'Moved gently from side to side', icon: '🎵' },
      { word: 'humming', meaning: 'Making a tune with your lips closed', icon: '🎶' },
      { word: 'power', meaning: 'The ability or strength to do something', icon: '⚡' },
    ],
    grammarSpotlight: [
      { pattern: 'Diphthongs (ou, ow)', example: '"loud", "sound", "around", "found", "brown", "down", "crowd", "bounced"', tip: 'ou and ow make the same /ow/ sound: loud, crowd, brown, found.' },
      { pattern: 'Contrast with "but"', example: 'His voice was small at first, but it grew louder.', tip: '"But" shows a change or contrast between two ideas.' },
    ],
  },

  {
    id: 'core-d-03',
    band: 'D', level: 4, phase: 'advanced-vowel',
    title: 'Giri Repairs the Chair',
    emoji: '🪑',
    mascotState: 'thinking',
    illustration: 'giri_level04_story03_repairs-chair.png.jpg',
    targetGraphemes: ['air', 'are', 'ear'],
    allowedHFWTier: 3,
    targetWordCount: 170,
    actualWordCount: 158,
    decodableRatio: 1.00,
    textType: 'bridge-reader',
    refrainCount: 0,
    lines: [
      { type: 'paragraph', text: "An old rocking chair sat in the stairwell of Giri's block. It had been there for as long as anyone could remember. The seat was bare, the armrest had a tear, and the paint was wearing thin. No one seemed to care about it any more." },
      { type: 'paragraph', text: "Giri decided to repair the chair. He searched his grandfather's workshop and found sandpaper, glue, and a spare plank. First he glued the cracked armrest. Then he sanded the seat until it was smooth. Finally he cut the plank to replace the broken slat at the back." },
      { type: 'paragraph', text: 'It took two afternoons of careful work. When the chair was done, Giri placed a small cushion on the seat and set it by the corridor window where the air was fresh.' },
      { type: 'end',       text: 'Mrs Tan from next door sat down and sighed with happiness. "This is the best seat in the block," she declared. Giri beamed. A little care had turned something forgotten into something shared.' },
    ],
    comprehension: [
      { q: 'Where was the old chair?', options: ['In the stairwell', "In Giri's flat", 'At the market', 'In the park'], answer: 0, type: 'literal' },
      { q: 'What tools did Giri find?', options: ['Sandpaper, glue, and a plank', 'A hammer and nails', 'Paint and a brush', 'A saw and tape'], answer: 0, type: 'literal' },
      { q: 'What does "something shared" mean at the end?', options: ['Something everyone can enjoy', 'Something expensive', 'Something Giri kept for himself', 'Something broken'], answer: 0, type: 'inferential' },
    ],
    vocab: [
      { word: 'stairwell', meaning: 'The part of a building that holds the stairs', icon: '🏢' },
      { word: 'sandpaper', meaning: 'Rough paper used to make wood smooth', icon: '📄' },
      { word: 'spare', meaning: 'Extra; kept in case it is needed', icon: '➕' },
      { word: 'declared', meaning: 'Said something firmly and clearly', icon: '📢' },
      { word: 'beamed', meaning: 'Smiled very widely with happiness', icon: '😄' },
    ],
    grammarSpotlight: [
      { pattern: 'Vowel patterns (air, are, ear)', example: '"chair", "repair", "stairwell", "air", "bare", "care", "spare", "declared", "wearing"', tip: 'air, are, and ear can all say the /air/ sound: chair, bare, wearing.' },
      { pattern: 'Sequence markers', example: 'First he glued. Then he sanded. Finally he cut.', tip: 'Use first, then, next, finally to put events in order.' },
    ],
  },

  {
    id: 'core-d-04',
    band: 'D', level: 4, phase: 'advanced-vowel',
    title: 'Giri Hears a Cheer',
    emoji: '🎉',
    mascotState: 'confetti',
    illustration: 'giri_level04_story04_hears-cheer.png.jpg',
    targetGraphemes: ['ear', 'eer', 'ere'],
    allowedHFWTier: 3,
    targetWordCount: 180,
    actualWordCount: 161,
    decodableRatio: 1.00,
    textType: 'bridge-reader',
    refrainCount: 0,
    lines: [
      { type: 'paragraph', text: 'It was the last day of the school year. Giri sat near the window, feeling a small knot of fear in his chest. His friends were moving to different classes next year. Would he still see them? Would they forget about him?' },
      { type: 'paragraph', text: 'The bell rang and Giri walked out to the field. The grass was clear and green in the sunshine. Then, from behind the canteen, he heard a cheer. "Giri! Over here!" He ran towards the sound and saw all his friends gathered under the old rain tree.' },
      { type: 'paragraph', text: "They had set up a small farewell party — sandwiches, lemon cordial, and a hand-drawn card with each friend's name. \"We will always be near, even if we are not in the same room,\" said Mei Ling." },
      { type: 'end',       text: 'Giri felt a tear of happiness on his cheek. He looked around at the smiling faces. This was not an ending — it was the beginning of something dear that would last for years to come.' },
    ],
    comprehension: [
      { q: 'Why was Giri feeling worried?', options: ['His friends were moving to different classes', 'He had lost his bag', 'He failed a test', 'The school was closing'], answer: 0, type: 'literal' },
      { q: 'What did his friends prepare?', options: ['A farewell party', 'A football match', 'A school play', 'A treasure hunt'], answer: 0, type: 'literal' },
      { q: 'What did Mei Ling mean by "we will always be near"?', options: ['True friends stay close in heart even when apart', 'They would be in the same class', 'They lived next door', 'They would call every day'], answer: 0, type: 'inferential' },
    ],
    vocab: [
      { word: 'farewell', meaning: 'A way of saying goodbye', icon: '👋' },
      { word: 'gathered', meaning: 'Came together in one place', icon: '🤝' },
      { word: 'cordial', meaning: 'A sweet fruit drink you mix with water', icon: '🥤' },
      { word: 'dear', meaning: 'Very important and loved', icon: '❤️' },
      { word: 'beginning', meaning: 'The start of something new', icon: '🌅' },
    ],
    grammarSpotlight: [
      { pattern: 'Vowel patterns (ear, eer, ere)', example: '"fear", "hear", "near", "cheer", "year", "dear", "tear", "here"', tip: 'ear and eer can say the /ear/ sound: hear, cheer, near, dear.' },
      { pattern: 'Theme or moral', example: 'This was not an ending — it was the beginning of something dear.', tip: 'Stories often end with a lesson or idea that makes us think.' },
    ],
  },

  {
    id: 'core-d-05',
    band: 'D', level: 4, phase: 'diphthongs',
    title: "Giri's Sports Day",
    emoji: '🏅',
    mascotState: 'trophy',
    illustration: 'giri_level04_story05_sports-day.jpg',
    targetGraphemes: ['ou', 'ow', 'oi', 'oy'],
    allowedHFWTier: 3,
    targetWordCount: 180,
    actualWordCount: 180,
    decodableRatio: 1.00,
    textType: 'bridge-reader',
    refrainCount: 0,
    lines: [
      { type: 'paragraph', text: 'It was Sports Day. The whole school poured out onto the field. Flags snapped in the wind, and the crowd on the bleachers made a joyful roar. Giri was in the relay team. He bounced on his toes at the starting point, trying to shake off the doubt in his stomach.' },
      { type: 'paragraph', text: 'The whistle blew. The first two runners sprinted down the track. When the baton reached Giri, he gripped it and ran with all his power. Halfway round the bend, his foot caught a mound of soil. He stumbled, fell, and felt the grit sting his knee.' },
      { type: 'paragraph', text: 'For a moment the noise around him seemed to fade away. Then the crowd found their voice. "Go, Giri! Get up!" Giri pushed himself to his feet and ran on. He was not the fastest, but he crossed the line with the baton held high.' },
      { type: 'end',       text: 'His team did not win a trophy, but they won something better — the loudest cheer of the day. Giri felt proud. Courage, he decided, was not about never falling down. It was about getting up and carrying on.' },
    ],
    comprehension: [
      { q: 'What caused Giri to fall?', options: ['His foot caught a mound of soil', 'He tripped on the baton', 'Another runner pushed him', 'He slipped on wet grass'], answer: 0, type: 'literal' },
      { q: 'What encouraged Giri to get back up?', options: ['The crowd cheering for him', 'His coach told him to', 'He wanted a trophy', 'He was not hurt'], answer: 0, type: 'inferential' },
      { q: 'What did Giri learn about courage?', options: ['It is about getting up when you fall', 'It means winning every race', 'It means never being scared', 'It means being the fastest'], answer: 0, type: 'inferential' },
    ],
    vocab: [
      { word: 'relay', meaning: 'A race where team members take turns running', icon: '🏃' },
      { word: 'stumbled', meaning: 'Nearly fell; lost balance while moving', icon: '🦶' },
      { word: 'baton', meaning: 'A stick passed between runners in a relay race', icon: '🪄' },
      { word: 'courage', meaning: 'Being brave enough to do something difficult or scary', icon: '🦁' },
      { word: 'trophy', meaning: 'A prize given to a winner, often a cup or medal', icon: '🏆' },
    ],
    grammarSpotlight: [
      { pattern: 'Diphthongs (ou, ow, oi, oy)', example: '"poured", "crowd", "found", "mound", "around", "soil", "voice", "joyful", "doubt", "power"', tip: 'ou/ow = /ow/ sound. oi/oy = /oy/ sound. Listen for them as you read.' },
      { pattern: 'Theme statement', example: 'Courage was not about never falling down. It was about getting up.', tip: 'A theme statement tells the big idea of the story in one or two sentences.' },
    ],
  },

  {
    id: 'core-d-06',
    band: 'D', level: 4, phase: 'advanced-vowel',
    title: 'Giri and the Treasure Map',
    emoji: '🗺️',
    mascotState: 'thinking',
    illustration: 'giri_level04_story06_treasure-map.jpg',
    targetGraphemes: ['aw', 'air', 'ear'],
    allowedHFWTier: 3,
    targetWordCount: 185,
    actualWordCount: 171,
    decodableRatio: 1.00,
    textType: 'bridge-reader',
    refrainCount: 0,
    lines: [
      { type: 'paragraph', text: "Giri found an old piece of parchment stuffed inside a drawer at his grandmother's house. It was a hand-drawn map with a dotted line, a compass, and one word: \"Treasure.\" Giri's jaw dropped. Could this be real?" },
      { type: 'paragraph', text: 'He followed the map across the lawn, past a pair of palm trees, and down a narrow stairway to the canal path. The trail led to an old wall covered in crawling vines. Near the base, a small wooden box was half-buried in the earth.' },
      { type: 'paragraph', text: 'Giri knelt and carefully pried the lid open. Inside he found a faded photograph of his grandmother as a girl, a pair of pearl earrings, and a short letter. "To the one who finds this," it read, "you have the same spirit of adventure that I had. Take care of it."' },
      { type: 'end',       text: 'Giri sat back on his heels, awed. The treasure was not gold or jewels — it was a story, a memory, and a connection to someone he loved. He carried the box home with the greatest care.' },
    ],
    comprehension: [
      { q: 'Where did Giri find the map?', options: ["In a drawer at his grandmother's house", 'At school', 'In the park', 'At the market'], answer: 0, type: 'literal' },
      { q: 'What was inside the box?', options: ['A photo, earrings, and a letter', 'Gold coins', 'Jewels and gems', 'Old books'], answer: 0, type: 'literal' },
      { q: 'Why is the real treasure not gold or jewels?', options: ['The real treasure is the family connection and memories', 'Gold is too heavy', 'Jewels are not real', 'The box was too small'], answer: 0, type: 'inferential' },
    ],
    vocab: [
      { word: 'parchment', meaning: 'Thick old paper, often used for maps or letters', icon: '📜' },
      { word: 'compass', meaning: 'A tool that shows direction (north, south, east, west)', icon: '🧭' },
      { word: 'canal', meaning: 'A man-made waterway for boats or drainage', icon: '🚢' },
      { word: 'awed', meaning: 'Filled with wonder and amazement', icon: '😲' },
      { word: 'spirit', meaning: 'A special quality of enthusiasm and energy', icon: '✨' },
    ],
    grammarSpotlight: [
      { pattern: 'Vowel patterns (aw, au, ear, air)', example: '"jaw", "drawer", "stairway", "crawling", "lawn", "earrings", "pair", "care"', tip: 'aw/au say /aw/: jaw, haul. ear says /ear/: ear, near. air says /air/: pair, stair.' },
      { pattern: 'Letter writing', example: '"To the one who finds this, you have the same spirit of adventure."', tip: 'Letters use a greeting, a message, and sometimes a sign-off. This letter speaks directly to the reader.' },
    ],
  },

  {
    id: 'core-d-07',
    band: 'D', level: 4, phase: 'diphthongs',
    title: 'Giri Goes to Town',
    emoji: '🎡',
    mascotState: 'pointRight',
    illustration: 'giri_level04_story07_windmill.jpg',
    targetGraphemes: ['ou', 'ow', 'oi', 'oy'],
    allowedHFWTier: 3,
    targetWordCount: 150,
    actualWordCount: 144,
    decodableRatio: 1.00,
    textType: 'bridge-reader',
    refrainCount: 0,
    lines: [
      { type: 'paragraph', text: "On Saturday, Giri took the bus to town with his mum. The town was loud and busy. Crowds moved up and down the street, and the smell of fresh bread drifted out of a baker's shop." },
      { type: 'paragraph', text: 'Mum gave Giri two dollars to spend. He counted the coins in his palm and looked round the stalls. So many choices! A round bun? A cold drink? A small brown mouse toy with a long tail?' },
      { type: 'paragraph', text: 'Then Giri spotted an old man selling paper windmills that spun round and round in the wind. One windmill was painted gold and brown, with a loud little clack as it turned. Giri handed over his coins without a doubt.' },
      { type: 'end',       text: 'On the bus home, Giri held his windmill up high and let it spin. Round and round it flew! A windmill that turns all day, he decided, was money well spent.' },
    ],
    comprehension: [
      { q: 'How much money did Mum give Giri to spend?', options: ['Two dollars', 'Ten dollars', 'Five cents', 'One coin'], answer: 0, type: 'literal' },
      { q: 'What did Giri buy in the end?', options: ['A paper windmill', 'A round bun', 'A cold drink', 'A toy mouse'], answer: 0, type: 'literal' },
      { q: 'Why did Giri pick the windmill over the other things?', options: ['It would keep spinning and turning all day', 'It was the cheapest', 'The man asked him to', 'His mum chose it'], answer: 0, type: 'inferential' },
    ],
    vocab: [
      { word: 'crowds', meaning: 'Big groups of people in one place', icon: '👥' },
      { word: 'counted', meaning: 'Said numbers to find how many', icon: '🔢' },
      { word: 'choices', meaning: 'Different things you can pick from', icon: '🤔' },
      { word: 'windmill', meaning: 'A toy with sails that spin in the wind', icon: '🎏' },
      { word: 'doubt', meaning: 'A feeling of not being sure', icon: '❓' },
    ],
    grammarSpotlight: [
      { pattern: 'Diphthongs (ou, ow, oi, oy)', example: '"loud", "round", "doubt", "crowds", "down", "brown", "coins", "toy"', tip: 'ou and ow say /ow/ as in "round" and "brown"; oi and oy say /oy/ as in "coin" and "toy".' },
      { pattern: 'Questions for choices', example: 'A round bun? A cold drink? A small brown mouse toy?', tip: 'Short questions in a row show someone thinking through their choices.' },
    ],
  },

  {
    id: 'core-d-08',
    band: 'D', level: 4, phase: 'advanced-vowel',
    title: 'Giri Draws at Dawn',
    emoji: '🌅',
    mascotState: 'thinking',
    illustration: 'giri_level04_story08_dawn.jpg',
    targetGraphemes: ['aw', 'au'],
    allowedHFWTier: 3,
    targetWordCount: 155,
    actualWordCount: 149,
    decodableRatio: 1.00,
    textType: 'bridge-reader',
    refrainCount: 0,
    lines: [
      { type: 'paragraph', text: 'Giri woke up before dawn one morning. The sky was still dark, and the air was cool. He took his pencils and paper down to the lawn, sat on the damp grass, and waited for the sun.' },
      { type: 'paragraph', text: 'Slowly, the sky began to change. Streaks of pink and gold crawled over the rooftops. A hawk soared past on wide, still wings. Giri drew it all — the dawn light, the yawning town, the small black cat creeping along the wall.' },
      { type: 'paragraph', text: 'His hand moved fast. He drew the lawn, the lamp posts, and the last pale star before it faded. When he looked down at his paper, the whole morning was caught there in pencil lines.' },
      { type: 'end',       text: 'At breakfast, Giri showed the drawing to his mum. "You saw all that at dawn?" she said, in awe. Giri nodded. The best part of the day, he decided, was the part most people never saw.' },
    ],
    comprehension: [
      { q: 'When did Giri go out to draw?', options: ['Before dawn, while it was still dark', 'After breakfast', 'At noon', 'At bedtime'], answer: 0, type: 'literal' },
      { q: 'What soared past on wide, still wings?', options: ['A hawk', 'A kite', 'A plane', 'A crow'], answer: 0, type: 'literal' },
      { q: 'What did Giri mean by "the part most people never saw"?', options: ['Most people are still asleep at dawn', 'The drawing was hidden', 'The lawn was private', 'The hawk flew too fast'], answer: 0, type: 'inferential' },
    ],
    vocab: [
      { word: 'dawn', meaning: 'The time when the sun first comes up', icon: '🌅' },
      { word: 'streaks', meaning: 'Long thin lines of colour or light', icon: '🖌️' },
      { word: 'soared', meaning: 'Flew high without flapping', icon: '🦅' },
      { word: 'caught', meaning: 'Held or trapped something', icon: '🪤' },
      { word: 'awe', meaning: 'A feeling of great wonder', icon: '🤩' },
    ],
    grammarSpotlight: [
      { pattern: 'Vowel patterns (aw, au)', example: '"dawn", "lawn", "crawled", "hawk", "yawning", "saw", "awe", "caught"', tip: 'aw and au make the same sound: dawn, saw, caught.' },
      { pattern: 'Describing with the senses', example: 'The sky was still dark, and the air was cool.', tip: 'Good describing tells what you can see, hear, and feel.' },
    ],
  },

  {
    id: 'core-d-09',
    band: 'D', level: 4, phase: 'advanced-vowel',
    title: 'Giri and Scout',
    emoji: '🐶',
    mascotState: 'celebrate',
    illustration: 'giri_level04_story09_scout.jpg',
    targetGraphemes: ['ear', 'are', 'air'],
    allowedHFWTier: 3,
    targetWordCount: 160,
    actualWordCount: 157,
    decodableRatio: 1.00,
    textType: 'bridge-reader',
    refrainCount: 0,
    lines: [
      { type: 'paragraph', text: 'Mrs Tan from next door was going away for the weekend. "Could you take care of Scout for two days?" she asked Giri. Scout was her small brown dog with one floppy ear and a tail that never stopped wagging.' },
      { type: 'paragraph', text: "Taking care of a dog turned out to be real work. Scout needed fresh food and clean water, a walk in the morning, and a walk at night. He barked at the lift and chased his tail on the stairs. Giri had to keep him away from Mrs Tan's plants!" },
      { type: 'paragraph', text: 'But on the second day, something changed. Scout curled up beside Giri while he read his book, one ear up, fast asleep. Giri scratched the soft fur behind it and smiled.' },
      { type: 'end',       text: 'When Mrs Tan came home, Scout ran to meet her. "He looks so happy and well cared for!" she said. Giri felt proud. Caring for a pet was hard work — and worth every minute of it.' },
    ],
    comprehension: [
      { q: 'Why did Giri look after Scout?', options: ['Mrs Tan was going away for the weekend', 'Scout was lost', 'Giri found him at the park', 'Scout was sick'], answer: 0, type: 'literal' },
      { q: 'What did Scout do on the second day?', options: ['Curled up beside Giri and slept', 'Ran away', 'Barked all day', 'Dug up the plants'], answer: 0, type: 'literal' },
      { q: 'What did Giri learn about caring for a pet?', options: ['It is hard work but worth it', 'It is always easy', 'Dogs do not need walks', 'Pets like being alone'], answer: 0, type: 'inferential' },
    ],
    vocab: [
      { word: 'weekend', meaning: 'Saturday and Sunday', icon: '📅' },
      { word: 'floppy', meaning: 'Soft and hanging down loosely', icon: '🐶' },
      { word: 'curled up', meaning: 'Lay down in a small round shape', icon: '😴' },
      { word: 'proud', meaning: 'Feeling glad about something you did well', icon: '🏅' },
      { word: 'worth', meaning: 'Good enough to deserve the effort', icon: '💎' },
    ],
    grammarSpotlight: [
      { pattern: 'Vowel patterns (ear, are, air)', example: '"ear", "care", "cared", "caring", "stairs"', tip: 'ear says /ear/ as in "ear"; are and air say /air/ as in "care" and "stairs".' },
      { pattern: 'Contrast with "but"', example: 'He was hard work. But on the second day, something changed.', tip: '"But" signals that the next idea is different from the one before.' },
    ],
  },

  {
    id: 'core-d-10',
    band: 'D', level: 4, phase: 'diphthongs',
    title: 'Giri and the Coin Jar',
    emoji: '🫙',
    mascotState: 'trophy',
    illustration: 'giri_level04_story10_orchids.jpg',
    targetGraphemes: ['oi', 'ou', 'ow'],
    allowedHFWTier: 3,
    targetWordCount: 160,
    actualWordCount: 156,
    decodableRatio: 1.00,
    textType: 'bridge-reader',
    refrainCount: 0,
    lines: [
      { type: 'paragraph', text: 'Giri kept a coin jar on his desk. Every day he dropped in the coins left in his pocket, and the jar grew heavier month by month. He was saving up for a big red robot from the toy shop in town.' },
      { type: 'paragraph', text: 'Every Sunday, he counted his coins out loud. "A dollar. Two dollars. Three dollars and twenty cents!" He was proud of every coin in that jar.' },
      { type: 'paragraph', text: 'Then one day, walking home past the school, Giri spotted his mum outside the flower shop. She was looking at a pot of orchids. She smiled at the flowers, then looked at the price and walked on. That night, Giri stopped counting his coins for the robot. He had found something better to buy.' },
      { type: 'end',       text: 'On Sunday, Giri proudly handed his mum the pot of orchids. Her mouth fell open, then curved into the widest smile. "My boy!" she said. Some things, Giri decided, count more than any toy.' },
    ],
    comprehension: [
      { q: 'What was Giri saving his coins for at first?', options: ['A big red robot', 'A pot of orchids', 'A bus ride', 'A book'], answer: 0, type: 'literal' },
      { q: 'What did Giri see his mum looking at?', options: ['A pot of orchids at the flower shop', 'A red robot', 'A coin jar', 'A new dress'], answer: 0, type: 'literal' },
      { q: 'Why did Giri stop saving for the robot?', options: ['He wanted to buy the orchids for his mum instead', 'He lost his coins', 'The robot was sold out', 'He stopped liking robots'], answer: 0, type: 'inferential' },
    ],
    vocab: [
      { word: 'saving', meaning: 'Keeping money to use later', icon: '🪙' },
      { word: 'heavier', meaning: 'Weighing more than before', icon: '⚖️' },
      { word: 'orchids', meaning: 'Bright flowers — the national flower of Singapore is one!', icon: '🌸' },
      { word: 'price', meaning: 'How much money something costs', icon: '🏷️' },
      { word: 'curved', meaning: 'Bent into a smooth round shape', icon: '🌙' },
    ],
    grammarSpotlight: [
      { pattern: 'Diphthongs (oi, ou, ow)', example: '"coin", "counted", "loud", "proud", "found", "mouth", "flowers", "town"', tip: 'oi says /oy/ as in "coin"; ou and ow say /ow/ as in "loud" and "town".' },
      { pattern: 'Showing feelings without telling', example: 'Her mouth fell open, then curved into the widest smile.', tip: 'Instead of saying "she was happy", the writer shows what her face did.' },
    ],
  },

  // ╔══════════════════════════════════════════════════════════════════════════╗
  // ║  EXTENSION — Singapore Readers  (6 books)                             ║
  // ║  Teacher-supported local-interest stories, category: 'nonfiction-sg'  ║
  // ╚══════════════════════════════════════════════════════════════════════════╝

  {
    id: 'sg-mrt',
    band: 'B', level: 2, phase: 'extension-sg',
    title: 'Giri Rides the MRT',
    emoji: '🚆',
    mascotState: 'neutral',
    illustration: 'giri_sg_mrt.jpg',
    category: 'nonfiction-sg',
    targetGraphemes: ['a', 'e', 'i', 'o', 'u'],
    allowedHFWTier: 2,
    targetWordCount: 65,
    actualWordCount: 54,
    decodableRatio: 1.00,
    textType: 'extension-sg',
    refrainCount: 0,
    lines: [
      { type: 'intro',   text: 'Giri got on the big red MRT. The MRT was fast.' },
      { type: 'label',   text: 'Problem:' },
      { type: 'beat',    text: 'The MRT was full. Giri had no seat.' },
      { type: 'label',   text: 'Attempt:' },
      { type: 'beat',    text: 'Giri held on to the rod and did not slip.' },
      { type: 'label',   text: 'Solution:' },
      { type: 'beat',    text: 'A kind man let Giri sit in his seat. Giri sat and felt glad.' },
      { type: 'end',     text: 'Giri got off at his stop and gave a big grin.' },
    ],
    comprehension: [
      { q: 'What did Giri get on?', options: ['MRT', 'bus', 'boat', 'car'], answer: 0, type: 'literal' },
      { q: 'What did Giri hold on to?', options: ['rod', 'mat', 'bag', 'hat'], answer: 0, type: 'literal' },
      { q: 'How did the kind man help Giri?', options: ['Let Giri sit', 'Gave Giri a bag', 'Ran with Giri', 'Got Giri a drink'], answer: 0, type: 'inferential' },
    ],
    vocab: [
      { word: 'MRT', meaning: 'A fast train that runs around Singapore', icon: '🚆' },
      { word: 'rod', meaning: 'A long bar you hold on to for balance', icon: '🪄' },
      { word: 'slip', meaning: 'To slide by mistake and lose your balance', icon: '🧊' },
      { word: 'kind', meaning: 'Caring and helpful to others', icon: '💛' },
      { word: 'grin', meaning: 'A big happy smile', icon: '😄' },
    ],
    grammarSpotlight: [
      { pattern: 'Adjectives (describing words)', example: 'The big red MRT was fast.', tip: '"Big" and "red" describe the MRT. These are adjectives!' },
      { pattern: 'Past-tense verbs', example: 'Giri held on to the rod. He slid on the mat.', tip: 'Words like "held" and "slid" tell us what happened in the past.' },
    ],
  },

  {
    id: 'sg-school',
    band: 'B', level: 2, phase: 'extension-sg',
    title: "Giri's First Day at School",
    emoji: '🎒',
    mascotState: 'encourage',
    illustration: 'giri_sg_school.jpg',
    category: 'nonfiction-sg',
    targetGraphemes: ['a', 'e', 'i', 'o', 'u'],
    allowedHFWTier: 3,
    targetWordCount: 70,
    actualWordCount: 54,
    decodableRatio: 1.00,
    textType: 'extension-sg',
    refrainCount: 0,
    lines: [
      { type: 'intro',   text: "It was Giri's first day at school. He had a big red bag." },
      { type: 'label',   text: 'Problem:' },
      { type: 'beat',    text: 'Giri lost his peg. He did not know where to put his bag.' },
      { type: 'label',   text: 'Attempt:' },
      { type: 'beat',    text: 'Giri ran to look in each room.' },
      { type: 'label',   text: 'Solution:' },
      { type: 'beat',    text: 'A kind pal led Giri to his peg. Giri hung his bag up.' },
      { type: 'end',     text: 'Giri sat at his desk and felt glad.' },
    ],
    comprehension: [
      { q: 'What could Giri not find?', options: ['His peg', 'His bag', 'His hat', 'His book'], answer: 0, type: 'literal' },
      { q: 'What room did Giri go into?', options: ['The wrong room', 'His own room', 'The big room', 'The wet room'], answer: 0, type: 'literal' },
      { q: 'How do you think Giri felt at the start of school?', options: ['Worried and lost', 'Very happy', 'Very sleepy', 'Very hungry'], answer: 0, type: 'inferential' },
    ],
    vocab: [
      { word: 'peg', meaning: 'A hook on a wall to hang your bag or coat', icon: '🪝' },
      { word: 'desk', meaning: 'A table used for doing school work', icon: '📚' },
      { word: 'room', meaning: 'A part of a building inside four walls', icon: '🏠' },
      { word: 'wrong', meaning: 'Not correct; not the right one', icon: '❌' },
      { word: 'kind', meaning: 'Friendly and helpful to others', icon: '💛' },
    ],
    grammarSpotlight: [
      { pattern: 'Nouns (naming words)', example: 'Giri had a big red bag, a peg, and a desk.', tip: '"Bag", "peg", and "desk" are nouns. Nouns name people, places, or things.' },
      { pattern: 'Plurals with -s', example: 'pegs, rooms, bags, desks', tip: 'Add -s to most nouns to show more than one: bag → bags, room → rooms.' },
    ],
  },

  {
    id: 'sg-hawker',
    band: 'B', level: 2, phase: 'extension-sg',
    title: 'Giri at the Hawker Centre',
    emoji: '🍜',
    mascotState: 'celebrate',
    illustration: 'giri_sg_hawker.jpg',
    category: 'nonfiction-sg',
    targetGraphemes: ['a_e', 'ai'],
    allowedHFWTier: 3,
    targetWordCount: 70,
    actualWordCount: 63,
    decodableRatio: 1.00,
    textType: 'extension-sg',
    refrainCount: 0,
    lines: [
      { type: 'intro',   text: 'Giri came to the hawker centre to get his mum a treat.' },
      { type: 'label',   text: 'Problem:' },
      { type: 'beat',    text: 'He got lost in the maze of lanes and stalls.' },
      { type: 'label',   text: 'Attempt:' },
      { type: 'beat',    text: 'Giri went from lane to lane and looked at each stall.' },
      { type: 'label',   text: 'Solution:' },
      { type: 'beat',    text: 'A nice auntie smiled and led him to the noodle stall. Giri gave a wave and paid.' },
      { type: 'end',     text: 'Giri went home with the noodles. His mum gave him a huge smile.' },
    ],
    comprehension: [
      { q: 'Why did Giri go to the hawker centre?', options: ['To get a treat for his mum', 'To play', 'To get a toy', 'To take a nap'], answer: 0, type: 'literal' },
      { q: 'Who helped Giri find the stall?', options: ['A nice auntie', 'A kind man', 'His mum', 'The chef'], answer: 0, type: 'literal' },
      { q: 'How do you think Giri felt when he was lost?', options: ['Worried and confused', 'Happy and excited', 'Hungry and tired', 'Bored and sleepy'], answer: 0, type: 'inferential' },
    ],
    vocab: [
      { word: 'hawker centre', meaning: 'An open-air place with many food stalls in Singapore', icon: '🏪' },
      { word: 'maze', meaning: 'A set of paths that is hard to find your way through', icon: '🌀' },
      { word: 'lane', meaning: 'A narrow path or street', icon: '↔️' },
      { word: 'stall', meaning: 'A small stand that sells food or things', icon: '🍜' },
      { word: 'auntie', meaning: 'A friendly word for an older woman in Singapore', icon: '👩' },
    ],
    grammarSpotlight: [
      { pattern: 'Past tense with -ed', example: 'The auntie smiled and led him to the stall. Giri waved and paid.', tip: 'Add -ed to show something happened in the past: smile → smiled, wave → waved.' },
      { pattern: 'Nouns in a list', example: 'He saw lanes, stalls, and treats at the hawker centre.', tip: 'Use commas to list nouns, and add "and" before the last one.' },
    ],
  },

  {
    id: 'sg-cny',
    band: 'B', level: 2, phase: 'extension-sg',
    title: 'Giri and the New Year',
    emoji: '🧧',
    mascotState: 'confetti',
    illustration: 'giri_sg_cny.jpg',
    category: 'nonfiction-sg',
    targetGraphemes: ['a_e', 'igh'],
    allowedHFWTier: 3,
    targetWordCount: 70,
    actualWordCount: 55,
    decodableRatio: 1.00,
    textType: 'extension-sg',
    refrainCount: 0,
    lines: [
      { type: 'intro',   text: 'It was the New Year. The lane was full of red lights and gold flags.' },
      { type: 'label',   text: 'Problem:' },
      { type: 'beat',    text: 'Giri lost his mum in the big crowd.' },
      { type: 'label',   text: 'Attempt:' },
      { type: 'beat',    text: 'Giri called out, "Mum! Mum!" He ran up the lane.' },
      { type: 'label',   text: 'Solution:' },
      { type: 'beat',    text: 'Giri spied the red light of the stall. His mum waved to him.' },
      { type: 'end',     text: 'They sat and ate sweet kueh. Giri felt glad.' },
    ],
    comprehension: [
      { q: 'What made the lane look bright?', options: ['Red lights and gold flags', 'A big cake', 'White stars', 'Blue lamps'], answer: 0, type: 'literal' },
      { q: 'What did Giri do when he got lost?', options: ['Called out to his mum', 'Sat down and cried', 'Ran home', 'Went to sleep'], answer: 0, type: 'literal' },
      { q: 'What helped Giri find his mum in the end?', options: ['He saw her wave from the stall', 'She called his name', 'He asked a stranger', 'He found her bag'], answer: 0, type: 'inferential' },
    ],
    vocab: [
      { word: 'crowd', meaning: 'A large group of people all in one place', icon: '👥' },
      { word: 'lane', meaning: 'A narrow street or path', icon: '↔️' },
      { word: 'lost', meaning: 'Not knowing where you are or where to go', icon: '❓' },
      { word: 'kueh', meaning: 'A sweet snack popular in Singapore', icon: '🍮' },
      { word: 'wave', meaning: 'To move your hand to greet or say goodbye', icon: '👋' },
    ],
    grammarSpotlight: [
      { pattern: 'Exclamation marks (!)', example: '"Mum! Mum!" Giri called out.', tip: 'Use ! when someone shouts or feels something strongly.' },
      { pattern: 'Adjectives before nouns', example: 'the big crowd, the red light, the sweet kueh', tip: 'Adjectives go right before the noun they describe.' },
    ],
  },

  {
    id: 'sg-deepavali',
    band: 'B', level: 2, phase: 'extension-sg',
    title: 'Giri at Deepavali',
    emoji: '🪔',
    mascotState: 'celebrate',
    illustration: 'giri_sg_deepavali.jpg',
    category: 'nonfiction-sg',
    targetGraphemes: ['a_e', 'igh'],
    allowedHFWTier: 3,
    targetWordCount: 70,
    actualWordCount: 54,
    decodableRatio: 1.00,
    textType: 'extension-sg',
    refrainCount: 0,
    lines: [
      { type: 'intro',   text: 'It was Deepavali. The street had bright oil lamps and gold clay pots.' },
      { type: 'label',   text: 'Problem:' },
      { type: 'beat',    text: "Giri's oil lamp went out in the wind." },
      { type: 'label',   text: 'Attempt:' },
      { type: 'beat',    text: 'Giri cupped his hands to keep the flame safe.' },
      { type: 'label',   text: 'Solution:' },
      { type: 'beat',    text: 'A kind auntie came and lit the lamp. The lane shone bright once more.' },
      { type: 'end',     text: 'Giri sat and ate a sweet treat. He felt glad.' },
    ],
    comprehension: [
      { q: 'What went out in the wind?', options: ["Giri's oil lamp", "Giri's candle", "Giri's torch", "Giri's star"], answer: 0, type: 'literal' },
      { q: 'What did Giri do to save the flame?', options: ['Cupped his hands', 'Ran inside', 'Got a new lamp', 'Blew on it'], answer: 0, type: 'literal' },
      { q: 'Why is Deepavali called the Festival of Lights?', options: ['People light lamps all around', 'People fly kites', 'People eat sweet treats', 'People sing songs'], answer: 0, type: 'inferential' },
    ],
    vocab: [
      { word: 'Deepavali', meaning: 'A festival of lights celebrated by Indian families in Singapore and around the world', icon: '🪔' },
      { word: 'oil lamp', meaning: 'A small dish with oil and a wick that gives light when lit', icon: '🕯️' },
      { word: 'flame', meaning: 'The bright, glowing part of a fire', icon: '🔥' },
      { word: 'bright', meaning: 'Giving out a lot of light; not dark', icon: '✨' },
      { word: 'celebrate', meaning: 'To do something fun and special for a happy occasion', icon: '🎉' },
    ],
    grammarSpotlight: [
      { pattern: 'Verbs (doing words)', example: 'Giri cupped his hands. The auntie lit the lamp. The lane shone bright.', tip: '"Cupped", "lit", and "shone" are verbs. They tell us what someone or something does.' },
      { pattern: '"And" and "but" (joining words)', example: 'The street was bright and busy, but the lamp went out.', tip: '"And" joins ideas that go together. "But" joins ideas that are different.' },
    ],
  },

  {
    id: 'sg-gardens',
    band: 'C', level: 3, phase: 'extension-sg',
    title: 'Giri at Gardens by the Bay',
    emoji: '🌿',
    mascotState: 'pointRight',
    illustration: 'giri_sg_gardens.jpg',
    category: 'nonfiction-sg',
    targetGraphemes: ['ar', 'ir', 'er'],
    allowedHFWTier: 3,
    targetWordCount: 80,
    actualWordCount: 62,
    decodableRatio: 1.00,
    textType: 'extension-sg',
    refrainCount: 0,
    lines: [
      { type: 'intro',   text: 'Giri went to the Gardens by the Bay to see the tall Supertrees.' },
      { type: 'label',   text: 'Problem:' },
      { type: 'beat',    text: 'A small bird got stuck in the garden net.' },
      { type: 'label',   text: 'Attempt:' },
      { type: 'beat',    text: 'Giri called for a park ranger to help.' },
      { type: 'label',   text: 'Solution:' },
      { type: 'beat',    text: 'Giri got a long stick and helped the bird get free. The bird flew up with a chirp.' },
      { type: 'end',     text: 'Giri looked up at the stars and felt a warm glow in his heart.' },
    ],
    comprehension: [
      { q: 'Where did Giri go?', options: ['Gardens by the Bay', 'The market', 'The MRT', 'His school'], answer: 0, type: 'literal' },
      { q: 'What was stuck in the net?', options: ['A small bird', 'A bat', 'A kite', 'A cat'], answer: 0, type: 'literal' },
      { q: 'What can we learn from Giri here?', options: ['Help when you can, even if it is hard', 'Always wait for a ranger', 'Never go to a garden', 'Birds can look after themselves'], answer: 0, type: 'inferential' },
    ],
    vocab: [
      { word: 'Supertrees', meaning: 'Tall tree-shaped structures at Gardens by the Bay in Singapore', icon: '🌳' },
      { word: 'ranger', meaning: 'A person whose job is to look after a park or garden', icon: '🧑‍🌾' },
      { word: 'net', meaning: 'A material made of crossed threads with gaps between them', icon: '🪢' },
      { word: 'chirp', meaning: 'A short high sound made by a small bird', icon: '🐦' },
      { word: 'glow', meaning: 'A soft, steady light, or a warm happy feeling inside', icon: '✨' },
    ],
    grammarSpotlight: [
      { pattern: 'r-controlled vowels (ar, ir, er)', example: '"garden", "bird", "ranger", "far", "her"', tip: 'When "r" comes after a vowel, the vowel sound changes. These are called r-controlled vowels.' },
      { pattern: 'Conjunctions (joining words)', example: 'The ranger was far away and could not hear.', tip: '"And" joins two ideas in one sentence.' },
    ],
  },


  {
    id: 'core-d-sw-01',
    band: 'D', level: 4, phase: 'diphthongs',
    title: 'How Giri Found Out',
    emoji: '\u2601\ufe0f',
    mascotState: 'thinking',
    illustration: 'giri_level04_story17_found-out.jpg',
    targetGraphemes: ['ou', 'ow', 'oi'],
    allowedHFWTier: 3,
    targetWordCount: 160,
    actualWordCount: 170,
    decodableRatio: 1.00,
    textType: 'bridge-reader',
    refrainCount: 0,
    lines: [
      { type: 'paragraph', text: 'A loud sound rolled up from town to the hill where Giri stood. He looked around. A big brown cloud was rolling down the hill — and it was coming his way! "What is that? How big is it? What do I do NOW?"' },
      { type: 'paragraph', text: '"I must find out about that cloud," said Giri. He did not run. He did not hide. He marched down the road to find out for himself. The sound got louder and louder. The cloud got rounder and rounder. Giri counted his steps out loud to stay brave. "One. Two. Three. Four."' },
      { type: 'paragraph', text: 'Then Giri saw it. It was not a cloud at all! It was a crowd — a crowd of kids from town, kicking up dust as they bounced a big round ball down the hill. "Come and join us!" they shouted.' },
      { type: 'end',       text: 'Giri let out a proud shout: "I found out by myself! A thing is not so big when you find out about it." He joined the crowd, and the game was loud, round, and fun.' },
    ],
    comprehension: [
      { q: 'What did Giri think was coming down the hill?', options: ['A big brown cloud', 'A truck', 'A storm', 'A dog'], answer: 0, type: 'literal' },
      { q: 'What was the cloud really?', options: ['A crowd of kids', 'Smoke', 'Dust from a truck', 'Rain'], answer: 0, type: 'literal' },
      { q: 'What did Giri learn about scary things?', options: ['They feel smaller when you find out about them', 'They always go away', 'You should hide first', 'You should shout at them'], answer: 0, type: 'inferential' },
    ],
  },

  // ╔══════════════════════════════════════════════════════════════════════════╗
  // ║  CHAPTER STORIES — The Lost Key  (3 chapters)                         ║
  // ║  Bridge-level chapter readers, category: 'chapter'                    ║
  // ╚══════════════════════════════════════════════════════════════════════════╝

  {
    id: 'chapter-lost-key-1',
    band: 'D', level: 4, phase: 'chapter',
    title: 'The Lost Key — Chapter 1',
    emoji: '🗝️',
    mascotState: 'thinking',
    illustration: 'giri_chapter_lost-key-1.jpg',
    category: 'chapter',
    chapterOf: 'chapter-lost-key',
    chapterNum: 1,
    targetGraphemes: ['ar', 'ir', 'er'],
    allowedHFWTier: 3,
    targetWordCount: 150,
    actualWordCount: 104,
    decodableRatio: 1.00,
    textType: 'chapter-reader',
    refrainCount: 0,
    lines: [
      { type: 'chapter', text: 'Chapter 1: Giri Finds the Key' },
      { type: 'paragraph', text: 'Giri was at the park when he saw a shiny key on the path. The key had no name on it. He looked around the park to see if anyone was searching, but the path was empty and no one was near.' },
      { type: 'paragraph', text: 'Giri picked up the key and put it in his shirt pocket. He walked to the ranger hut and showed it to the park ranger. "I will help find the owner," said the ranger. He pinned a note on the park board.' },
      { type: 'end',       text: 'Giri felt glad he had not left the key on the ground. Someone out there needed it to get home.' },
    ],
    comprehension: [
      { q: 'Where did Giri find the key?', options: ['On the path at the park', 'In his pocket', 'At school', 'At the hawker centre'], answer: 0, type: 'literal' },
      { q: 'What did Giri do with the key?', options: ['Put it in his pocket and asked the ranger', 'Left it on the ground', 'Took it home', 'Threw it away'], answer: 0, type: 'literal' },
      { q: 'Why was it a good idea not to leave the key?', options: ['Someone might need it to get home', 'Keys are pretty', 'Giri wanted to keep it', 'The ranger asked him to'], answer: 0, type: 'inferential' },
    ],
    vocab: [
      { word: 'shiny', meaning: 'Bright and gleaming, like polished metal', icon: '✨' },
      { word: 'belong', meaning: 'To be the property of someone', icon: '🔑' },
      { word: 'empty', meaning: 'Having nothing or no one in it', icon: '🏜️' },
      { word: 'pocket', meaning: 'A small bag sewn into clothing to keep things in', icon: '👕' },
      { word: 'owner', meaning: 'The person who something belongs to', icon: '🙋' },
    ],
    grammarSpotlight: [
      { pattern: 'Questions and question marks (?)', example: '"Who does this key belong to?" Giri wondered.', tip: 'Use ? at the end of a question.' },
      { pattern: 'Pronouns (I, he, she, it, they)', example: 'Giri found the key. He put it in his pocket.', tip: '"He" takes the place of "Giri" and "it" takes the place of "the key".' },
    ],
  },

  {
    id: 'chapter-lost-key-2',
    band: 'D', level: 4, phase: 'chapter',
    title: 'The Lost Key — Chapter 2',
    emoji: '🗝️',
    mascotState: 'whiteboard',
    illustration: 'giri_chapter_lost-key-2.jpg',
    category: 'chapter',
    chapterOf: 'chapter-lost-key',
    chapterNum: 2,
    targetGraphemes: ['ar', 'ir', 'er'],
    allowedHFWTier: 3,
    targetWordCount: 155,
    actualWordCount: 94,
    decodableRatio: 1.00,
    textType: 'chapter-reader',
    refrainCount: 0,
    lines: [
      { type: 'chapter', text: 'Chapter 2: The Search' },
      { type: 'paragraph', text: 'The ranger put up a paper on the park board: "Found: one key. Ask at the hut." Many people came to ask, but none had the right key. Giri and the ranger checked each person one by one.' },
      { type: 'paragraph', text: 'A boy tried to grab the key and run. Giri called out, "Stop! That is not yours!" The boy gave back the key. Giri felt firm and fair. It was important to make sure the key went to the right person.' },
      { type: 'end',       text: 'Giri and the ranger went on looking. They were sure the right owner would come.' },
    ],
    comprehension: [
      { q: 'What did the ranger put on the board?', options: ['A note about the found key', 'A picture of the key', 'A list of names', 'A map of the park'], answer: 0, type: 'literal' },
      { q: 'What did the boy try to do?', options: ['Grab the key and run', 'Help find the owner', 'Ask if it was his', 'Leave the park'], answer: 0, type: 'literal' },
      { q: 'Why was it important for Giri to speak up?', options: ['To stop someone taking something that was not theirs', 'To scare the boy', 'To show off', 'Because he wanted the key'], answer: 0, type: 'inferential' },
    ],
    vocab: [
      { word: 'board', meaning: 'A flat surface on a wall where notices can be put up', icon: '📋' },
      { word: 'hut', meaning: 'A small, simple building', icon: '🛖' },
      { word: 'check', meaning: 'To look at something carefully to see if it is correct', icon: '✅' },
      { word: 'grab', meaning: 'To take something quickly and roughly', icon: '✋' },
      { word: 'fair', meaning: 'Treating everyone equally and in the right way', icon: '⚖️' },
    ],
    grammarSpotlight: [
      { pattern: 'Speech marks (" ")', example: '"Stop! That is not yours!" Giri called out.', tip: 'Use speech marks to show the exact words someone says out loud.' },
      { pattern: 'Adjectives for feelings', example: 'Giri felt firm and fair.', tip: '"Firm" and "fair" describe how Giri felt.' },
    ],
  },

  {
    id: 'chapter-lost-key-3',
    band: 'D', level: 4, phase: 'chapter',
    title: 'The Lost Key — Chapter 3',
    emoji: '🗝️',
    mascotState: 'confetti',
    illustration: 'giri_chapter_lost-key-3.jpg',
    category: 'chapter',
    chapterOf: 'chapter-lost-key',
    chapterNum: 3,
    targetGraphemes: ['oy', 'ear'],
    allowedHFWTier: 3,
    targetWordCount: 160,
    actualWordCount: 97,
    decodableRatio: 1.00,
    textType: 'chapter-reader',
    refrainCount: 0,
    lines: [
      { type: 'chapter', text: 'Chapter 3: The Right Owner' },
      { type: 'paragraph', text: 'The next day, an old man came to the hut with a sad face. He said he could not find the key to his flat. Without it, he could not get home. Giri showed him the key they had found.' },
      { type: 'paragraph', text: "The old man could not remember what his key looked like. His eyes were not clear. They tried the key in his lock. It fit! The old man gave a shout of joy. \"You are a good boy!\" he said." },
      { type: 'end',       text: 'Giri walked home and felt a warm glow of pride. A kind act is never too small.' },
    ],
    comprehension: [
      { q: 'What could the old man not find?', options: ['The key to his flat', 'His bag', 'His hat', 'His glasses'], answer: 0, type: 'literal' },
      { q: 'How did they find out it was the right key?', options: ['It fit the lock', 'The man remembered it', 'It had his name', 'The ranger said so'], answer: 0, type: 'literal' },
      { q: 'What does "a kind act is never too small" mean?', options: ['Even small kind acts make a big difference', 'Big acts are the only ones that matter', 'Keys are small things', 'Giri was a small boy'], answer: 0, type: 'inferential' },
    ],
    vocab: [
      { word: 'joy', meaning: 'A feeling of great happiness', icon: '😊' },
      { word: 'fit', meaning: 'To be the right size and shape to go into something', icon: '🔑' },
      { word: 'pride', meaning: 'A feeling of happiness about something good you have done', icon: '🌟' },
      { word: 'act', meaning: 'Something that is done; an action', icon: '✨' },
      { word: 'clear', meaning: 'Easy to see or understand; not blurry', icon: '👁️' },
    ],
    grammarSpotlight: [
      { pattern: 'Diphthongs (oi, oy)', example: '"joy", "boy", "shout of joy"', tip: '"Oi" and "oy" make the same sound: coin, boy, join, joy.' },
      { pattern: 'Moral statements (theme)', example: '"A kind act is never too small."', tip: 'The last line tells us the lesson of the story. This is the theme or moral.' },
    ],
  },

  // ╔══════════════════════════════════════════════════════════════════════════╗
  // ║  CHAPTER STORIES — The Night Market  (2 chapters)                     ║
  // ║  Bridge-level chapter readers, category: 'chapter'                    ║
  // ╚══════════════════════════════════════════════════════════════════════════╝

  {
    id: 'chapter-night-market-1',
    band: 'D', level: 4, phase: 'chapter',
    title: 'The Night Market — Chapter 1',
    emoji: '🏮',
    mascotState: 'thinking',
    illustration: 'giri_chapter_night-market-1.jpg',
    category: 'chapter',
    chapterOf: 'chapter-night-market',
    chapterNum: 1,
    targetGraphemes: ['igh', 'ur', 'ou'],
    allowedHFWTier: 3,
    targetWordCount: 100,
    actualWordCount: 92,
    decodableRatio: 1.00,
    textType: 'chapter-reader',
    refrainCount: 0,
    lines: [
      { type: 'chapter', text: 'Chapter 1: Lights in the Dark' },
      { type: 'paragraph', text: "One evening, Giri's street began to change. Vans pulled up, and stalls grew along the road like mushrooms after rain. By sunset, strings of little lights hung between the lamp posts. The night market had come to town." },
      { type: 'paragraph', text: 'Giri walked through the market with wide eyes. Stalls sold cold drinks, warm kueh, and sizzling snacks. Smoke curled up from a satay grill, and somewhere a bell rang again and again.' },
      { type: 'end',       text: 'Then, near the last stall, Giri heard a sound that made him stop and turn. Somewhere close by, someone small was crying.' },
    ],
    comprehension: [
      { q: 'What came to Giri\'s street one evening?', options: ['The night market', 'A circus', 'A storm', 'A new bus stop'], answer: 0, type: 'literal' },
      { q: 'What hung between the lamp posts by sunset?', options: ['Strings of little lights', 'Long flags', 'Paper kites', 'Wet clothes'], answer: 0, type: 'literal' },
      { q: 'What do you think Giri will do next?', options: ['Go and find out who is crying', 'Go home to sleep', 'Buy a snack', 'Ring the bell'], answer: 0, type: 'inferential' },
    ],
    vocab: [
      { word: 'stalls', meaning: 'Small open shops at a market', icon: '🛖' },
      { word: 'sunset', meaning: 'The time when the sun goes down', icon: '🌇' },
      { word: 'sizzling', meaning: 'Making a hissing sound from hot cooking', icon: '🍳' },
      { word: 'satay', meaning: 'Grilled meat on small sticks', icon: '🍢' },
      { word: 'curled', meaning: 'Moved in a twisting, rolling shape', icon: '💨' },
    ],
    grammarSpotlight: [
      { pattern: 'Similes with "like"', example: 'Stalls grew along the road like mushrooms after rain.', tip: 'A simile compares two things using "like" to paint a picture.' },
      { pattern: 'Cliffhanger endings', example: 'Somewhere close by, someone small was crying.', tip: 'A chapter can end on a mystery so you want to read the next one!' },
    ],
  },

  {
    id: 'chapter-night-market-2',
    band: 'D', level: 4, phase: 'chapter',
    title: 'The Night Market — Chapter 2',
    emoji: '🏮',
    mascotState: 'encourage',
    illustration: 'giri_chapter_night-market-2.jpg',
    category: 'chapter',
    chapterOf: 'chapter-night-market',
    chapterNum: 2,
    targetGraphemes: ['ir', 'ou', 'ow'],
    allowedHFWTier: 3,
    targetWordCount: 140,
    actualWordCount: 146,
    decodableRatio: 1.00,
    textType: 'chapter-reader',
    refrainCount: 0,
    lines: [
      { type: 'chapter', text: 'Chapter 2: The Lost Girl' },
      { type: 'paragraph', text: 'Behind the last stall stood a small girl with tears on her cheeks. She had lost her father in the crowd. "I turned round and he was gone," she sobbed. "I cannot find him."' },
      { type: 'paragraph', text: 'Giri remembered being lost once, and how scary it had felt. "Hold my hand," he said. "We will find him together." They walked to the satay man, who lifted the girl up high and called out in a booming voice, "Whose girl is this?"' },
      { type: 'paragraph', text: 'A man burst out of the crowd with wide, worried eyes. He hugged the girl tight and shook hands with Giri again and again. As they rode away on the bus, the girl waved at Giri through the window.' },
      { type: 'end',       text: 'Giri walked home under the strings of lights. The market was loud and bright, but the best part of the night was the small hand that had held his.' },
    ],
    comprehension: [
      { q: 'Why was the girl crying?', options: ['She had lost her father in the crowd', 'She dropped her snack', 'She was scared of the dark', 'She hurt her knee'], answer: 0, type: 'literal' },
      { q: 'Who helped call out to the crowd?', options: ['The satay man', 'A police officer', 'Mrs Tan', 'The bus driver'], answer: 0, type: 'literal' },
      { q: 'Why did Giri know how the girl felt?', options: ['He remembered being lost once himself', 'He read about it', 'The satay man told him', 'He was lost right then too'], answer: 0, type: 'inferential' },
    ],
    vocab: [
      { word: 'sobbed', meaning: 'Cried with big, shaking breaths', icon: '😢' },
      { word: 'booming', meaning: 'Very loud and deep', icon: '📢' },
      { word: 'burst', meaning: 'Came out suddenly and fast', icon: '💥' },
      { word: 'worried', meaning: 'Feeling afraid something bad happened', icon: '😟' },
      { word: 'waved', meaning: 'Moved a hand to say hello or goodbye', icon: '👋' },
    ],
    grammarSpotlight: [
      { pattern: 'Direct speech in a story', example: '"Hold my hand," he said. "We will find him together."', tip: 'Speech marks show the exact words a character says.' },
      { pattern: 'Endings that echo the title', example: 'The best part of the night was the small hand that had held his.', tip: 'A strong last line ties the whole chapter together.' },
    ],
  },

];
