...
export function validateClozePassage(p, ctx = {}) {
  ...
  if (Array.isArray(p.wordBank) && Array.isArray(p.answers)) {
    const normalizedBank = p.wordBank.map(v => String(v).trim().toLowerCase());
    const normalizedAnswers = p.answers.map(v => String(v).trim().toLowerCase());
    ...
    if (ctx.strictWordBankQuality) {
      if (new Set(normalizedBank).size !== normalizedBank.length) {
        issues.push(`${tag}: duplicate wordBank entries after case/space normalization`);
      }

      if (p.wordBank.length <= p.answers.length) {
        issues.push(`${tag}: wordBank must include distractors beyond answers`);
      }

      const distractors = normalizedBank.filter(word => !normalizedAnswers.includes(word));
      if (!distractors.length) {
        issues.push(`${tag}: wordBank has no valid distractor after answer normalization`);
      }
    }
  }
