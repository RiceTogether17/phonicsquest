...
describe('MCQ difficulty and context metadata', () => {
  it('tags multi-sentence contexts from question text', () => {
    expect(inferQuestionContextType('Rina had never visited the museum before. She saw ___ vase.')).toBe('multi');
  ...
  it('writes context metadata to generated MCQ items', () => {
    ...
    expect(items.every(item => item.contextType === 'single' || item.contextType === 'multi')).toBe(true);
