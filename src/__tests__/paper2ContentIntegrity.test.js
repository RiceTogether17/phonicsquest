...
const BANNED_INTERNAL_LABEL =
  /(practice set|p1 practice|p2 practice|p3 practice|p4 practice|p5 practice|p6 practice|set 88)/i;
...
describe('Paper 2 content integrity — Grammar MCQ', () => {
  ...
  it(`has at least ${MIN_GRAMMAR_MCQ_PER_LEVEL} items at every level`, () => {
    for (const level of GRAMMAR_MCQ_LEVELS) {
      ...
      expect(count, `${level} has only ${count} items`).toBeGreaterThanOrEqual(
        MIN_GRAMMAR_MCQ_PER_LEVEL,
      );
    }
  });
  ...
  it(`covers at least ${MIN_GRAMMAR_MCQ_CATEGORIES_PER_LEVEL} distinct categories per level`, () => {
    ...
    for (const level of GRAMMAR_MCQ_LEVELS) {
      ...
      expect(cats.length, `${level} only covers ${cats.join(',')}`).toBeGreaterThanOrEqual(
        MIN_GRAMMAR_MCQ_CATEGORIES_PER_LEVEL,
      );
    }
  });
  ...
  it('only uses grammar categories from GRAMMAR_CATEGORIES', () => {
    for (const level of GRAMMAR_MCQ_LEVELS) {
      for (const item of GRAMMAR_MCQ_ITEMS[level] || []) {
        expect(
          GRAMMAR_CATEGORY_KEYS.has(item.category),
          `${item.id} uses unknown grammar category ${item.category}`,
        ).toBe(true);
      }
    }
  });
  ...
  it('contains no internal generation labels in grammar MCQ stems', () => {
    for (const level of GRAMMAR_MCQ_LEVELS) {
      for (const item of GRAMMAR_MCQ_ITEMS[level] || []) {
        expect(
          BANNED_INTERNAL_LABEL.test(String(item.q || '')),
          `${item.id} contains banned internal label text`,
        ).toBe(false);
      }
    }
  });
...
describe('Paper 2 content integrity — Vocabulary MCQ', () => {
  ...
  it(`covers at least ${MIN_VOCAB_MCQ_CATEGORIES_PER_LEVEL} distinct categories per level`, () => {
    ...
    for (const level of VOCAB_MCQ_LEVELS) {
      const cats = Object.keys(counts[level] || {});
      expect(cats.length, `${level} only covers ${cats.join(',')}`).toBeGreaterThanOrEqual(
        MIN_VOCAB_MCQ_CATEGORIES_PER_LEVEL,
      );
    }
  });
  ...
  it('only uses vocabulary categories from VOCAB_CATEGORIES', () => {
    for (const level of VOCAB_MCQ_LEVELS) {
      for (const item of VOCAB_MCQ_ITEMS[level] || []) {
        expect(
          VOCAB_CATEGORY_KEYS.has(item.category),
          `${item.id} uses unknown vocab category ${item.category}`,
        ).toBe(true);
      }
    }
  });
  ...
  it('connectorClue items focus on meaning inference, not connector insertion', () => {
    const insertionPattern =
      /\\b(and|but|so|because|although|however|therefore|unless)\\b\\s*___|___\\s*\\b(and|but|so|because|although|however|therefore|unless)\\b/i;
    for (const level of VOCAB_MCQ_LEVELS) {
      const items = (VOCAB_MCQ_ITEMS[level] || []).filter(
        (item) => item.category === 'connectorClue',
      );
      for (const item of items) {
        expect(
          insertionPattern.test(item.q),
          `${item.id} appears to test connector insertion instead of vocabulary inference`,
        ).toBe(false);
      }
    }
  });
...
describe('Paper 2 content integrity — Grammar Cloze passages', () => {
  ...
  it(`has at least ${MIN_GRAMMAR_PASSAGES_PER_LEVEL} passages per level`, () => {
    for (const level of Object.keys(passages)) {
      const total = Object.values(passages[level]).reduce(
        (sum, arr) => sum + (arr?.length || 0),
        0,
      );
      expect(total, `${level} only has ${total} passages`).toBeGreaterThanOrEqual(
        MIN_GRAMMAR_PASSAGES_PER_LEVEL,
      );
    }
  });
  ...
  it('contains no internal generation labels in grammar cloze titles/text', () => {
    for (const [level, cats] of Object.entries(passages)) {
      for (const [cat, arr] of Object.entries(cats || {})) {
        for (const p of arr || []) {
          expect(
            BANNED_INTERNAL_LABEL.test(String(p.title || '')),
            `${p.id} (${level}/${cat}) has banned label in title`,
          ).toBe(false);
          expect(
            BANNED_INTERNAL_LABEL.test(String(p.text || '')),
            `${p.id} (${level}/${cat}) has banned label in text`,
          ).toBe(false);
        }
      }
    }
...
describe('Paper 2 content integrity — Vocabulary Cloze passages', () => {
  ...
  it('every category in vocabPassages is registered in VOCAB_CATEGORIES', () => {
    for (const key of Object.keys(vocabPassages)) {
      expect(
        ...
        `"${key}" in vocabPassages but not in VOCAB_CATEGORIES`,
      ).toBeTruthy();
    }
  });
  ...
  it('all cloze-eligible vocab categories have p1-p6 coverage with at least 12 passages per level', () => {
    ...
    for (const category of Object.keys(VOCAB_CATEGORIES)) {
      ...
      for (const lv of levels) {
        ...
        expect(arr.length, `${category}/${lv} has ${arr.length} passages`).toBeGreaterThanOrEqual(
          12,
        );
      }
    }
  ...
  it('every vocab passage has complete hints, clues and real definitions for wordBank words', () => {
    const badDefinition =
      /(a useful word|matches the context|definition unavailable|placeholder|todo|tbd|n\/a)/i;
    const genericHint = /(choose the best word|read carefully|fill in the blank)/i;
    const genericPrompt =
      /(which word or phrase helps you choose the answer|best clue for this blank)/i;
      ...
      for (const [lv, arr] of Object.entries(levels || {})) {
        for (const p of arr || []) {
          ...
          for (const hint of p.hints || []) {
            expect(genericHint.test(String(hint || '')), `${p.id} has generic hint text`).toBe(
              false,
            );
          }
          ...
          for (const clue of p.clues || []) {
            expect(
              genericPrompt.test(String(clue?.prompt || '')),
              `${p.id} has generic clue prompt`,
            ).toBe(false);
          }
          expect(
            p.definitions && typeof p.definitions === 'object',
            `${p.id} missing definitions`,
          ).toBe(true);
          for (const wb of p.wordBank || []) {
            const def = p.definitions[wb];
            expect(
              typeof def === 'string' && def.trim().length > 0,
              `${p.id} missing definition for ${wb}`,
            ).toBe(true);
            expect(
              badDefinition.test(def.trim()),
              `${p.id} has placeholder definition for ${wb}`,
            ).toBe(false);
            expect(def.trim().toLowerCase(), `${p.id} definition repeats word for ${wb}`).not.toBe(
              String(wb).trim().toLowerCase(),
            );
          }
        }
      }
    ...
    for (const [category, levels] of Object.entries(vocabPassages)) {
      for (const [lv, arr] of Object.entries(levels || {})) {
        for (const p of arr || []) {
          expect(
            BANNED_INTERNAL_LABEL.test(String(p.title || '')),
            `${p.id} (${category}/${lv}) has banned label in title`,
          ).toBe(false);
          expect(
            BANNED_INTERNAL_LABEL.test(String(p.text || '')),
            `${p.id} (${category}/${lv}) has banned label in text`,
          ).toBe(false);
        }
      }
    }
  ...
  it('prints current category-level coverage for audit visibility', () => {
    ...
    const grammarClozeCounts = Object.fromEntries(
      Object.entries(passages).map(([level, cats]) => [
        level,
        Object.fromEntries(
          Object.entries(cats || {}).map(([cat, arr]) => [cat, (arr || []).length]),
        ),
      ]),
    );
...
describe('Paper 2 validator guardrails — malformed MCQ detection', () => {
  it('flags duplicate options even when case/spacing differ', () => {
    ...
    expect(issues.some((i) => i.includes('duplicate choices after case/space normalization'))).toBe(
      true,
    );
  });
...
describe('Paper 2 validator guardrails — strict cloze quality checks', () => {
  it('fails strict mode when word banks do not provide enough distractors', () => {
    ...
    expect(
      issues.some((i) => i.includes('strict quality requires at least 2 distractors in wordBank')),
    ).toBe(true);
    expect(
      issues.some((i) =>
        i.includes('strict quality requires at least 2 non-answer distractors in wordBank'),
      ),
    ).toBe(true);
  });
  ...
  it('flags duplicate word-bank entries after normalization in strict mode', () => {
    ...
    expect(
      issues.some((i) =>
        i.includes('wordBank has duplicate entries after case/space normalization'),
      ),
    ).toBe(true);
  });
});
