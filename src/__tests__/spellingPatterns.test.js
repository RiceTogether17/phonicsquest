...
describe('deriveSpellingPattern()', () => {
  it.each([
    ...
    ['sail', 'ai'],
    ['day', 'ay'],
    ['play', null /* play is in 'blends' group, not long-a */],
    // Long E
    ['tree', 'ee'],
    ['feet', 'ee'],
    ['bee', 'ee'],
  ...
  it('every long-* and diphthongs word has a non-null spellingPattern', () => {
    const offenders = WORDS.filter(
      (w) => (w.group?.startsWith('long-') || w.group === 'diphthongs') && !w.spellingPattern,
    ).map((w) => w.id);
...
describe('long-vowel / diphthong micro-stage curriculum coverage', () => {
  ...
  it('the old macro stages (long-a, long-e, …, diphthongs) no longer exist', () => {
    for (const oldId of ['long-a', 'long-e', 'long-i', 'long-o', 'long-u', 'diphthongs']) {
      ...
      expect(
        stage,
        `old macro stage "${oldId}" should have been replaced by micro-stages`,
      ).toBeUndefined();
    }
  });
  ...
  it('every micro-stage has at least one word backing it', () => {
    for (const id of microIds) {
      ...
      expect(
        words.length,
        `micro-stage "${id}" has zero words — empty stages block the unlock chain`,
      ).toBeGreaterThan(0);
    }
  });
  ...
  it('each micro-stage word filter only includes its own pattern', () => {
    for (const id of microIds) {
      ...
      for (const w of words) {
        expect(w.spellingPattern, `${w.id} in ${id} should have pattern "${pattern}"`).toBe(
          pattern,
        );
      }
    }
