...
import { buildCopySummaryText, getModeConfig, getSummaryScoreLine, buildWrongAnswerLines } from '../modes/clozeSessionSummary.js';
import { showAnswerReviewPanel } from '../modes/clozeReviewPanel.js';
import { incrementHintUsage } from '../modes/hintUsage.js';
import { resetWordVaultSessionState } from '../modes/wordVaultSessionState.js';
...
describe('copy summary', () => {
  ...
  it('builds wrong-answer lines from multiple passage rows (not just latest)', () => {
    const lines = buildWrongAnswerLines([
      { passageTitle: 'Passage 1', blank: '#2', studentAnswer: 'since', correctAnswer: 'although' },
      { passageTitle: 'Passage 2', blank: '#1', studentAnswer: 'despite', correctAnswer: 'because' },
    ]);
    expect(lines).toHaveLength(2);
    expect(lines[0]).toContain('Passage 1');
    expect(lines[1]).toContain('Passage 2');
  });
});
...
describe('word vault session reset', () => {
  it('resets blank counters to 0/0 for new session', () => {
    const reset = resetWordVaultSessionState();
    expect(reset.sessionBlankCorrect).toBe(0);
    expect(reset.sessionBlankTotal).toBe(0);
    expect(reset.sessionReviewRows).toEqual([]);
  });
});
