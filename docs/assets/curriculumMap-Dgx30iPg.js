import{b as h,P as d,E as m,F as g,C as v,H as $,s as b}from"./index-CgdeZYZv.js";const p=g.map(a=>({phase:a.phase,label:`Phase ${a.phase}`,title:a.title,icon:a.icon,desc:a.description,sightWords:a.sightWords||[]})),f=[{id:"sentenceSkills",label:"Sentence Forge",icon:"🔨",desc:"Build and arrange sentences"},{id:"grammarCloze",label:"Cloze Castle",icon:"🏰",desc:"Grammar in context"},{id:"vocabCloze",label:"Word Vault",icon:"🔑",desc:"Vocabulary in context"},{id:"editingQuest",label:"Editing Quest",icon:"✏️",desc:"Spot and fix errors"},{id:"writingQuest",label:"Writing Quest",icon:"📝",desc:"Extended writing tasks"}];function y(a){const e=b.get("groupMastery")||{};return typeof e[a]=="number"?e[a]:null}function u(a){const e=v.filter(r=>r.phase===a);if(!e.length)return{status:"locked",pct:0,masteredCount:0,total:0};let s=0,t=0;for(const r of e){const l=y(r.group);l!==null&&(l>=.8?s++:l>0&&t++)}const c=e.length,n=Math.round(s/c*100);let i;return s===c?i="complete":s>0||t>0?i="in-progress":i="locked",{status:i,pct:n,masteredCount:s,total:c}}function P(){for(let a=p.length;a>=1;a--){const{status:e}=u(a);if(e==="in-progress"||e==="complete")return a}for(const a of d)if(m(a.phase).status!=="complete")return a.phase;return 1}function C(a,e){return d.map(s=>{const{status:t,pct:c}=m(s.phase),n=s.phase===a,i={complete:"Complete ✅","in-progress":"In progress",locked:"Not started yet"}[t]||"";return`
      ...
      </div>`}).join("")}function k(a){const s=$().find(n=>n.id===a);if(!s||s.composite===null)return{status:"not-started",pct:0};const t=Math.round((s.composite??0)*100);return{status:t>=80?"strong":t>=50?"building":t>0?"starting":"not-started",pct:t}}function E(a="🦁"){const e=P();return C(e,a)+p.map(s=>{const{status:t,pct:c,masteredCount:n,total:i}=u(s.phase),r=s.phase===e&&t!=="complete",l={complete:"Complete ✅","in-progress":"In progress",locked:"Not started yet"}[t]||"";return`
      ...
      </div>`}).join("")}function S(){return f.map(a=>{const{status:e,pct:s}=k(a.id),t=`cm-domain--${e}`,c={strong:"Strong ✅",building:"Building",starting:"Just started","not-started":"Not started yet"}[e]||"";return`
      ...
      </div>`}).join("")}function w(a,{onClose:e}={}){var l,o;if(!a)return;const s=h(),t=(s==null?void 0:s.schoolLevel)==="primary",c=(s==null?void 0:s.avatar)||"🦁",n=((l=s==null?void 0:s.name)==null?void 0:l.split(" ")[0])||"Learner",i=E(c),r=S();a.innerHTML=`
    ...
    </div>`,(o=document.getElementById("cm-close-btn"))==null||o.addEventListener("click",()=>e==null?void 0:e())}export{S as a,E as b,w as r};
