import{p as M,C as O,a as pe}from"./passages-DNXaSraU.js";import{s as v,p as ge,q as fe,G as A,x as g,y as Be,z as be,u as Ae,t as G,v as ve,w as Oe}from"./index-Bku_Opw4.js";import{g as Ie,a as je,b as Me,c as J,s as Ge,d as Ue,e as U,f as Ve,h as Ze,p as Je,i as Qe,j as Xe,k as Ke,l as Te,r as Ye,m as et,n as tt,o as Re,q as Pe,t as lt,u as nt,v as ct,w as at,x as st,y as re,z as ot,A as We,B as rt,C as it,D as he,E as $e,F as dt,G as He,H as ut,I as mt,J as De,K as pt}from"./masteryMap-CeG1VHu1.js";import"./gsap-C8pce-KX.js";let p=null,T=null,z="P1",d="",I=0,le=[],L=[],E=[],ne=0,ce=0,ye=0,q=null,x=-1,R={},V=0,P=!1,ie=0,ke=0,w="practice",j=0,xe=0,Z=[],W=0,F=0,ze=!1,Y=!1,de=0,te=0,D={};function Lt(e,l){p=e,T=l}function Tt(){Ee()}function Ce(){p&&(p.innerHTML=""),L=[],E=[],q&&(document.removeEventListener("keydown",q),q=null)}function Ee(){if(!p)return;const e=v.get("ccqCompleted")||{},l=Object.keys(M);let t='<div class="cloze-browser">';t+='<div class="cloze-browser-grid">';for(const c of l){const n=Object.keys(M[c]),a=n.reduce((u,m)=>u+M[c][m].length,0),i=Ie({level:c,ccqCompletedByPassage:v.get("ccqCompletedByPassage")||{},ccqCompleted:e}),f=i>=a,y=pe[c];t+=`
      <button class="cloze-level-btn ${f?"cloze-level-btn--done":""}"
              data-level="${c}" aria-label="${O[c]}">
        <span class="cloze-level-icon">${f?"⭐":y}</span>
        <span class="cloze-level-name">${O[c]}</span>
        <span class="cloze-level-count">${n.length} topics · ${Math.min(i,a)} / ${a} done</span>
      </button>`}t+="</div></div>",p.innerHTML=t,p.querySelectorAll(".cloze-level-btn").forEach(c=>{c.addEventListener("click",()=>{z=c.dataset.level,ue(z)})})}function ue(e){var r,s,C,h,b;if(!p)return;const l=Object.keys(M[e]),t=v.get("ccqCatCompleted")||{},c=pe[e];let n='<div class="cloze-browser">';n+=`<div class="cloze-cat-header">
    ...
    </div>`,ae(e),Q(e),a&&ft(e),(u=document.getElementById("cloze-clear"))==null||u.addEventListener("click",()=>{Re(L,E),ae(e),Q(e)}),(m=document.getElementById("cloze-listen"))==null||m.addEventListener("click",()=>{let h=e.text;for(const b of e.answers)h=h.replace("___",b);G.speakWord(h)}),(r=document.getElementById("cloze-check"))==null||r.addEventListener("click",()=>yt(e)),(s=document.getElementById("cloze-rule-hint"))==null||s.addEventListener("click",()=>{const h=document.getElementById("cloze-rule-hint"),b=document.getElementById("cloze-rule-hint-panel");if(!h||!b)return;const o=b.hidden;if(b.hidden=!o,h.setAttribute("aria-expanded",String(o)),h.textContent=o?"💡 Hide Rule":"💡 Show Rule",o){const $=d!=="__all__"?d:null,S=$?be($):{rule:"Read each sentence and look for grammar clues about which word fits best.",example:"The words around each blank — tense, pronouns, singular/plural — point to the answer.",tip:"Check tense markers, subject–verb agreement, and pronoun reference."};b.innerHTML=`
        <p class="mcq-hint-rule"><strong>Rule:</strong> ${g(S.rule)}</p>
        <p class="mcq-hint-eg"><em>${g(S.example)}</em></p>
        <p class="mcq-hint-tip">${g(S.tip)}</p>`}}),(C=document.getElementById("cloze-quit"))==null||C.addEventListener("click",()=>{Ce(),T==null||T()}),q&&document.removeEventListener("keydown",q),q=h=>{var b;h.key==="Enter"&&!P&&(h.preventDefault(),(b=document.getElementById("cloze-check"))==null||b.click()),h.key==="Escape"&&(Ce(),T==null||T())},document.addEventListener("keydown",q)}function kt(e){if(p){if(ze){qe(e);return}Ye({host:p,quest:"cloze",passageTitle:e.title||"Cloze Castle",passageText:e.text||"",onContinue:()=>{ze=!0,qe(e)},onQuit:()=>{Ce(),T==null||T()}})}}function qe(e){if(!p)return;if(Y||w==="exam"){ee(e);return}const l=et(e);if(!l){Y=!0,ee(e);return}p.innerHTML=`
    ...
    </div>`;const t=document.getElementById("cloze-scan-host");tt({host:t,attention:l,onContinue:()=>{Y=!0,ee(e)},onSkip:()=>{Y=!0,ee(e)}})}function ht(){const e=Object.values(R||{}).map($e);if(!e.length)return"☆ ☆ ☆";const l=e.reduce((c,n)=>c+n,0)/e.length,t=l>=.85?3:l>=.45?2:1;return`${"★".repeat(t)}${"☆".repeat(3-t)}`}function gt(e){const l=_e(e);if(!l)return"";const t=U(w);return`
    <div class="clue-hunt-panel" id="clue-hunt-panel">
      <div class="clue-hunt-header">
        ...
        <span class="clue-hunt-sub">Blank ${x+1} · Clue Score ${ht()}</span>
      </div>
    ...
    </div>`,(a=p.querySelector(".cloze-game"))==null||a.appendChild(n),(i=document.getElementById("clue-explanation-next"))==null||i.addEventListener("click",()=>{n.remove(),l()})}function H(e,l){const t=document.getElementById("cloze-feedback");t&&(t.textContent=e,t.className=`cloze-feedback cloze-feedback--${l?"success":"error"}`,t.hidden=!1,l&&setTimeout(()=>{t.hidden=!0},1600))}function $t(){var o,$,S,X,K;if(!p)return;const e=pe[z];Ae(),G.playSfx("levelUp"),ve.celebrate(!0);const l=d!=="__all__"&&A[d]?`${A[d].icon} ${A[d].label}`:"All Topics",t=W>0?Math.round(F/W*100):100,c=t>=90?3:t>=70?2:1,n=U(w),a=Math.max(1,Math.round((Date.now()-(xe||Date.now()))/1e3)),i=Ke({accuracy:t,skillLabel:l,hintsUsed:j}),f=Me({level:z,weakSkillsMap:v.get("ccqWeakSkills")||{}}),y=f.length?`<p class="cloze-complete-score">Mastery focus: ${f.map(k=>`${J(k.skill)} (${k.wrong}/${k.attempts})`).join(" · ")}</p>`:"",u=w==="exam"&&Z.length?Ve(Z):Z.map(k=>`- ${k.passageTitle} ${k.blank}: ${k.studentAnswer||"(blank)"} → ${k.correctAnswer}`),m=Object.keys(R).length,r=m>0?Math.round(ke/m*100):null,s=r!==null?`<p class="cloze-complete-clue">🔍 Clue accuracy: ${r}%</p>`:"",C=te>0?Math.round(de/te*100):null,h=C!==null?`<p class="cloze-complete-clue">🔎 Scan accuracy: ${C}% (${de}/${te})</p>`:"";let b="";if(t<70){let k=d!=="__all__"?d:null;if(!k){const _=Object.entries(D).filter(([,B])=>B.total>0);_.length>0&&(k=_.sort(([,B],[,se])=>B.correct/B.total-se.correct/se.total)[0][0])}if(k){const _=be(k),B=A[k]||{icon:"🏰",label:k};b=`
