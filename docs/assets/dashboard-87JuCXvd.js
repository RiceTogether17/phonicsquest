...
`)}ne.register(...Ee);let O=null,D=null;function Wt(e,s={}){D=s.onNavigate||null;const t=I.getOverallStats();e.innerHTML=`
  ...
  `,mt(),ht(),ft(),$t(),kt(),bt(),wt(),St(),yt(),vt(),pt(),Ct(t),Lt(t),Rt(t),xt(t),Mt(),At(),Et()}function pt(){const e=document.getElementById("dash-curriculum-map");if(!e)return;e.innerHTML='<h3 class="dash-section-title">Learning Journey Map</h3><div id="dash-cm-inner"></div>';const s=document.getElementById("dash-cm-inner");s&&at(s)}function mt(){var w,k,f,R;const e=document.getElementById("dash-parent-report-card");if(!e)return;const s=T(),t=I.getOverallStats(),a=ge(),r=Te(),n=v.get("questMastery")||{},c=[];for(const[,g]of Object.entries(n))if(!(!g||typeof g!="object"))for(const[C,S]of Object.entries(g)){if(typeof S!="number")continue;const M=c.find(x=>x.skill===C);M?S>M.score&&(M.score=S):c.push({skill:C,score:S})}const i=c.filter(g=>g.score>=.75).sort((g,C)=>C.score-g.score).map(g=>({skill:g.skill,label:gt(g.skill),score:g.score})),d=(t.recentHistory||[]).filter(g=>g&&g.correct===!1).slice(0,5).map(g=>({word:g.wordId,mode:g.mode,when:xe(g.timestamp),correct:!1})),m={days:(a==null?void 0:a.weekDays)??0,words:(a==null?void 0:a.weekWords)??(t.totalAttempts||0),accuracy:t.overallAccuracy||0},p=v.get("wordStats")||{},o=Oe(p,N).map(g=>({word:g.item.word||g.id})),u=_e(p,N).map(g=>({word:g.item.word||g.id})),l=it({profile:s,weakSkills:r,strengths:i,recentMistakes:d,weekly:m,graduatingSoon:o,slippingRecently:u});e.innerHTML=`
  ...
  `}function vt(){const e=document.getElementById("dash-moe-section");if(!e)return;const s=Je();e.innerHTML=`
    ...
    <ul class="dash-pattern-list">
    ...
    </ul>`)}function Ct(e){const s=document.getElementById("chart-mastery");if(!s)return;O&&(O.destroy(),O=null);const t=oe.filter(c=>G[c]),a=t.map(c=>G[c].label),r=t.map(c=>Math.round((e.groupMastery[c]??0)*100)),n=t.map(c=>G[c].color);O=new ne(s,{type:"bar",data:{labels:a,datasets:[{label:"Mastery %",data:r,backgroundColor:n.map(c=>c+"80"),borderColor:n,borderWidth:2,borderRadius:6,barPercentage:.7}]},options:{responsive:!0,maintainAspectRatio:!1,plugins:{legend:{display:!1}},scales:{y:{beginAtZero:!0,max:100,ticks:{callback:c=>c+"%",font:{size:11}},grid:{display:!1}},x:{ticks:{font:{size:10},maxRotation:45},grid:{display:!1}}}}})}function Lt(e){const s=document.getElementById("mastery-bars");s&&(s.innerHTML=oe.map(t=>{const a=G[t];if(!a)return"";const r=Math.round((e.groupMastery[t]??0)*100);return`
      ...
      </div>`}).join(""))}function Rt(e){const s=document.getElementById("learning-path");if(!s)return;const t=Pe(De());s.innerHTML=re.map(a=>{const r=t.includes(a.id),n=a.groups??(a.group?[a.group]:[]),c=n.map(d=>e.groupMastery[d]??0),i=n.length?Math.round(c.reduce((d,m)=>d+m,0)/n.length*100):0;return`
