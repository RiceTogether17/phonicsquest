...
`)}ae.register(...Ce);let M=null,j=null;function Ot(e,t={}){j=t.onNavigate||null;const s=N.getOverallStats();e.innerHTML=`
  ...
  `,tt(),nt(),ct(),dt(),pt(),it(),lt(),ut(),mt(),rt(),ot(),et(),gt(s),ht(s),yt(s),vt(s),ft(),$t(),bt(),Je()}function I(e,t){e&&(t?e.setAttribute("data-dash-empty","true"):e.removeAttribute("data-dash-empty"))}function Ze(e){return e.getAttribute("data-dash-empty")==="true"?!0:e.children.length===0&&!e.textContent.trim()}function Je(){for(const e of document.querySelectorAll(".dash-group")){const t=e.querySelectorAll(".dash-group__body > div[id]");if(!t.length)continue;const s=Array.from(t).every(Ze);e.hidden=s}}function et(){const e=document.getElementById("dash-curriculum-map");if(!e)return;e.innerHTML='<h3 class="dash-section-title">Learning journey</h3><div id="dash-cm-inner"></div>';const t=document.getElementById("dash-cm-inner");t&&De(t)}function tt(){var S,C,$,L,x;const e=document.getElementById("dash-parent-report-card");if(!e)return;const t=W(),s=N.getOverallStats(),a=ue(),r=Ee(),n=v.get("questMastery")||{},o=[];for(const[,m]of Object.entries(n))if(!(!m||typeof m!="object"))for(const[k,_]of Object.entries(m)){if(typeof _!="number")continue;const E=o.find(w=>w.skill===k);E?_>E.score&&(E.score=_):o.push({skill:k,score:_})}const i=o.filter(m=>m.score>=.75).sort((m,k)=>k.score-m.score).map(m=>({skill:m.skill,label:at(m.skill),score:m.score})),d=(s.recentHistory||[]).filter(m=>m&&m.correct===!1).slice(0,5).map(m=>({word:m.wordId,mode:m.mode,when:Y(m.timestamp),correct:!1})),g={days:(a==null?void 0:a.weekDays)??0,words:(a==null?void 0:a.weekWords)??(s.totalAttempts||0),accuracy:s.overallAccuracy||0},u=v.get("wordStats")||{},c=Re(u,P).map(m=>({word:m.item.word||m.id})),p=Me(u,P).map(m=>({word:m.item.word||m.id})),l=Qe({profile:t,weakSkills:r,strengths:i,recentMistakes:d,weekly:g,graduatingSoon:c,slippingRecently:p});e.innerHTML=`
      ...
      <div class="parent-report-card__grid">
        ...
        <div class="parent-report-card__cell">
          ...
          <ul class="needs-practice-list">
            ${l.needsPractice.map(m=>`<li class="needs-practice-item needs-practice-item--${m.band||"amber"}"><span class="needs-practice-dot" aria-hidden="true"></span>${f(m.label)} <strong>${m.pct==null?"(not enough practice yet)":`(${m.pct}%)`}</strong>${st(m)}</li>`).join("")||"<li>Nothing has been measured yet — a few short sessions will show where things stand</li>"}
          </ul>
        </div>
    ...
    <div class="coaching-card ${s}" aria-label="Parent coaching summary">
      ...
      ${i}
      ${g}
    </div>`}function rt(){const e=document.getElementById("dash-reporting-section");if(!e)return;const t=ve().sort((u,c)=>u.accuracy-c.accuracy).slice(0,8),s=fe().sort((u,c)=>u.accuracy-c.accuracy).slice(0,8),a=Fe(),r=be(),n=we({days:7}),o=qe({limit:6});I(e,!t.length&&!s.length&&!a.length);const i=u=>u.map(c=>{const p=Math.round((c.accuracy||0)*100),l=Math.round((c.clueSuccess||0)*100);return`<div class="dash-category-row" title="${c.tooltip}">
  ...
  `}function ot(){const e=document.getElementById("dash-moe-section");if(!e)return;const t=We();I(e,!t.length),e.innerHTML=`
    ...
    ${c}`}function gt(e){const t=document.getElementById("chart-mastery");if(!t)return;M&&(M.destroy(),M=null);const s=re.filter(o=>D[o]),a=s.map(o=>D[o].label),r=s.map(o=>Math.round((e.groupMastery[o]??0)*100)),n=s.map(o=>D[o].color);M=new ae(t,{type:"bar",data:{labels:a,datasets:[{label:"Mastery %",data:r,backgroundColor:n.map(o=>o+"80"),borderColor:n,borderWidth:2,borderRadius:6,barPercentage:.7}]},options:{responsive:!0,maintainAspectRatio:!1,plugins:{legend:{display:!1}},scales:{y:{beginAtZero:!0,max:100,ticks:{callback:o=>o+"%",font:{size:11}},grid:{display:!1}},x:{ticks:{font:{size:10},maxRotation:45},grid:{display:!1}}}}})}function ht(e){const t=document.getElementById("mastery-bars");t&&(t.innerHTML=re.map(s=>{const a=D[s];if(!a)return"";const r=Math.round((e.groupMastery[s]??0)*100);return`
      ...
      </div>`}).join(""))}function yt(e){const t=document.getElementById("learning-path");if(!t)return;const s=Oe(je());t.innerHTML=Pe.map(a=>{const r=s.includes(a.id),n=a.groups??(a.group?[a.group]:[]),o=n.map(d=>e.groupMastery[d]??0),i=n.length?Math.round(o.reduce((d,g)=>d+g,0)/n.length*100):0;return`
      ...
      </div>`}).join("")}function vt(e){const t=document.getElementById("word-history-body");if(!t)return;const s=e.recentHistory.slice(0,30).map(a=>{const r=P.find(d=>d.id===a.wordId),n=(r==null?void 0:r.emoji)||"",o=Y(a.timestamp),i=a.correct?'<span style="color:var(--color-success)">✓</span>':'<span style="color:var(--color-error)">✗</span>';return`<tr>
      <td>${n} ${a.wordId}</td>
      <td>${a.mode}</td>
      <td>${i}</td>
      <td>${o}</td>
    </tr>`});t.innerHTML=s.join("")||'<tr><td colspan="4" style="text-align:center;color:var(--text-muted)">No history yet</td></tr>'}function ft(){const e=document.getElementById("badge-grid");if(!e)return;const t=z.getAll();e.setAttribute("aria-label",`${z.earnedCount} of ${z.totalCount} badges earned`),e.innerHTML=t.map(s=>`
...
`),p=new Blob([c],{type:"text/csv"}),l=URL.createObjectURL(p),y=document.createElement("a");y.href=l,y.download="phonicsquest-progress-anonymised.csv",y.click(),URL.revokeObjectURL(l)}),(o=document.getElementById("btn-export-report"))==null||o.addEventListener("click",()=>{const c=kt(),p=new Blob([JSON.stringify(c,null,2)],{type:"application/json"}),l=URL.createObjectURL(p),y=document.createElement("a");y.href=l,y.download=`phonicsquest-parent-report-${new Date().toISOString().slice(0,10)}.json`,y.click(),URL.revokeObjectURL(l)}),(i=document.getElementById("btn-import-csv"))==null||i.addEventListener("click",()=>{const c=document.getElementById("csv-import-panel");c&&(c.hidden=!c.hidden)}),(d=document.getElementById("csv-browse-btn"))==null||d.addEventListener("click",()=>{var c;(c=document.getElementById("csv-file-input"))==null||c.click()}),(g=document.getElementById("csv-file-input"))==null||g.addEventListener("change",c=>{var l;const p=(l=c.target.files)==null?void 0:l[0];p&&se(p)});const e=document.getElementById("csv-drop-zone");e&&(e.addEventListener("dragover",c=>{c.preventDefault(),e.classList.add("dash-import-drop--active")}),e.addEventListener("dragleave",()=>e.classList.remove("dash-import-drop--active")),e.addEventListener("drop",c=>{var l,y;c.preventDefault(),e.classList.remove("dash-import-drop--active");const p=(y=(l=c.dataTransfer)==null?void 0:l.files)==null?void 0:y[0];p&&se(p)}));const t=document.getElementById("adaptive-weak-weight"),s=document.getElementById("adaptive-unseen-weight"),a=v.get("adaptiveConfig")||{};t&&(t.value=String(a.weakWeight??5),t.addEventListener("input",()=>{v.set("adaptiveConfig",{...v.get("adaptiveConfig")||{},weakWeight:Number(t.value)})})),s&&(s.value=String(a.unseenWeight??3),s.addEventListener("input",()=>{v.set("adaptiveConfig",{...v.get("adaptiveConfig")||{},unseenWeight:Number(s.value)})})),(u=document.getElementById("btn-print-report"))==null||u.addEventListener("click",()=>{document.body.classList.add("print-mode"),window.print(),setTimeout(()=>document.body.classList.remove("print-mode"),2e3)})}function wt(e,t=Date.now()){if(!Array.isArray(e))return 0;const s=864e5,a=new Set;for(let n=0;n<7;n++)a.add(new Date(t-n*s).toISOString().slice(0,10));const r=new Set;for(const n of e)a.has(n==null?void 0:n.date)&&(n.xp||0)>0&&r.add(n.date);return r.size}function $t(){const e=document.getElementById("print-report-content");if(!e)return;const t=new Date().toLocaleDateString("en-SG",{day:"numeric",month:"long",year:"numeric"});function s(d){const g=d*100;return g>=70?"#22c55e":g>=40?"#f59e0b":"#ef4444"}function a(d,g,u){var p;const c=[];for(const l of g){const y=Ie.getSkillScore(d,l);if(y===.5)continue;const b=Math.round(y*100),h=s(y),S=((p=u[l])==null?void 0:p.label)||l,C=Math.max(b,4);c.push(`
        <div class="print-report-skill-row">
          ...
          <span>${f(S)}</span>
        </div>`)}return c.join("")||'<p style="font-size:12px;color:#6b7280">No attempts recorded yet.</p>'}const r=v.get("xp")??0,n=v.get("streak")??0,o=v.get("dailyGoal")??0,i=wt(v.get("weeklyXpLog"));e.innerHTML=`
    ...
    </div>`}function kt(){const e=N.getOverallStats(),t=W();return{generatedAt:new Date().toISOString(),learnerSummary:ce(),literacyDomains:ie(),clueInsights:le(),recommendedActions:de(),recentPatternInsights:pe(),progress:{wordsAttempted:e.wordsAttempted,wordsMastered:e.wordsMastered,overallAccuracy:e.overallAccuracy,bestStreak:e.bestStreak,totalAttempts:e.totalAttempts,totalCorrect:e.totalCorrect,groupMastery:e.groupMastery,recentHistory:e.recentHistory.slice(0,50),profile:t}}}const ke=new Set(["a","e","i","o","u"]),St=new Set("bcdfghjklmnpqrstvwxyz".split(""));function F(e){const s=e.toLowerCase().split(""),a=s.map(r=>ke.has(r)?"sv":(St.has(r),"c"));return{graphemes:s,types:a}}function ee(e){const t=e.toLowerCase().split("").find(s=>ke.has(s));return t?`short-${t}`:"short-a"}function te(e,t){const s=[],a=[];let r=!1;for(const i of t){if(i==="sv"||i==="lv"){r=!0;continue}r?a.push(i):s.push(i)}const n=s.length,o=a.length;return n<=1&&o<=1?"CVC":n>=2&&o<=1?"blend":n<=1&&o>=2?"CVCC":"CCVCC"}async function se(e){var g,u;const t=document.getElementById("csv-import-preview"),s=document.getElementById("csv-import-status");if(!t||!s)return;const r=(await e.text()).trim().split(`
`).filter(c=>c.trim());if(!r.length){s.hidden=!1,s.textContent="File is empty.",s.className="dash-import-status dash-import-status--error";return}const n=r[0].trim(),o=n.includes(","),i=[],d=new Set(P.map(c=>c.id));if(o){const c=n.toLowerCase().split(",").map(C=>C.trim()),p=c.indexOf("word");if(p<0){s.hidden=!1,s.textContent='CSV must have a "word" column.',s.className="dash-import-status dash-import-status--error";return}const l=c.indexOf("graphemes"),y=c.indexOf("types"),b=c.indexOf("group"),h=c.indexOf("level"),S=c.indexOf("emoji");for(let C=1;C<r.length;C++){const $=r[C].split(",").map(k=>k.trim()),L=$[p];if(!L||d.has(L.toLowerCase()))continue;const x=l>=0&&$[l]?$[l].split(/[|;]/):F(L).graphemes,m=y>=0&&$[y]?$[y].split(/[|;]/):F(L).types;i.push({id:L.toLowerCase(),word:L.toLowerCase(),graphemes:x,types:m,pattern:te(x,m),group:b>=0&&$[b]||ee(L),level:h>=0&&parseInt($[h])||1,emoji:S>=0&&$[S]||""})}}else for(const c of r){const p=c.trim().toLowerCase();if(!p||d.has(p))continue;const{graphemes:l,types:y}=F(p);i.push({id:p,word:p,graphemes:l,types:y,pattern:te(l,y),group:ee(p),level:1,emoji:""})}if(!i.length){s.hidden=!1,s.textContent="No new words found (all may already exist).",s.className="dash-import-status dash-import-status--error";return}t.hidden=!1,t.innerHTML=`
    ...
    <button class="btn btn--ghost btn--sm" id="csv-cancel-import">Cancel</button>`,(g=document.getElementById("csv-cancel-import"))==null||g.addEventListener("click",()=>{t.hidden=!0,s.hidden=!0}),(u=document.getElementById("csv-confirm-import"))==null||u.addEventListener("click",()=>{P.push(...i);const c=v.get("customWords")||[];v.set("customWords",[...c,...i]),t.hidden=!0,s.hidden=!1,s.textContent=`Imported ${i.length} word${i.length>1?"s":""} successfully!`,s.className="dash-import-status dash-import-status--success"})}function Y(e){if(!e)return"";const t=Date.now()-new Date(e).getTime(),s=Math.floor(t/6e4);if(s<1)return"just now";if(s<60)return`${s}m ago`;const a=Math.floor(s/60);return a<24?`${a}h ago`:`${Math.floor(a/24)}d ago`}function jt(){M&&(M.destroy(),M=null)}export{wt as countDaysPlayedThisWeek,jt as destroyDashboard,Ot as renderDashboard};
