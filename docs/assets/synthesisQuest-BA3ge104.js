import{S as U}from"./synthesisItems-BI7gkXfp.js";import{s as L,z as N,ap as V,ar as Z,au as J,a0 as ee,a7 as te,ac as se,ad as ne,av as ae}from"./index-CkuCUw9a.js";import{g as re}from"./shortAnswerGrader-BbVaplnA.js";import"./gsap-C8pce-KX.js";import"./stories-BYImWThp.js";const oe={P4:"Primary 4",P5:"Primary 5",P6:"Primary 6"},ie=8,O=2,y={connectorContrast:{icon:"⚡",rule:"Contrast connectors join two opposing ideas",structure:"Although/Even though + [clause], [main clause].",tip:'Never use "but" in the same sentence as "although" — they do the same job.'},connectorResult:{icon:"💥",rule:'"So…that" and "such…that" express cause and result',structure:"so + [adjective] + that  /  such + a/an + [adjective + noun] + that.",tip:'Use "so" before an adjective alone; use "such" when a noun follows.'},connectorAddition:{icon:"➕",rule:'"Not only…but also" links two facts and emphasises both',structure:"[Subject] not only [verb phrase 1] but also [verb phrase 2].",tip:'When "not only" starts the sentence, invert subject and auxiliary: "Not only did he…"'},connectorTime:{icon:"⏰",rule:'"As soon as" and "no sooner…than" show immediate sequence',structure:"As soon as [past simple], [past simple].  /  No sooner had [subject] [past participle] than [past simple].",tip:'"No sooner had…" uses past perfect. No comma is needed when "as soon as" falls in the middle.'},connectorCondition:{icon:"🔐",rule:'"Unless" means "if not"; "provided that" means "on the condition that"',structure:"[Main clause] unless [condition].  /  [Main clause] provided that [condition].",tip:'Never pair "unless" with "not" in the same clause — "unless" already contains the negative.'},activeToPassive:{icon:"🔄",rule:"Active → Passive shifts focus from the doer to the receiver",structure:"[Object] + was/were + past participle + by + [agent].",tip:'Choose was/were based on the new subject. Drop "by [agent]" if obvious or unimportant.'},passiveToActive:{icon:"🔁",rule:"Passive → Active: the agent becomes the subject, verb returns to active form",structure:"[Agent] + [active verb in correct tense] + [original object].",tip:'Remove "was/were…by" and reconstruct the verb in the matching tense.'},reportedSpeechStatement:{icon:"💬",rule:"Reported statements shift tense back and change pronouns and time words",structure:"[He/She] said (that) + [back-shifted verb clause].",tip:'"Yesterday" → "the day before"; "ago" → "before/earlier"; present → past; past → past perfect.'},reportedSpeechQuestion:{icon:"❓",rule:"Reported questions use statement word order — no inversion, no question mark",structure:"[He/She] asked [who/what/whether/if] + [subject] + [verb].",tip:'No auxiliary "do/does/did" after the question word. Use "if/whether" for yes/no questions.'},reportedSpeechCommand:{icon:"📢",rule:'Reported commands use "told/asked + object + to + base verb"',structure:"[He/She] told/asked [person] + (not) to + [base verb].",tip:'For negative commands: "told her not to…". Use "asked" for polite requests.'},relativeClause:{icon:"🔗",rule:"Relative clauses add information about a noun using who/which/whose/whom/that",structure:"[Main noun] + who/which/whose/that + [relative clause].",tip:'Non-defining clauses (extra info) use commas and cannot use "that". Defining clauses can use "that".'},comparison:{icon:"⚖️",rule:"Comparison: as…as (equal), not as…as (unequal), more/less…than (different degrees)",structure:"[Subject] is as [adjective] as [comparison].  /  [Subject] is more [adj] than [comparison].",tip:'"The more…the more" shows proportional increase. Both halves use the comparative form.'},advancedConstruction:{icon:"🏆",rule:"Fronted negative/emphatic elements require subject-verb inversion",structure:"[Never/Seldom/Not until] + [auxiliary] + [subject] + [main verb].",tip:'"It was not until…that" and fronted negatives (never/seldom/rarely) all trigger inversion.'},causativeHave:{icon:"🛠️",rule:'"Have something done" shows that someone arranged for another person to perform an action',structure:"[Subject] + have/had + [object] + past participle.",tip:"The subject does NOT do the action — they arranged for someone else to do it."},cleftSentence:{icon:"🎯",rule:"Cleft sentences (It was/is…who/that) split a sentence to emphasise one element",structure:"It was/is + [emphasised element] + who/that + [rest of sentence].",tip:'Use "who" for people, "that" for things or ideas. Match "was/is" to the original tense.'},conditionalType2:{icon:"🌀",rule:"Type 2 conditional: imaginary or unlikely present/future situation",structure:"If + [past simple], [would/could/might] + [base verb].",tip:'Use "were" instead of "was" after "if" in formal writing: "If I were you…"'},conditionalType3:{icon:"⏮️",rule:"Type 3 conditional: imaginary past — something that did NOT happen",structure:"If + [past perfect], [would/could/might] + have + [past participle].",tip:'"Would have" is most common; "could have" = possibility; "might have" = weaker probability.'},despiteInSpiteOf:{icon:"🧱",rule:'"Despite" and "in spite of" show contrast — followed by a noun or -ing form, NOT a full clause',structure:"Despite/In spite of + [noun phrase or -ing form], [main clause].",tip:'Never write "despite of…". When a full clause follows the contrast, use "although/even though" instead.'}};let d=null,C=null,E=null,A=[],v=0,k=0,b=null;function ce(){return{correct:0,total:0,firstTry:0,bySkill:{}}}function Ae(e,t){d=e,C=t}function le(){d&&de()}function ue(){d&&(d.innerHTML=""),d=null,C=null}function de(){var s;const e=["P4","P5","P6"],t=((s=L.get("questMastery"))==null?void 0:s.synthesisQuest)||{};d.innerHTML=`
    ...
    </div>`,d.querySelectorAll(".sq-level-btn").forEach(n=>{n.addEventListener("click",()=>G(n.dataset.level))})}function G(e){E=e,b=ce(),A=pe(e),v=0,x()}function pe(e){const s=U.filter(n=>n.level===e).map(n=>{const a=N.getSkillScore("synthesisQuest",n.skillKey)??.5,o=(Math.random()-.5)*.12;return{item:n,score:1-a+o}});return s.sort((n,a)=>a.score-n.score),s.slice(0,ie).map(n=>n.item)}function x(){var n,a,o;if(v>=A.length){ye();return}const e=A[v];k=0;const t=Math.round(v/A.length*100);d.innerHTML=`
    <div class="sq-game">
      <div class="sfq-header">
        <span class="sfq-badge">✏️ Synthesis — ${E}</span>
        <span class="sfq-progress">${v+1} / ${A.length}</span>
      </div>
    ...
    </div>`;const s=document.getElementById("sq-answer-input");(n=document.getElementById("sq-check"))==null||n.addEventListener("click",()=>Q(e)),s==null||s.addEventListener("keydown",r=>{r.key==="Enter"&&!r.shiftKey&&(r.preventDefault(),Q(e))}),(a=document.getElementById("sq-hint-btn"))==null||a.addEventListener("click",()=>qe(e)),(o=document.getElementById("sq-quit"))==null||o.addEventListener("click",()=>{ue(),C==null||C()}),s==null||s.focus()}function h(e){return e.toLowerCase().replace(/['']/g,"'").replace(/[""]/g,'"').replace(/\s+/g," ").replace(/[.,!?;:]+$/g,"").trim()}function he(e,t){if(!e)return null;const s=h(e);if(!h(t).startsWith(s))return null;const a=e.trim().length;return t.trim().slice(a).replace(/^[\s,]+/,"").trim()}function D(e){return e.allowAnyStructure?{requiredStart:null,requiredConnector:null}:{requiredStart:e.requiredStart??e.stem??null,requiredConnector:e.requiredConnector??null}}function fe(e,t){const{requiredStart:s,requiredConnector:n}=D(t),a=h(e||"");if(!a)return{ok:!0,violation:null,message:null};if(n&&!a.includes(h(n)))return{ok:!1,violation:"connector",message:`This one asks you to use "${n}".`};if(s){const o=h(s);if(a.startsWith(o))return{ok:!0,violation:null,message:null};if(W.some(i=>o===i||o.startsWith(`${i} `))){if(a.includes(` ${o} `))return{ok:!1,violation:"connector-position",message:`You used "${s}" correctly — but this task asks you to begin the sentence with it. Move it to the front and rewrite.`};if(W.some(p=>a.startsWith(`${p} `)&&!o.startsWith(p)))return{ok:!1,violation:"start",message:`Good sentence — but this task asks you to begin with "${s}". Try again starting there.`}}}return{ok:!0,violation:null,message:null}}const W=Object.freeze(["although","though","even though","even if","despite","in spite of","because","since","as soon as","unless","until","whereas","while","if","not only","no sooner","hardly","provided that","so that","in order to","after","before","when","whenever"]);function ve(e){const{requiredStart:t}=D(e);if(!t)return[];const s=h(t);return(e.alternates||[]).filter(n=>!h(n).startsWith(s))}function me(e){const t=new Set(ve(e).map(r=>h(r))),s=[e.answer,...e.alternates||[]].filter(Boolean).filter(r=>!t.has(h(r))),n=[];for(const r of s){const i=he(e.stem,r);i&&n.push(i)}const a=new Set,o=[];for(const r of[...n,...s]){const i=h(r);!i||a.has(i)||(a.add(i),o.push(r))}return o}function be(e,t){const s=me(t);return re(e,{expected:s[0]||"",accepts:s.slice(1),requiredGroups:Array.isArray(t.requiredGroups)?t.requiredGroups:null})}async function Q(e){var I,w,$;const t=document.getElementById("sq-answer-input"),s=((I=t==null?void 0:t.value)==null?void 0:I.trim())||"";if(!s){m("Type your rewritten sentence first.","neutral");return}k++;const n=fe(s,e);if(!n.ok){if(_(e,!1),k>=O){F(e,()=>{v++,x()},s);return}m(`◐ ${n.message}`,"hint"),t==null||t.focus(),t==null||t.select();return}const a=be(s,e),o=a.fraction>=1,r=a.fraction>0&&a.fraction<1;if(!o&&Z()){m("✨ Checking with AI…","hint");const l=document.getElementById("sq-check");l&&(l.disabled=!0);const g=e.alternates||[],u=await J(e.original,e.stem||"",e.answer,g,s,e.skill||e.skillKey);if(l&&(l.disabled=!1),(u==null?void 0:u.verdict)==="CORRECT"){_(e,!0,s);return}if((u==null?void 0:u.verdict)==="PARTIAL"){_(e,!1);const f=y[e.skillKey],q=f!=null&&f.structure?`Structure: ${f.structure}`:`Use the "${e.pattern||e.skill}" pattern.`,j=u.feedback?` ${u.feedback}`:"";m(`◐ Almost!${j} ${q}`,"hint"),t&&(t.value=""),t==null||t.focus();return}}if(o){_(e,!0,s);return}if(_(e,!1),k>=O){F(e,()=>{v++,x()},s);return}const i=y[e.skillKey],p=i!=null&&i.structure?`Structure: ${i.structure}`:`Use the "${e.pattern||e.skill}" pattern.`;if(r){const l=Math.round(a.fraction*100),g=((w=a.trace)==null?void 0:w.hits)||[],u=(($=a.trace)==null?void 0:$.misses)||[],f=g.length?` ✓ ${g.join(", ")}`:"",q=u.length?` ✗ still need: ${u.join(", ")}`:"";m(`◐ Partial (${l}% structure). ${p}${f}${q}`,"hint")}else{const l=Y(e,s);m(`Not yet. ${l.misconception.cue} ${p}`,"error")}t==null||t.focus(),t==null||t.select()}function ge(e){const t=String(e.stem||"").trim();return t?t.split(/\s+/).filter(Boolean):[]}function Y(e,t){return ae({given:t,model:e.answer,stem:e.original||"",requiredWords:ge(e),domain:"writing"})}function _(e,t,s){if(ee.playSfx(t?"correct":"wrong"),k===1){const n=b.bySkill[e.skillKey]||{correct:0,total:0,label:e.skill};n.total++,b.total++,N.recordAttempt({quest:"synthesisQuest",skill:e.skillKey,correct:t,level:E}),t&&(n.correct++,b.correct++,b.firstTry++),b.bySkill[e.skillKey]=n,N.updateSkill("synthesisQuest",e.skillKey,t)}t&&s!==void 0&&ke(e,s)}function qe(e){const t=y[e.skillKey];m(`💡 ${(t==null?void 0:t.tip)||`Begin your sentence with: "${e.stem||e.pattern}"`}`,"hint")}function m(e,t){const s=document.getElementById("sq-feedback");s&&(s.hidden=!1,s.className=`sfq-feedback${t==="error"?" sfq-feedback--error":t==="hint"?" sfq-feedback--hint":""}`,s.textContent=e)}function ke(e,t){var r;const s=d.querySelector(".sq-game");if(!s)return;const n=k===1?"⭐⭐":"⭐",a=document.createElement("div");a.className="sq-success-overlay";const o=e.stem?`${e.stem} ${t.trim()}`:t.trim();a.innerHTML=`
    ...
    </div>`,s.appendChild(a),a.querySelector(".sq-success-next").addEventListener("click",()=>{a.remove(),v++,x()})}function F(e,t,s=""){var i;const n=y[e.skillKey]||{icon:"📘",rule:e.skill,structure:"",tip:""},a=d.querySelector(".sq-game");if(!a){t();return}const o=s?Y(e,s):null;o&&te(o.id,{skill:e.skillKey,mode:"synthesisQuest"});const r=document.createElement("div");r.className="eq-teachback-overlay",r.setAttribute("role","dialog"),r.setAttribute("aria-modal","true"),r.setAttribute("aria-label","Grammar pattern explanation"),r.innerHTML=`
    <div class="eq-teachback-card">
      ...
      ${(i=e.alternates)!=null&&i.length?`<p class="eq-teachback-explanation">Also accepted: ${e.alternates.map(p=>`<em>${c(p)}</em>`).join("; ")}</p>`:""}
      ...
      </div>`}).join(""),l=w[0],g=l&&l[1].correct/Math.max(l[1].total,1)<.7?`Review the "${((q=(f=(u=y[l[0]])==null?void 0:u.rule)==null?void 0:f.split(":")[0])==null?void 0:q.trim())||l[0]}" pattern.`:"Great work — try the next level!";d.innerHTML=`
    <div class="sq-game">
    ...
    </div>`,(j=document.getElementById("sq-retry"))==null||j.addEventListener("click",()=>G(E)),(B=document.getElementById("sq-home"))==null||B.addEventListener("click",()=>le())}function c(e){return String(e??"").replace(/[<>&"']/g,t=>({"<":"&lt;",">":"&gt;","&":"&amp;",'"':"&quot;","'":"&#39;"})[t])}export{me as buildAcceptableAnswers,fe as checkTaskConstraints,ue as cleanupSynthesisQuest,D as getTaskConstraints,Ae as initSynthesisQuest,ve as offTaskAlternates,le as showSynthesisBrowser};
