import{S as F}from"./synthesisItems-BI7gkXfp.js";import{s as x,z as L,ap as Z,X as U,ar as J,au as ee,a0 as te,a7 as se,ac as ne,ad as ae,av as re}from"./index-iVVKU6aP.js";import{g as oe}from"./shortAnswerGrader-zj_e83SX.js";import"./gsap-C8pce-KX.js";import"./stories-BYImWThp.js";const ie={P4:"Primary 4",P5:"Primary 5",P6:"Primary 6"},ce=8,O=2,k={connectorContrast:{icon:"⚡",rule:"Contrast connectors join two opposing ideas",structure:"Although/Even though + [clause], [main clause].",tip:'Never use "but" in the same sentence as "although" — they do the same job.'},connectorResult:{icon:"💥",rule:'"So…that" and "such…that" express cause and result',structure:"so + [adjective] + that  /  such + a/an + [adjective + noun] + that.",tip:'Use "so" before an adjective alone; use "such" when a noun follows.'},connectorAddition:{icon:"➕",rule:'"Not only…but also" links two facts and emphasises both',structure:"[Subject] not only [verb phrase 1] but also [verb phrase 2].",tip:'When "not only" starts the sentence, invert subject and auxiliary: "Not only did he…"'},connectorTime:{icon:"⏰",rule:'"As soon as" and "no sooner…than" show immediate sequence',structure:"As soon as [past simple], [past simple].  /  No sooner had [subject] [past participle] than [past simple].",tip:'"No sooner had…" uses past perfect. No comma is needed when "as soon as" falls in the middle.'},connectorCondition:{icon:"🔐",rule:'"Unless" means "if not"; "provided that" means "on the condition that"',structure:"[Main clause] unless [condition].  /  [Main clause] provided that [condition].",tip:`Because "unless" already carries the "not", adding another one usually reverses your meaning by accident: "unless you do not hurry" says the opposite of what you meant. Say the condition positively — "unless you hurry". (Some sentences genuinely need both, as in "unless you don't mind waiting", so check the meaning rather than counting negatives.)`},activeToPassive:{icon:"🔄",rule:"Active → Passive shifts focus from the doer to the receiver",structure:"[Object] + was/were + past participle + by + [agent].",tip:'Choose was/were based on the new subject. Drop "by [agent]" if obvious or unimportant.'},passiveToActive:{icon:"🔁",rule:"Passive → Active: the agent becomes the subject, verb returns to active form",structure:"[Agent] + [active verb in correct tense] + [original object].",tip:'Remove "was/were…by" and reconstruct the verb in the matching tense.'},reportedSpeechStatement:{icon:"💬",rule:"Reported statements shift tense back and change pronouns and time words",structure:"[He/She] said (that) + [back-shifted verb clause].",tip:'"Yesterday" → "the day before"; "ago" → "before/earlier"; present → past; past → past perfect.'},reportedSpeechQuestion:{icon:"❓",rule:"Reported questions use statement word order — no inversion, no question mark",structure:"[He/She] asked [who/what/whether/if] + [subject] + [verb].",tip:'No auxiliary "do/does/did" after the question word. Use "if/whether" for yes/no questions.'},reportedSpeechCommand:{icon:"📢",rule:'Reported commands use "told/asked + object + to + base verb"',structure:"[He/She] told/asked [person] + (not) to + [base verb].",tip:'For negative commands: "told her not to…". Use "asked" for polite requests.'},relativeClause:{icon:"🔗",rule:"Relative clauses add information about a noun using who/which/whose/whom/that",structure:"[Main noun] + who/which/whose/that + [relative clause].",tip:'Non-defining clauses (extra info) use commas and cannot use "that". Defining clauses can use "that".'},comparison:{icon:"⚖️",rule:"Comparison: as…as (equal), not as…as (unequal), more/less…than (different degrees)",structure:"[Subject] is as [adjective] as [comparison].  /  [Subject] is more [adj] than [comparison].",tip:'"The more…the more" shows proportional increase. Both halves use the comparative form.'},advancedConstruction:{icon:"🏆",rule:'A fronted negative inverts the verb; the "It was not until…that" cleft does not',structure:"Fronted: [Never/Seldom/Not until …] + [auxiliary] + [subject] + [verb].  /  Cleft: It was not until [time] that + [subject] + [verb].",tip:'Compare: "Not until noon did he arrive." (fronted — inverted) with "It was not until noon that he arrived." (cleft — normal order). Same meaning, and only the fronted one inverts.'},causativeHave:{icon:"🛠️",rule:'"Have something done" shows that someone arranged for another person to perform an action',structure:"[Subject] + have/had + [object] + past participle.",tip:"The subject does NOT do the action — they arranged for someone else to do it."},cleftSentence:{icon:"🎯",rule:"Cleft sentences (It was/is…who/that) split a sentence to emphasise one element",structure:"It was/is + [emphasised element] + who/that + [rest of sentence].",tip:'Use "who" for people, "that" for things or ideas. Match "was/is" to the original tense.'},conditionalType2:{icon:"🌀",rule:"Type 2 conditional: imaginary or unlikely present/future situation",structure:"If + [past simple], [would/could/might] + [base verb].",tip:'Use "were" instead of "was" after "if" in formal writing: "If I were you…"'},conditionalType3:{icon:"⏮️",rule:"Type 3 conditional: imaginary past — something that did NOT happen",structure:"If + [past perfect], [would/could/might] + have + [past participle].",tip:'"Would have" is most common; "could have" = possibility; "might have" = weaker probability.'},despiteInSpiteOf:{icon:"🧱",rule:'"Despite" and "in spite of" show contrast — followed by a noun or -ing form, NOT a full clause',structure:"Despite/In spite of + [noun phrase or -ing form], [main clause].",tip:'Never write "despite of…". When a full clause follows the contrast, use "although/even though" instead.'}};let d=null,E=null,C=null,A=[],m=0,y=0,b=null;function le(){return{correct:0,total:0,firstTry:0,bySkill:{}}}function Ee(e,t){d=e,E=t}function ue(){d&&he()}function de(){d&&(d.innerHTML=""),d=null,E=null}function he(){var s;const e=["P4","P5","P6"],t=((s=x.get("questMastery"))==null?void 0:s.synthesisQuest)||{};d.innerHTML=`
    <div class="sq-browser">
      ...
      <div class="sq-level-grid">
        ${e.map(n=>{const a=F.filter(i=>i.level===n),o=[...new Set(a.map(i=>i.skillKey))],r=o.filter(i=>(t[i]||0)>=.7).length;return`
          <button class="sq-level-btn" data-level="${n}">
            <span class="sq-level-badge">${n}</span>
            <span class="sq-level-name">${ie[n]}</span>
    ...
    </div>`,d.querySelectorAll(".sq-level-btn").forEach(n=>{n.addEventListener("click",()=>G(n.dataset.level))})}function G(e){C=e,b=le(),A=pe(e),m=0,M()}function pe(e){const s=F.filter(n=>n.level===e).map(n=>{const a=L.getSkillScore("synthesisQuest",n.skillKey)??.5,o=(Math.random()-.5)*.12;return{item:n,score:1-a+o}});return s.sort((n,a)=>a.score-n.score),s.slice(0,ce).map(n=>n.item)}function M(){var n,a,o;if(m>=A.length){we();return}const e=A[m];y=0;const t=Math.round(m/A.length*100);d.innerHTML=`
    <div class="sq-game">
      <div class="sfq-header">
        <span class="sfq-badge">✏️ Synthesis — ${C}</span>
    ...
    </div>`;const s=document.getElementById("sq-answer-input");(n=document.getElementById("sq-check"))==null||n.addEventListener("click",()=>D(e)),s==null||s.addEventListener("keydown",r=>{r.key==="Enter"&&!r.shiftKey&&(r.preventDefault(),D(e))}),(a=document.getElementById("sq-hint-btn"))==null||a.addEventListener("click",()=>ye(e)),(o=document.getElementById("sq-quit"))==null||o.addEventListener("click",()=>{de(),E==null||E()}),s==null||s.focus()}function p(e){return e.toLowerCase().replace(/['']/g,"'").replace(/[""]/g,'"').replace(/\s+/g," ").replace(/[.,!?;:]+$/g,"").trim()}function fe(e,t){if(!e)return null;const s=p(e);if(!p(t).startsWith(s))return null;const a=e.trim().length;return t.trim().slice(a).replace(/^[\s,]+/,"").trim()}function Y(e){return e.allowAnyStructure?{requiredStart:null,requiredConnector:null}:{requiredStart:e.requiredStart??e.stem??null,requiredConnector:e.requiredConnector??null}}function me(e,t){const{requiredStart:s,requiredConnector:n}=Y(t),a=p(e||"");if(!a)return{ok:!0,violation:null,message:null};if(n&&!a.includes(p(n)))return{ok:!1,violation:"connector",message:`This one asks you to use "${n}".`};if(s){const o=p(s);if(a.startsWith(o))return{ok:!0,violation:null,message:null};if(W.some(i=>o===i||o.startsWith(`${i} `))){if(a.includes(` ${o} `))return{ok:!1,violation:"connector-position",message:`You used "${s}" correctly — but this task asks you to begin the sentence with it. Move it to the front and rewrite.`};if(W.some(h=>a.startsWith(`${h} `)&&!o.startsWith(h)))return{ok:!1,violation:"start",message:`Good sentence — but this task asks you to begin with "${s}". Try again starting there.`}}}return{ok:!0,violation:null,message:null}}const W=Object.freeze(["although","though","even though","even if","despite","in spite of","because","since","as soon as","unless","until","whereas","while","if","not only","no sooner","hardly","provided that","so that","in order to","after","before","when","whenever"]);function ve(e){const{requiredStart:t}=Y(e);if(!t)return[];const s=p(t);return(e.alternates||[]).filter(n=>!p(n).startsWith(s))}function be(e){const t=new Set(ve(e).map(r=>p(r))),s=[e.answer,...e.alternates||[]].filter(Boolean).filter(r=>!t.has(p(r))),n=[];for(const r of s){const i=fe(e.stem,r);i&&n.push(i)}const a=new Set,o=[];for(const r of[...n,...s]){const i=p(r);!i||a.has(i)||(a.add(i),o.push(r))}return o}function ge(e,t){const s=be(t);return oe(e,{expected:s[0]||"",accepts:s.slice(1),requiredGroups:Array.isArray(t.requiredGroups)?t.requiredGroups:null})}async function D(e){var I,w,$;const t=document.getElementById("sq-answer-input"),s=((I=t==null?void 0:t.value)==null?void 0:I.trim())||"";if(!s){v("Type your rewritten sentence first.","neutral");return}y++;const n=me(s,e);if(!n.ok){if(_(e,!1),y>=O){Q(e,()=>{m++,M()},s);return}v(`◐ ${n.message}`,"hint"),t==null||t.focus(),t==null||t.select();return}const a=ge(s,e),o=a.fraction>=1,r=a.fraction>0&&a.fraction<1;if(!o&&J()){v("✨ Checking with AI…","hint");const l=document.getElementById("sq-check");l&&(l.disabled=!0);const g=e.alternates||[],u=await ee(e.original,e.stem||"",e.answer,g,s,e.skill||e.skillKey);if(l&&(l.disabled=!1),(u==null?void 0:u.verdict)==="CORRECT"){_(e,!0,s,U.GUIDED);return}if((u==null?void 0:u.verdict)==="PARTIAL"){_(e,!1);const f=k[e.skillKey],q=f!=null&&f.structure?`Structure: ${f.structure}`:`Use the "${e.pattern||e.skill}" pattern.`,j=u.feedback?` ${u.feedback}`:"";v(`◐ Almost!${j} ${q}`,"hint"),t&&(t.value=""),t==null||t.focus();return}}if(o){_(e,!0,s);return}if(_(e,!1),y>=O){Q(e,()=>{m++,M()},s);return}const i=k[e.skillKey],h=i!=null&&i.structure?`Structure: ${i.structure}`:`Use the "${e.pattern||e.skill}" pattern.`;if(r){const l=Math.round(a.fraction*100),g=((w=a.trace)==null?void 0:w.hits)||[],u=(($=a.trace)==null?void 0:$.misses)||[],f=g.length?` ✓ ${g.join(", ")}`:"",q=u.length?` ✗ still need: ${u.join(", ")}`:"";v(`◐ Partial (${l}% structure). ${h}${f}${q}`,"hint")}else{const l=X(e,s);v(`Not yet. ${l.misconception.cue} ${h}`,"error")}t==null||t.focus(),t==null||t.select()}function qe(e){const t=String(e.stem||"").trim();return t?t.split(/\s+/).filter(Boolean):[]}function X(e,t){return re({given:t,model:e.answer,stem:e.original||"",requiredWords:qe(e),domain:"writing"})}function _(e,t,s,n=U.INDEPENDENT){if(te.playSfx(t?"correct":"wrong"),y===1){const a=b.bySkill[e.skillKey]||{correct:0,total:0,label:e.skill};a.total++,b.total++,L.recordAttempt({quest:"synthesisQuest",skill:e.skillKey,correct:t,level:C}),t&&(a.correct++,b.correct++,b.firstTry++),b.bySkill[e.skillKey]=a,L.updateSkill("synthesisQuest",e.skillKey,t,{evidence:n})}t&&s!==void 0&&ke(e,s)}function ye(e){const t=k[e.skillKey];v(`💡 ${(t==null?void 0:t.tip)||`Begin your sentence with: "${e.stem||e.pattern}"`}`,"hint")}function v(e,t){const s=document.getElementById("sq-feedback");s&&(s.hidden=!1,s.className=`sfq-feedback${t==="error"?" sfq-feedback--error":t==="hint"?" sfq-feedback--hint":""}`,s.textContent=e)}function ke(e,t){var r;const s=d.querySelector(".sq-game");if(!s)return;const n=y===1?"⭐⭐":"⭐",a=document.createElement("div");a.className="sq-success-overlay";const o=e.stem?`${e.stem} ${t.trim()}`:t.trim();a.innerHTML=`
    ...
    </div>`,s.appendChild(a),a.querySelector(".sq-success-next").addEventListener("click",()=>{a.remove(),m++,M()})}function Q(e,t,s=""){var i;const n=k[e.skillKey]||{icon:"📘",rule:e.skill,structure:"",tip:""},a=d.querySelector(".sq-game");if(!a){t();return}const o=s?X(e,s):null;o&&se(o.id,{skill:e.skillKey,mode:"synthesisQuest"});const r=document.createElement("div");r.className="eq-teachback-overlay",r.setAttribute("role","dialog"),r.setAttribute("aria-modal","true"),r.setAttribute("aria-label","Grammar pattern explanation"),r.innerHTML=`
    ...
    </div>`,a.appendChild(r),s&&ne(r.querySelector(".eq-teachback-card"),()=>ae({skillLabel:e.skill||e.skillKey,exercise:`Rewrite: "${e.original}"${e.stem?` starting with "${e.stem}"`:""}`,studentAnswer:s,correctAnswer:e.answer,level:C})),r.querySelector(".eq-teachback-continue").addEventListener("click",()=>{r.remove(),t()})}function we(){var u,f,q,j,B;const{correct:e,total:t,firstTry:s,bySkill:n}=b,a=t>0?e/t:0,o=t>0?s/t:0,r=a>=.9&&o>=.7?3:a>=.7?2:1,i=Math.round(30+e*8+(r-1)*10),h=(x.get("xp")||0)+i,I=Z(h);x.patch({xp:h,level:I.level});const w=Object.entries(n).sort((S,T)=>S[1].correct/Math.max(S[1].total,1)-T[1].correct/Math.max(T[1].total,1)),$=w.map(([S,T])=>{var R,P,H,K;const N=Math.round(T.correct/Math.max(T.total,1)*100),z=((K=(H=(P=(R=k[S])==null?void 0:R.rule)==null?void 0:P.split(":")[0])==null?void 0:H.split("—")[0])==null?void 0:K.trim())||S,V=N>=70?"var(--color-success)":N>=40?"var(--color-primary)":"var(--color-error)";return`
      <div class="sq-skill-row">
        <span class="sq-skill-name">${c(z)}</span>
        <div class="sq-skill-track"><div class="sq-skill-bar" style="width:${N}%;background:${V}"></div></div>
        <span class="sq-skill-pct">${N}%</span>
    ...
    </div>`,(j=document.getElementById("sq-retry"))==null||j.addEventListener("click",()=>G(C)),(B=document.getElementById("sq-home"))==null||B.addEventListener("click",()=>ue())}function c(e){return String(e??"").replace(/[<>&"']/g,t=>({"<":"&lt;",">":"&gt;","&":"&amp;",'"':"&quot;","'":"&#39;"})[t])}export{k as SQ_TEACHBACK,be as buildAcceptableAnswers,me as checkTaskConstraints,de as cleanupSynthesisQuest,Y as getTaskConstraints,Ee as initSynthesisQuest,ve as offTaskAlternates,ue as showSynthesisBrowser};
