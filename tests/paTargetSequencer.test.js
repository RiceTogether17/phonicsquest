...
const SHORT_A_POOL = [
  { id: 'bag', word: 'bag', graphemes: ['b','a','g'], types: ['c','sv','c'], group: 'short-a', level: 1 },
  { id: 'bat', word: 'bat', graphemes: ['b','a','t'], types: ['c','sv','c'], group: 'short-a', level: 1 },
  { id: 'bad', word: 'bad', graphemes: ['b','a','d'], types: ['c','sv','c'], group: 'short-a', level: 1 },
  { id: 'cab', word: 'cab', graphemes: ['c','a','b'], types: ['c','sv','c'], group: 'short-a', level: 1 },
  { id: 'can', word: 'can', graphemes: ['c','a','n'], types: ['c','sv','c'], group: 'short-a', level: 1 },
  { id: 'cap', word: 'cap', graphemes: ['c','a','p'], types: ['c','sv','c'], group: 'short-a', level: 1 },
  { id: 'cat', word: 'cat', graphemes: ['c','a','t'], types: ['c','sv','c'], group: 'short-a', level: 1 },
  { id: 'dad', word: 'dad', graphemes: ['d','a','d'], types: ['c','sv','c'], group: 'short-a', level: 1 },
  { id: 'dam', word: 'dam', graphemes: ['d','a','m'], types: ['c','sv','c'], group: 'short-a', level: 1 },
  { id: 'fan', word: 'fan', graphemes: ['f','a','n'], types: ['c','sv','c'], group: 'short-a', level: 1 },
  { id: 'hat', word: 'hat', graphemes: ['h','a','t'], types: ['c','sv','c'], group: 'short-a', level: 1 },
  { id: 'ham', word: 'ham', graphemes: ['h','a','m'], types: ['c','sv','c'], group: 'short-a', level: 1 },
];

const SHORT_E_POOL = [
  { id: 'bed', word: 'bed', graphemes: ['b','e','d'], types: ['c','sv','c'], group: 'short-e', level: 1 },
  { id: 'beg', word: 'beg', graphemes: ['b','e','g'], types: ['c','sv','c'], group: 'short-e', level: 1 },
  { id: 'hen', word: 'hen', graphemes: ['h','e','n'], types: ['c','sv','c'], group: 'short-e', level: 1 },
];
...
describe('First Sound — alphabetical ladder of initial phonemes', () => {
  it('visits initial graphemes alphabetically, 3 exemplars per target', () => {
    ...
    const words = runSession(state, {
      mode: 'first', group: 'short-a', wordList: SHORT_A_POOL,
    }, 12);
...
describe('Last Sound — alphabetical ladder of final phonemes', () => {
  it('visits final graphemes alphabetically', () => {
    const state = createPaSequencerState();
    const words = runSession(state, {
      mode: 'last', group: 'short-a', wordList: SHORT_A_POOL,
    }, 8);
...
describe('Middle Sound — single-vowel stage collapses ladder to one rung', () => {
  it('cycles all words alphabetically by word when only one medial grapheme exists', () => {
    const state = createPaSequencerState();
    const words = runSession(state, {
      mode: 'middle', group: 'short-a', wordList: SHORT_A_POOL,
    }, 6);
...
describe('Edge cases', () => {
  ...
  it('honours maxLevel filter', () => {
    ...
    const mixed = [
      ...SHORT_A_POOL,
      { id: 'flat', word: 'flat', graphemes: ['f','l','a','t'], types: ['c','c','sv','c'], group: 'short-a', level: 2 },
    ];
    const words = runSession(state, {
      mode: 'first', group: 'short-a', maxLevel: 1, wordList: mixed,
    }, 12);
