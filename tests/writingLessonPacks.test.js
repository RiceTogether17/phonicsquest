...
import {
  ...
  getPlanDraftMatchReport,
  _extractPlanKeywords,
  _expandWithSynonyms,
  _splitIntoSections,
...
import {
  ...
  getLevelTrackProgress,
  isLevelFullyComplete,
} from '../src/modules/writingTrackProgress.js';
import {
  scoreClimaxPresence,
  scoreResolutionPresence,
  scoreReflectionEnding,
  scoreDialoguePurpose,
  scoreNarrativeArc,
  scoreChronologicalFlow,
  computeNarrativeQuality,
} from '../src/modules/writingNarrativeHelpers.js';
...
describe('writing lesson packs schema', () => {
  it('validates all packs in P3 tracks', () => {
    const lessons = [
      ...getLessonsForTrack(WRITING_TRACKS.p3t1Creative.id),
      ...getLessonsForTrack(WRITING_TRACKS.p3t2Creative.id),
    ];
...
describe('narrative analysis helpers', () => {
  ...
  it('detects resolution markers and actions', () => {
    expect(
      scoreResolutionPresence('in the end we found the key and solved the puzzle'),
    ).toBeGreaterThan(0.3);
    expect(scoreResolutionPresence('i walked to school')).toBe(0);
  });

  it('detects reflection near the end of text', () => {
    const textWithEndReflection =
      'The day began with sunshine. We played all morning. After lunch we explored the garden. I learned that teamwork makes things easier.';
  ...
  it('scores reflection higher when at the end vs start', () => {
    const reflectionAtEnd =
      'We went to the park. We played games. We had fun. After everything, I learned to be patient.';
    const reflectionAtStart =
      'I learned to be patient. We went to the park. We played games. We had fun all day long.';
    expect(scoreReflectionEnding(reflectionAtEnd)).toBeGreaterThan(
      scoreReflectionEnding(reflectionAtStart),
    );
  });
  ...
  it('scores narrative arc completeness', () => {
    const fullArc =
      'The school hall was quiet that morning. But then I noticed something strange under the bench. Suddenly a cat leaped out and startled everyone. In the end the teacher laughed and we all felt relieved.';
  ...
  it('scores chronological flow with correct order bonus', () => {
    const ordered =
      'First I packed my bag. Then I walked to school. Suddenly the bell rang. Finally I reached my class.';
  ...
  it('computes aggregate narrative quality', () => {
    const goodNarrative =
      'The corridor was dark and cold. I heard footsteps behind me. "Run!" whispered my friend, grabbing my sleeve. Suddenly the lights went out. We stumbled forward, hearts pounding. In the end we found the exit and I learned to trust my instincts.';
...
describe('improved evaluator with narrative quality', () => {
  it('includes narrativeQuality in metrics', () => {
    ...
    const text =
      'The court was empty. First I found an umbrella. Suddenly a cat appeared. Finally I went home. In the end I learned to be careful.';
  ...
  it('rewards richer narratives with higher scores', () => {
    ...
    const rich =
      'A thud jolted me awake. I tiptoed to the dark kitchen, heart pounding. "Who is there?" I whispered. Suddenly my brother popped up, laughing. He was making noodles. In the end I learned not to panic before checking.';
...
describe('section-aware paragraph missions', () => {
  ...
  it('splits text into sections by sentences for single-paragraph text', () => {
    const text =
      'The court was empty. I found an umbrella on the ground. A classmate was searching nearby. Finally we found the keychain together.';
    const sections = _splitIntoSections(text);
  ...
  it('backward-compatible: missions still match on full text', () => {
    ...
    const text =
      'Steam rose from the stall. Suddenly I slipped, but finally we were safe in the end.';
    const status = getParagraphMissionStatus(lesson, text);
...
describe('plan-to-draft validation', () => {
  ...
  it('creates plan checks with synonym-expanded keywords', () => {
    ...
    expect(
      keywords.some((k) => k === 'rainy' || k === 'court' || k === 'scared' || k === 'afraid'),
    ).toBe(true);
  });
  ...
  it('generates plan-draft match report with section scores', () => {
    ...
    const draft =
      'The basketball court was wet with rain and puddles everywhere. I searched under the bench and found the missing keychain.';
...
describe('revise drills', () => {
    ...
    const drills = [
      {
        ...
        options: ['Weak.', 'Strong and vivid.', 'Bad.'],
        correctIndex: 1,
      },
    ];
  ...
  it('grades dialogue_improve as MCQ', () => {
    const drills = [
      { type: 'dialogue_improve', correctIndex: 1, options: ['Weak dialogue', 'Strong dialogue'] },
    ];
    expect(gradeDrills(drills, ['1']).correctCount).toBe(1);
  });
  ...
  it('renders different drill types with distinct markup', () => {
    ...
    const upgrade = renderDrill(
      { type: 'sentence_upgrade', weakSentence: 'Bad.', options: ['a', 'b'], correctIndex: 1 },
      1,
    );
    expect(upgrade).toContain('Sentence Upgrade');

    const arrange = renderDrill(
      { type: 'arrange_sequence', sentences: ['A.', 'B.'], correctOrder: [0, 1] },
      2,
    );
    expect(arrange).toContain('Arrange the Story');

    const revision = renderDrill(
      { type: 'choose_best_revision', original: 'Old.', options: ['a', 'b'], correctIndex: 1 },
      3,
    );
    expect(revision).toContain('Better Revision');

    const dialogue = renderDrill(
      { type: 'dialogue_improve', weakDialogue: '"Hi."', options: ['a', 'b'], correctIndex: 1 },
      4,
    );
  ...
  it('lesson packs use the new drill types', () => {
    const allDrills = Object.values(writingLessonPacks).flatMap((p) => p.reviseDrills || []);
