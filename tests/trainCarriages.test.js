...
stubAudioGlobals();
const { pickTrainRound, setupTrainCarriages, cleanup } = await import('../src/modes/trainCarriages.js');
...
const POOL = [
  ...
  { id: 'bad', word: 'bad', graphemes: ['b','a','d'], types: ['c','sv','c'], group: 'short-a', emoji: '😠' },
  { id: 'bag', word: 'bag', graphemes: ['b','a','g'], types: ['c','sv','c'], group: 'short-a', emoji: '👜' },
  { id: 'bat', word: 'bat', graphemes: ['b','a','t'], types: ['c','sv','c'], group: 'short-a', emoji: '🦇' },
  // /c/
  { id: 'cab', word: 'cab', graphemes: ['c','a','b'], types: ['c','sv','c'], group: 'short-a', emoji: '🚕' },
  { id: 'can', word: 'can', graphemes: ['c','a','n'], types: ['c','sv','c'], group: 'short-a', emoji: '🥫' },
  { id: 'cat', word: 'cat', graphemes: ['c','a','t'], types: ['c','sv','c'], group: 'short-a', emoji: '🐱' },
  // others
  { id: 'dad', word: 'dad', graphemes: ['d','a','d'], types: ['c','sv','c'], group: 'short-a', emoji: '👨' },
  { id: 'pan', word: 'pan', graphemes: ['p','a','n'], types: ['c','sv','c'], group: 'short-a', emoji: '🍳' },
  { id: 'ran', word: 'ran', graphemes: ['r','a','n'], types: ['c','sv','c'], group: 'short-a', emoji: '🏃' },
  { id: 'tan', word: 'tan', graphemes: ['t','a','n'], types: ['c','sv','c'], group: 'short-a', emoji: '🍂' },
  { id: 'ham', word: 'ham', graphemes: ['h','a','m'], types: ['c','sv','c'], group: 'short-a', emoji: '🍖' },
];

describe('pickTrainRound', () => {
  it('returns exactly cardCount cards with matchCount matches by default', () => {
    const { matches, distractors } = pickTrainRound('b', 'c', POOL, { cardCount: 6, matchCount: 3 });
